...
export function accountCard(account, displayedAccountId = "", { privacyMode, prices, displayCurrency }) {
  ...
  const addressText = account.network
    ? `${shortAddress(account.address)} (${account.network})`
    : shortAddress(account.address);
  title.append(
    textNode("strong", account.name, "wallet-card-name"),
    textNode("span", addressText, "wallet-card-address"),
  );
  identity.append(
  ...
  const assets = account.assets.slice(0, 3);
  if (assets.length === 0) {
    card.append(top);
    return card;
  }
