...
mod tests {
    ...
    fn custom_mount_persists_and_resolves_mapped_handles() {
        ...
        let record = state
            .upsert_mount(
                "Cloud".to_string(),
                "cloud://drive".to_string(),
                None,
                Some("cloud-drive".to_string()),
                Some("Cloud Drive WebSpace mount.".to_string()),
                Some(true),
        ...
        assert_eq!(record.moniker, "Cloud");
        ...
        assert!(entries.iter().any(|entry| entry.name == "Cloud"));

        let resolved = resolve_path(
            &reloaded,
            "localhost://WebSpaces/Cloud/Drive/Project X/file.pdf",
        )
        ...
        match resolved {
            ResolvedPath::Handle { handle } => {
                ...
                assert_eq!(
                    ...
                    Some("cloud://drive/Drive/Project X/file.pdf")
                );
                assert_eq!(handle.resolver, "cloud-drive");
    ...
    fn custom_mount_index_persists_and_lists_children() {
            ...
            .replace_index(
                ...
                vec![
                    ...
                    WebSpaceIndexInput {
                        ...
                        kind: "file".to_string(),
                        target_uri: Some("cloud://drive/shared/report.md".to_string()),
                        resolver_state: Some("indexed".to_string()),
                        readonly: None,
        ...
        let cloud = resolve_path(&reloaded, "localhost://WebSpaces/Cloud")
            .expect("Cloud mount should resolve");
        let entries = list_for(&reloaded, &cloud).expect("Cloud mount should list");
        ...
        let project = resolve_path(&reloaded, "localhost://WebSpaces/Cloud/Drive/Project X")
    ...
    fn refresh_replaces_index_and_persists_refreshed_head() {
        ...
        let receipt = state
            .refresh_handle(
                "localhost://WebSpaces/Cloud".to_string(),
                Some(vec![
                    WebSpaceIndexInput {
                        path: "Drive".to_string(),
                        kind: "directory".to_string(),
    ...
    fn health_reports_external_resolver_attention_and_metadata_readiness() {
        ...
        let cloud = attention["mounts"]
            .as_array()
            .unwrap()
            .iter()
            .find(|mount| mount["moniker"] == "Cloud")
            .expect("Cloud health should be listed");
        assert_eq!(cloud["state"], "mounted_no_index");
        assert_eq!(cloud["live_adapter"], false);
        assert_eq!(cloud["adapter_state"], "not_registered");
        ...
        let ready = state
            .health_report(Some("Cloud".to_string()))
        ...
        state
            .fork_mount(
                "localhost://WebSpaces/Cloud/Drive/file.pdf".to_string(),
        ...
        let cloud_still_ready = state
            .health_report(Some("Cloud".to_string()))
            .expect("filtered Cloud health should ignore fork heads");
        assert_eq!(cloud_still_ready["state"], "metadata_ready");
        assert_eq!(cloud_still_ready["dirty_head_count"], 0);
    ...
    fn adapter_registry_persists_and_informs_health() {
        ...
        let adapter = state
            .upsert_adapter(
                ...
                Some("cloud-drive-provider".to_string()),
                Some("connected".to_string()),
                vec!["metadata_index".to_string(), "read_bytes".to_string()],
                Some(true),
                Some("Cloud Drive resolver adapter.".to_string()),
            )
            .expect("adapter should register");
        assert_eq!(adapter.resolver, "cloud-drive");
        ...
        let cloud = health["mounts"]
            .as_array()
            .unwrap()
            .iter()
            .find(|mount| mount["moniker"] == "Cloud")
            .expect("Cloud mount should be listed");
        ...
        assert_eq!(reloaded.adapters[0].resolver, "cloud-drive");
        ...
        let removed = reloaded
            .unregister_adapter("cloud-drive")
            .expect("adapter should unregister");
        assert_eq!(removed.resolver, "cloud-drive");
    ...
    fn unmount_removes_index_and_head_state() {
        ...
        state
            ...
            .expect("index should persist");
        let handle = handle_from_resolved_path(
            resolve_path(&state, "localhost://WebSpaces/Cloud/Drive/file.pdf")
                .expect("indexed file should resolve"),
        )
        .expect("resolved path should be a handle");
        state
        ...
        state.unmount("Cloud").expect("unmount should succeed");
    ...
    fn fork_creates_mutable_mount_and_dirty_head() {
        ...
        let (record, head) = state
            .fork_mount(
                "localhost://WebSpaces/Cloud/Drive/Project X/file.pdf".to_string(),
                "ProjectFork".to_string(),
                None,
                None,
                None,
        ...
        assert_eq!(
            ...
            Some("localhost://WebSpaces/Cloud/Drive/Project X/file.pdf")
        );
    ...
    fn cache_handle_materializes_adapter_bytes_without_dirty_sync_debt() {
        ...
        state
            .upsert_adapter(
                "cloud-drive".to_string(),
                Some("Cloud Drive".to_string()),
                Some("provider:cloud-drive-adapter".to_string()),
                Some("cloud-drive-adapter".to_string()),
                Some("connected".to_string()),
                vec!["metadata_index".to_string(), "read_bytes".to_string()],
                Some(true),
                Some("Cloud Drive adapter.".to_string()),
        ...
        state
            .replace_index(
                ...
                vec![WebSpaceIndexInput {
                    path: "Drive/Project X/file.pdf".to_string(),
                    kind: "file".to_string(),
                    target_uri: Some("cloud://drive/Drive/Project X/file.pdf".to_string()),
                    resolver_state: Some("indexed".to_string()),
                    readonly: Some(true),
