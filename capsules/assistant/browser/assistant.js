...
const MAX_STUDIO_RESOURCE_ID_BYTES = 4 * 1024;
...
function validStudioResourceId(resourceId) {
  return (
    typeof resourceId === "string" &&
    resourceId.length > 0 &&
    resourceId === resourceId.trim() &&
    TEXT_ENCODER.encode(resourceId).length <= MAX_STUDIO_RESOURCE_ID_BYTES &&
    !/[\u0000-\u001f\u007f]/u.test(resourceId)
  );
}
...
function parseStudioOutput(output) {
  if (
    ...
    !Object.prototype.hasOwnProperty.call(output, "resource_id")
  ) {
    return null;
  }
  if (
    output.schema === MODEL_OBJECT_OUTPUT_SCHEMA &&
    validStudioResourceId(output.resource_id)
  ) {
    return { schema: output.schema, resourceId: output.resource_id };
  }
  if (
...
export function mountAssistantApp(root, app) {
  ...
  function render(view) {
    ...
    studioResultUriNode.textContent = view.studioResult?.resourceId || "";
