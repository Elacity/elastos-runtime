...
window.API = (function () {
  ...
  // The runtime launches this capsule with the token in the URL FRAGMENT (`#home_token=…`) — a
  // fragment is never sent to any server, so it stays out of Referer, access logs and history
  // (gateway_home_runtime.rs::append_home_launch_token_to_route). Same idiom as marketplace.js and
  // people.js. Gated GET/POSTs carry it as a header instead.
  const homeToken = () => {
    try {
      return new URLSearchParams(location.hash.replace(/^#/, "")).get("home_token") || "";
    } catch { return ""; }
  };
  // Home's real origin, handed to every launched capsule as a query param. This frame is
  // opaque-origin (`location.origin === "null"`), so it is the only usable postMessage target.
  const homeParentOrigin = (() => {
    try { return new URL(location.href).searchParams.get("home_origin") || ""; } catch { return ""; }
  })();
  // Home binds this frame's message source on its FIRST `home:app-ready` and drops every message
  // from an unbound source (home-shell-host.js::homeMessageContext), so the handshake has to run
  // before any brokered request. Same idiom as marketplace.js.
  try {
    const bootToken = homeToken();
    if (bootToken && homeParentOrigin && window.top !== window) {
      window.top.postMessage({ type: "home:app-ready", homeToken: bootToken }, homeParentOrigin);
    }
  } catch { /* standalone review: no Home host to greet */ }
  ...
  function launch(target, query) {
    try {
      const t = homeToken();
      if (!t || !homeParentOrigin || window.top === window) return false;
      window.top.postMessage({ type: "home:open-target", target, query: query || {}, homeToken: t }, homeParentOrigin);
      return true;
    } catch { return false; }
  }

  // Request/reply against the Home shell host. Home settles every brokered request with a
  // `home:shell-response` carrying our `requestId` (home-shell-host.js::replyToShellRequest).
  const HOME_REQUEST_TIMEOUT_MS = 120_000;
  function requestHome(type, payload, timeoutMs = HOME_REQUEST_TIMEOUT_MS) {
    const t = homeToken();
    if (!t || !homeParentOrigin || window.top === window) {
      return Promise.reject(new Error("not running inside Home"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const onMessage = (event) => {
        if (event.source !== window.top || event.origin !== homeParentOrigin) return;
        const message = event.data && typeof event.data === "object" ? event.data : null;
        if (!message || message.type !== "home:shell-response" || message.requestId !== requestId) return;
      ...
      timer = window.setTimeout(() => {
        window.removeEventListener("message", onMessage);
        reject(new Error("Home did not answer the request"));
      }, timeoutMs);
      window.addEventListener("message", onMessage);
      window.top.postMessage({ type, requestId, homeToken: t, ...payload }, homeParentOrigin);
    });
  }
  ...
  return {
    ...
    async assembleOrder({ content_id, quantity, seller, price, pay_token }) {
      ...
      //
      // We do NOT call the route ourselves. `/api/market/buy` accepts only a `home` launch token
      // presented from the exact Home origin, and it demands a fresh step-up bound to this exact
      // body; this frame is opaque-origin and holds a `marketplace-content` token, so it can satisfy
      // neither. Home brokers it (home-shell-host.js::runMoneyVerb): it runs the step-up ceremony
      // over the intent below verbatim and posts the spend under its own cookie authority. The
      // step-up token never enters this frame.
      ...
      if (liveMode()) {
        // JSON round-trip drops the `undefined` members, so the object Home signs over is byte-for-byte
        // the object the gateway re-hashes. Sending them raw would put `null`s in the postMessage clone.
        const body = JSON.parse(JSON.stringify(intent));
        try {
          const result = await requestHome("elastos.home.money-verb.request/v1", {
          ...
          // Honest failure in live mode: surface Home's/the gateway's refusal verbatim (no mock).
          return { error: `buy refused${err && err.message ? ": " + String(err.message).slice(0, 400) : ""}` };
        }
      }
