...
pub fn encrypt_samples(
    ...
    for (i, &size) in sample_sizes.iter().enumerate() {
        ...
        let iv16 = cenc_core::pad_iv(&iv8).expect("8-byte iv");

        if protected > 0 {
            cenc_core::ctr_xor(&mut output[offset + clear..offset + clear + protected], cek, &iv16);
        }
...
///
/// Thin wrapper kept for callers (this module's own tests) that rely on the
/// infallible `&[u8; 8]` signature; the actual padding is `cenc-core`'s.
pub fn pad_iv(iv8: &[u8; 8]) -> [u8; 16] {
    cenc_core::pad_iv(iv8).expect("8-byte iv")
}
...
mod tests {
    ...
    // Independent reference cipher for these round-trip tests: `encrypt_samples`
    // now delegates its keystream to `cenc-core`, so decrypting here with a
    // separately-instantiated AES-128-CTR (rather than reusing `cenc_core::ctr_xor`)
    // is what proves the keystream is unchanged, not just self-consistent.
