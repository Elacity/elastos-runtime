/* Agent Harness — Home drops, Shelf stays as composer hinge.
   UI ≠ authority (Principle 16): never mints grants, never opens Carrier/
   capsule ambient paths. Inference is the typed model contract or nothing. */
...
import { clampReasoningEffort, stampMessageNode } from "./agent-context.js";
import { registerAgentHarnessApi } from "./agent-send.js";
import {
...
/* No seeded conversation: a fresh Home starts empty and the first chat is the
   user's own. */
const SEED_SESSIONS = [];
...
/** Follow-up prompts queued while a turn is streaming (fx7). */
let followUpQueue = [];
...
/** Prompt prefs — persisted in the capsule workspace. */
let systemPrompt = "";
/** Wave 7 — sticky On-Home notes (host-persisted; appended to Live system). */
let agentNotes = "";
let maxTokens = 8192;
/** Requested reasoning effort — recorded on the turn manifest; the typed
    text input carries no effort knob, so no chip claims one. */
...
function setSidebarCollapsed(collapsed) {
  ...
  /* The rail's track eases shut or open and main's left edge moves with it;
     the dock does not resize, so the observer stays quiet. Follow the whole
     ease so the stream column stays on the Shelf composer throughout. */
  const ms = parseFloat(
    getComputedStyle(document.documentElement).getPropertyValue("--harness-sidebar-ms"),
  );
  trackComposerGeometry((Number.isFinite(ms) && ms > 0 ? ms : 360) + 80);
}

let composerGeometryTrackRaf = 0;

function trackComposerGeometry(durationMs) {
  cancelAnimationFrame(composerGeometryTrackRaf);
  const until = performance.now() + durationMs;
  const tick = () => {
    syncComposerGeometry({ tracking: true });
    composerGeometryTrackRaf = performance.now() < until ? requestAnimationFrame(tick) : 0;
  };
  composerGeometryTrackRaf = requestAnimationFrame(tick);
}
...
/* `tracking`: a bounded per-frame follow of a CSS ease (the rail collapse);
   it is not a ResizeObserver feedback loop, so the burst guard stands aside. */
function syncComposerGeometry({ tracking = false } = {}) {
  ...
  /* The column follows the pill only while the pill is the composer. Once the
     face drops for the leave, the pill is 28px idle glass and the room, still
     fading, would fold its transcript into that width. */
...
/** Suspend dock geometry work for the whole turn (stream growth). */
...
/* Quick switcher: the model offers advertised on this Home. */
...
export function showAgentHarness({
  ...
  /* Home's morph raises the composer face before the room; a restore or a
     Send that lands here without one settles it so the pill is never empty. */
  if (!fromShelf) {
...
function teardownHarnessDom(motionGen) {
...
function scheduleHarnessTeardown(motionGen) {
  if (prefersReducedMotion()) {
    teardownHarnessDom(motionGen);
    return;
  }
  window.setTimeout(() => teardownHarnessDom(motionGen), HOME_RISE_MS);
}

export function hideAgentHarness({ syncStage = true } = {}) {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (sidebarNav?.dataset.sidebarNav) {
      ...
      if (sidebarNav.classList.contains("is-active")) {
        ...
        openHarnessPage(dest, dest === "configure" ? { section: "models" } : {});
      }
      return;
    }
    ...
    if (sessionMenuBtn?.dataset.sessionMenu) {
      ...
      if (sessionActionsOpen() && currentSessionActionsId() === id) {
        closeSessionActions();
      } else {
    ...
    if (sessionProject?.dataset.sessionProject) {
      event.preventDefault();
      const id = currentSessionActionsId();
