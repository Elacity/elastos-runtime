...
/* What the typed text contract (elastos.model.input.text/v1) lets a turn carry.
   No provider or model is named here — the offer chosen at run time is the
   only model this capsule knows. */
export const TEXT_CONTRACT_CAPABILITIES = {
...
export function supportedReasoningEfforts(caps = TEXT_CONTRACT_CAPABILITIES) {
  return Array.isArray(caps?.reasoning?.efforts) ? caps.reasoning.efforts : [];
}

export function resolveReasoningPolicy(requested, caps = TEXT_CONTRACT_CAPABILITIES) {
  ...
  return {
    ...
    model: caps.model,
    degraded: true,
    reason: efforts.length ? "unsupported-effort" : "text-input-has-no-effort-param",
  };
}
...
export function attachProviderPayload(compiled) {
  const messages = serializeContractContext(compiled.items);
...
export function serializeContractContext(items = []) {
  const out = [];
