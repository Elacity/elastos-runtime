...
export function appendGrantCard(spec) {
  ...
  // Preview theatre is stripped: non-Inbox grants do not mint mock approvals.
  // Library/wallet authority is Runtime Inbox only (A8).
  if (state === "pending" && !approvalId && !spec.inbox && !spec.requestId) {
    state = "denied";
    summary = summary || "Preview path cannot mint grants — use Inbox";
    spec.state = state;
