...
export const PREFERRED_STATUS_VISIBLE_MS = 2500;
...
const PHASE_HEADING = [
  [ProgressPhase.SEARCHING, /\b(search|look(?:ing)? up|find(?:ing)? sources)/i],
  [ProgressPhase.READING, /\b(read(?:ing|s)?|review|inspect|skim)/i],
  [ProgressPhase.CODING, /\b(implement|cod(?:e|ing)|patch|refactor)/i],
  [ProgressPhase.RUNNING, /\b(run(?:ning)?|execute|benchmark)/i],
  [ProgressPhase.VERIFYING, /\b(verif|validat|test(?:ing)?|check(?:ing)? (?:the )?(?:result|fix|implementation))/i],
  [ProgressPhase.SYNTHESIZING, /\b(synthes|summar|pull(?:ing)? (?:it|the findings)|conclusion)/i],
  [ProgressPhase.ANALYZING, /\b(analy[sz]|compar|deconstruct|consider|evaluat)/i],
  [ProgressPhase.PLANNING, /\b(plan(?:ning)?|approach|outline)/i],
];
...
export function inferProgressHint(reasoningText) {
  ...
  const finding = heading.match(
    /\bfound (?:a |the )?(?:likely )?(?:cause|bottleneck|issue|bug|problem)\b/i,
  );
  if (finding && sanitizeSubject(heading)) {
    return {
      type: "FINDING",
      importance: "high",
      text: heading.replace(/[.]+$/, "").slice(0, 120),
      key: `finding:${heading.slice(0, 48).toLowerCase()}`,
    };
  }
  for (const [phase, re] of PHASE_HEADING) {
    if (re.test(heading)) {
      return {
        type: "PHASE_CHANGED",
        phase,
