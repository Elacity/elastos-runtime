...
import { stampMessageNode, contentHash, newTurnId, createTurnManifest, turnStorePut, turnStorePatch, turnStoreGet, TurnState, cheapTurnSnapshot, resolveReasoningPolicy } from "./agent-context.js?v=home-20260814a";
import { setAgentComposerProcessing } from "./agent-shelf.js?v=home-20260814a";
import {
...
} from "./agent-progress.js?v=home-20260814a";
import { createStreamQos } from "./agent-stream-qos.js?v=home-20260814a";
...
const TAIL_INVARIANT_CHARS = 1600;
...
function paintThinkingBody(body, text, { streaming = false } = {}) {
  ...
  body.dataset.mdSource = raw;
  /* Thinking stays plain text always — formatting a long reasoning dump at
     end-of-turn was freezing the tab after the answer had already arrived. */
  body.textContent = raw;
  delete body.dataset.mdPainted;
}
...
export function appendMessage(
  role,
  text,
  { streaming = false, asHtml = false, msgIndex = null, artifacts = null, parts = null, modelText = "" } = {},
) {
  const stream = host.streamEl();
  if (!stream) {
    return null;
  ...
  appendArtifactCards(body, artifacts);
...
/* ---- Artifact cards -------------------------------------------------------
   A message can carry `artifacts: [{ id, kind, title, subtitle, target, query }]`.
   Renders a tappable chevron card (Claude-style) that opens the side-panel
   viewer (shell-viewer-rail) hosting the artifact's capsule. The panel launch
   goes through the authority-carrying launchHomeTarget path — UI != authority. */

const ARTIFACT_GLYPH = {
  document:
    '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 1.75h6l3 3v9.5h-9z"/><path d="M9.5 1.75v3h3"/></svg>',
  video:
    '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="currentColor"><path d="M5 3.5v9l7-4.5z"/></svg>',
  web: '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.3"><circle cx="8" cy="8" r="6.25"/><path d="M1.75 8h12.5M8 1.75c-3.5 3.5-3.5 9 0 12.5M8 1.75c3.5 3.5 3.5 9 0 12.5"/></svg>',
  data: '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.3"><rect x="2" y="2.5" width="12" height="11" rx="1.5"/><path d="M2 6h12M6.5 6v7.5"/></svg>',
  code: '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M5.75 4.5 2.25 8l3.5 3.5M10.25 4.5 13.75 8l-3.5 3.5"/></svg>',
  game: '<svg viewBox="0 0 16 16" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><rect x="1.75" y="4.5" width="12.5" height="8" rx="2.5"/><path d="M5 7.25v2.5M3.75 8.5h2.5M10.5 7.5h.01M12 9h.01"/></svg>',
};

const CHEVRON_SVG =
  '<svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3.5 10.5 8 6 12.5"/></svg>';
...
function artifactCardEl(artifact) {
  ...
  const subtitle = String(artifact?.subtitle || artifact?.detail || "");

  /* Card = wrapper holding a main "open" zone plus hover actions (Save / Delete).
     A single <button> can't nest interactive children, so the wrapper is a div
     and the open zone is its own button. */
  ...
  main.innerHTML =
    `<span class="agent-artifact-thumb" aria-hidden="true">${ARTIFACT_GLYPH[kind] || ARTIFACT_GLYPH.document}</span>` +
    `<span class="agent-artifact-copy">` +
    `<span class="agent-artifact-title"></span>` +
    (subtitle ? `<span class="agent-artifact-sub"></span>` : "") +
    `</span>` +
    `<span class="agent-artifact-chevron" aria-hidden="true">${CHEVRON_SVG}</span>`;
  main.querySelector(".agent-artifact-title").textContent = title;
  if (subtitle) {
    main.querySelector(".agent-artifact-sub").textContent = subtitle;
  }
  main.addEventListener("click", () => {
    openArtifact(artifact, title, kind);
  });
  ...
  if (artifact.mediaUrl) {
    ...
    save.setAttribute("aria-label", "Save clip");
    save.innerHTML =
...
async function saveArtifact(artifact) {
  try {
    ...
    a.href = url;
    a.download = `clip-${String(artifact.id || "video").slice(0, 8)}.mp4`;
  ...
  if (session) {
    for (const m of session.messages) {
      if (Array.isArray(m.artifacts)) {
        const idx = m.artifacts.findIndex((a) => a?.id === artifact.id);
        if (idx !== -1) {
          m.artifacts.splice(idx, 1);
        }
      }
    }
    renderActiveSession();
...
/* Single place that opens an artifact: media → direct rail view; otherwise →
   the owning capsule. Called by the card's own listener. */
function openArtifact(artifact, title, kind) {
  if (artifact.mediaUrl) {
    showViewerRail({
      mediaUrl: artifact.mediaUrl,
      mediaType: artifact.mediaType || (kind === "image" ? "image" : "video"),
      title,
      kind,
    });
    return;
  ...
  /* Text/code artifact with inline bytes → Documents in the rail, content
     delivered via the chat-attachment message (no Library object needed). */
  const isTextArtifact =
    (kind === "document" || kind === "code") &&
    typeof artifact.text === "string" &&
    artifact.text.length > 0;
  if (isTextArtifact) {
    const fileName = String(artifact.fileName || title || "artifact.txt");
    const mimeType = String(
      artifact.mimeType || (kind === "code" ? "text/plain" : "text/markdown"),
    );
    showViewerRail({
      target: "documents",
      title,
      ...
      deliver: {
        ...
        attachmentId: String(artifact.id || `artifact-${Date.now()}`),
        fileName,
        mimeType: mimeType,
        dataUrl: `data:${mimeType};base64,${utf8ToBase64(artifact.text)}`,
      },
    });
    return;
  }
  ...
  showViewerRail({
    target: artifact.target || "documents",
    title,
    kind,
...
/* Open a code/text block from a reply as a first-class artifact in the rail
   (Documents, via the chat-attachment path). This is the real producer for
   non-media artifacts: content the agent already gave the user, opened in the
   panel — no fake/demo object, no Library write. */
  ...
  openArtifact(
    {
      ...
      subtitle: `Code · ${language} · ${lines} line${lines === 1 ? "" : "s"}`,
...
/* Render artifact cards for a message into its bubble body. */
function appendArtifactCards(body, artifacts) {
  if (!Array.isArray(artifacts) || !artifacts.length) {
    return;
  }
  const wrap = document.createElement("div");
  wrap.className = "agent-artifacts";
  for (const artifact of artifacts) {
    wrap.append(artifactCardEl(artifact));
  }
  body.append(wrap);
}

/* Append a fresh agent message carrying an artifact card. This is the seam a
   producer (Studio video completion, a future tool) calls when it has made
   something viewable. Persists into the workspace so the card survives refresh. */
export function appendArtifactMessage(text, artifact) {
  const session = ctx.sessions.find((s) => s.id === ctx.activeSessionId);
  if (!session) {
    return false;
  }
  const last = session.messages[session.messages.length - 1];
  /* De-dupe: don't stack an identical card if the producer fires twice. */
  if (
    last?.role === "agent" &&
    Array.isArray(last.artifacts) &&
    last.artifacts.some((a) => a?.id && a.id === artifact?.id)
  ) {
    return true;
  }
  session.messages.push({
...
/* Attach an artifact card to a message (the seam Studio / a future tool uses
   when a run produces something viewable). Persists into the workspace so the
   card survives refresh, then re-renders. */
export function attachArtifactToMessage(msgIndex, artifact) {
  const session = ctx.sessions.find((s) => s.id === ctx.activeSessionId);
  const index = Number(msgIndex);
  if (!session || !Number.isInteger(index) || !session.messages[index]) {
    return false;
  }
  const msg = session.messages[index];
  msg.artifacts = [...(Array.isArray(msg.artifacts) ? msg.artifacts : []), artifact];
  session.updatedAt = Date.now();
  renderActiveSession();
  try {
    host.persistAgentWorkspaceSoon?.();
