 ...
 * above history. Every entry that flows through the Runtime summary is remembered
 * in a small session-memory ring buffer, so a toast you missed is never gone
 * during this GUI session. No extra gateway writes, and clearing history here
...
import { clockNode } from "./shell-core.js?v=home-20260814a";
...
import { showWalletRail } from "./shell-wallet-rail.js?v=home-20260814a";
  ...
  for (const item of history) {
    ...
    card.addEventListener("click", () => {
      ...
      if (item.kind === "wallet_approval_request") {
        showWalletRail();
      } else if (item.kind === HOME_SETUP_KIND) {
