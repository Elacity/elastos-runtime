/* Agent Settings / Usage / Studio pages + workbench panels.
   Bound from agent-harness.js (ctx + host). Tip: home-20260814a
...
import { renderStudioPage } from "./agent-studio.js?v=home-20260814a";
import { fetchJson } from "./shell-core.js?v=home-20260814a";
...
function fillOffersPanel(offers) {
  ...
  if (!rows.length) {
    ...
    li.textContent = "No model offers on this Home yet";
    list.append(li);
  }
  for (const offer of rows) {
    ...
    const modalities = (offer?.operations || offer?.descriptor?.operations || [])
      ...
      .join(" → ");
    li.textContent = modalities ? `${modelId} · ${modalities}` : modelId;
    list.append(li);
  }
  const status = document.querySelector("[data-offers-status]");
  if (status) {
    status.textContent = "Operator-configured · UI ≠ authority";
  }
}
...
function renderPromptPanel() {
  ...
  fillOffersPanel(getModelOffersCache());
  void fetchModelOffers({ force: true }).then((offers) => {
    if (configureSection === "prompt") {
      fillOffersPanel(offers);
    }
  });
}
...
function renderRuntimePanel() {
  ...
  const live = getLiveInferenceState();
  /* Compute/backend readiness — derived from the live probe + offers cache, the
     same truth the chat header uses. Ready = provider live + text offer. */
  const offers = getModelOffersCache();
  const offerCount = Array.isArray(offers?.offers) ? offers.offers.length : 0;
  const readiness = live.checking
    ? "Preparing"
    : live.live
      ? "Ready"
      : "Offline";
  const readinessReason = live.checking
    ? "probing…"
    : live.live
      ? `${offerCount} offer${offerCount === 1 ? "" : "s"}`
      : (live.reason === "no-model-offers" ? "no offers" : live.reason || "offline");
  ...
  /* Readiness row: offers + state, re-rendered when the probe resolves. */
  const offersRow = document.querySelector("[data-runtime-offers]");
  if (offersRow) {
    offersRow.textContent = `Compute: ${readiness} · ${readinessReason}`;
  }
}

/* Refresh the runtime panel once a probe resolves, so "Preparing…" flips to
   Ready/Offline without the user re-opening the panel. */
function refreshRuntimeReadiness() {
  probeLiveInference().then(() => {
    if (document.querySelector("[data-configure-panel='runtime']")) {
      renderRuntimePanel();
    }
  });
}

/* ---- Permissions panel: review + revoke granted capabilities -------------
   Lists from GET /api/apps/home/permissions (gateway proxy → runtime shell-only
   /api/capability/list). Revoke goes through DELETE .../permissions/:id, which
   proxies the runtime's own revoke. UI ≠ authority. */

let permissionsCache = null;
let permissionsLoading = false;

async function renderPermissionsPanel() {
  const host = document.querySelector("[data-permissions-list]");
  if (!host) {
    return;
  }
  if (!permissionsCache && !permissionsLoading) {
    permissionsLoading = true;
    host.replaceChildren();
    const loading = document.createElement("p");
    loading.className = "agent-permissions-empty";
    loading.textContent = "Loading grants…";
    host.append(loading);
    try {
      const data = await fetchJson("/api/apps/home/permissions");
      permissionsCache = Array.isArray(data?.capabilities) ? data.capabilities : [];
    } catch {
      permissionsCache = null;
      host.replaceChildren();
      const err = document.createElement("p");
      err.className = "agent-permissions-empty";
      err.textContent = "Runtime unavailable — grants can't be listed right now.";
      host.append(err);
  ...
  host.replaceChildren();
  if (!permissionsCache || !permissionsCache.length) {
    const empty = document.createElement("p");
    empty.className = "agent-permissions-empty";
    empty.textContent = "No granted capabilities — nothing is ambient.";
    host.append(empty);
    return;
  }
  for (const cap of permissionsCache) {
    const row = document.createElement("div");
    row.className = "agent-permission-row";
    const resource = String(cap?.resource || "resource");
    const action = String(cap?.action || "use");
    const duration = String(cap?.duration || "session");
    row.innerHTML =
      `<span class="agent-permission-copy">` +
      `<span class="agent-permission-resource"></span>` +
      `<span class="agent-permission-meta"></span>` +
      `</span>` +
      `<button type="button" class="agent-permission-revoke">Revoke</button>`;
    row.querySelector(".agent-permission-resource").textContent = resource;
    row.querySelector(".agent-permission-meta").textContent = `${action} · ${duration}`;
    const revokeBtn = row.querySelector(".agent-permission-revoke");
    revokeBtn.addEventListener("click", async () => {
      revokeBtn.disabled = true;
      revokeBtn.textContent = "Revoking…";
      try {
        await fetchJson(`/api/apps/home/permissions/${encodeURIComponent(cap.request_id)}`, {
          method: "DELETE",
        });
        permissionsCache = permissionsCache.filter((c) => c.request_id !== cap.request_id);
      } catch {
        /* leave the row; re-enable so the user can retry */
...
export function renderHarnessPage() {
  ...
  if (harnessPage === "configure") {
    ...
    } else if (configureSection === "permissions") {
      renderPermissionsPanel();
    }
