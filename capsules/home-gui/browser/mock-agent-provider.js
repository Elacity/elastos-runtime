...
export function probeHardwareEstimate() {
  ...
  hardwareProbePromise = (async () => {
    ...
    /* Upgrade to on-device truth: the gateway probes the real host hardware
       (GET /api/apps/home/machine). Browser estimate stays as the fallback if
       the probe is unreachable. */
    try {
      const probe = await fetchJson("/api/apps/home/machine");
      if (probe && probe.source === "runtime-probe" && probe.status === "ok") {
        const realCores = Number(probe.cores) || cores;
        const realMemGb = Number(probe.memory_gb) || poolGb;
        const chip = typeof probe.chip === "string" && probe.chip ? probe.chip : gpuLabel;
        const realParts = [];
        if (chip) {
          realParts.push(chip);
        }
        if (realCores) {
          realParts.push(`${realCores} cores`);
        }
        if (realMemGb) {
          realParts.push(`${realMemGb} GB`);
        }
        hardwareEstimate = {
          cores: realCores,
          poolGb: realMemGb,
          memLabel: realMemGb ? `${realMemGb} GB` : memLabel,
          gpuLabel: chip || gpuLabel,
          deviceLabel: `This device · ${realParts.join(" · ")}`,
          source: "runtime-probe",
        };
      }
    } catch {
      /* probe unreachable — keep the honest browser estimate. */
    }
...
export function getMachineProfile() {
  ...
  return {
    ...
    platform:
      hw?.source === "runtime-probe"
        ? hw?.gpuLabel || "This device"
        : hw?.gpuLabel?.includes("Apple")
          ? "Mac"
          : "This device",
    cores: hw?.cores ?? null,
...
export function getConfigureOverviewSnapshot() {
  ...
  return [
    ...
    },
    {
      ...
      title: "Permissions",
      status: "Review",
      statusTitle: "This agent's session grants — not machine-wide access",
      detail: "Agent grants · review & revoke",
      section: "permissions",
    },
  ];
}
