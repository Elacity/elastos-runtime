...
import {
  ...
  syncIdentity,
  updateClock,
} from "./shell-chrome.js?v=home-20260814a";
import {
  beginDesktopMarquee,
  bindShellSurfaceDom,
...
} from "./shell-keyboard.js?v=home-20260814a";
import {
  bindSpotlight,
  hideSpotlight,
  showSpotlight,
} from "./shell-spotlight.js?v=home-20260814a";
import {
...
import {
  ...
  setMenuManifest,
  syncMenubar,
} from "./shell-menubar.js?v=home-20260814a";
import {
  bindQuickLook,
  hideQuickLook,
  toggleQuickLook,
...
import { setUiSoundsEnabled } from "./shell-sounds.js?v=home-20260814a";
import {
  bindControlCentre,
  hideControlCentre,
  syncControlCentre,
} from "./shell-control-centre.js?v=home-20260814a";
import {
  bindWalletRail,
  retireWalletRail,
  showWalletRail,
...
} from "./shell-wallet-rail.js?v=home-20260814a";
import {
  bindInboxRail,
  retireInboxRail,
  toggleInboxRail,
...
} from "./shell-inbox-rail.js?v=home-20260814a";
import {
  bindViewerRail,
  viewerRailOpen,
  viewerRailFrame,
...
export function deliverMessageToHomeGuiTargetFrame(target, payload, options = null) {
  ...
  if (viewerRailOpen() && viewerRailTarget() === target) {
    const railFrame = viewerRailFrame();
    if (railFrame?.contentWindow) {
      railFrame.contentWindow.postMessage(payload, OPAQUE_FRAME_TARGET);
      return true;
    }
...
function broadcastHomeGuiUiPreferences(preferences) {
  ...
  try {
    viewerRailFrame()?.contentWindow?.postMessage(message, OPAQUE_FRAME_TARGET);
  } catch (_error) {
    // Rail not mounted.
  }
}
