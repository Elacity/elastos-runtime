...
const STALE_TARGET_TITLES = Object.freeze({
  "archive-manager": new Set(["Archive Manager"]),
});
...
export function canonicalTargetTitle(targetId, title) {
  ...
  const staleTitles = STALE_TARGET_TITLES[targetId];
  if (!normalizedTitle || staleTitles?.has(normalizedTitle)) {
    return override;
  }
...
function normalizeDesktopLabels(labels, summary) {
  ...
  for (const [targetId, label] of Object.entries(labels)) {
    ...
    if (STALE_TARGET_TITLES[targetId]?.has(nextLabel)) {
      continue;
    }
