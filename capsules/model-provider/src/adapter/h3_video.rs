...
const RUN_CEILING: Duration = Duration::from_secs(3000);
...
pub fn run_video(run: Arc<Mutex<Run>>, upstream_url: String, output_dir: PathBuf, params: VideoParams) {
    ...
    // Stream to a `.partial` temp, then rename to `<artifact_id>.mp4` once the
    // sha256 (and thus the artifact id) is known — the dogfood durability
    // pattern (`.partial` → rename) and id-addressable filename in one.
    ...
    let response = match ureq::post(&upstream_url)
        ...
        .timeout(RUN_CEILING + Duration::from_secs(30))
        .send_bytes(&body)
    {
    ...
    // addressed, and matches the dogfood `is_creative_job_id` (32-hex) shape so
    // the existing library + video endpoints resolve it with zero changes.
