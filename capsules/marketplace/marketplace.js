(function () {
  ...
  function capsuleToApp(capsule) {
    ...
    return {
      ...
      size: capsule.cid ? "CID-backed" : "Local",
  ...
  function catalogErrorMessage(response, payload) {
    if (response.status === 404) {
      return "capsule catalog route missing; deploy the matching Runtime gateway for this Marketplace capsule";
    }
  ...
  function permissionsFor(capsule) {
    ...
    if (capsule.cid) permissions.push(`Content ID ${short(capsule.cid)}`);
      ...
      <article class="app-card" data-action="detail" data-app="${escapeAttr(app.id)}" tabindex="0">
        ...
        <div class="app-card-footer">
          <div class="app-footer-left">
            ...
            <span class="app-price free">${app.cid ? "Signed package" : "Local app"}</span>
          </div>
          ${actionButton(app)}
        </div>
      </article>
  ...
  function showAppDetail(appId) {
    ...
    els.detailContent.innerHTML = `
      ...
      <div class="modal-body">
        ...
        <section class="modal-section">
          <div class="modal-section-title">Runtime Fit</div>
          <div class="requirements-grid">
            <div class="requirement-item">
              <div class="requirement-value">${escapeHtml(app.requirements.storage)}</div>
              <div class="requirement-label">Package</div>
            </div>
            <div class="requirement-item">
      ...
      <footer class="modal-footer">
        <div class="modal-footer-price"><span class="price-tag">${app.cid ? "CID" : "Local"}</span></div>
  ...
  function distributionItems(app) {
    return [
      ...
      `CID: ${app.cid ? short(app.cid) : app.cidState || "local-only"}`,
      app.repository ? `Repository: ${app.repository}` : "",
      app.sourcePath ? `Source path: ${app.sourcePath}` : "",
      app.installPath ? `Install path: ${app.installPath}` : "",
      app.releasePath ? `Release path: ${app.releasePath}` : "",
    ].filter(Boolean);
  }
  ...
  function showInstallPending(appId) {
    ...
    els.installContent.innerHTML = `
      ...
      <div class="install-modal-status">Marketplace can browse and open installed apps. Remote install must verify signed CID manifests, receipts, and provider policy before an app can run.</div>
