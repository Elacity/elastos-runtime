...
check_forbidden_in_path 'wasi-preview1|wasi-receipt|wasi-app' elastos/esp/esp_v0.ts 'ESP type package must not expose unsupported WASI product ABI variants'
...
check_required 'standalone capsule-inspector is optional until System Inspector and shared' docs/SHELL_ESP_BOUNDARY_MAP.md 'standalone capsule-inspector must remain optional until System Inspector and shared projections are coherent'
check_required 'must not vendor `spend_audit\.js`' docs/SHELL_ESP_BOUNDARY_MAP.md 'standalone capsule-inspector must not duplicate spend-audit projection logic'
...
check_forbidden_in_path 'spend_audit|HomeCustodyView|spendBudgetView|auditChainView|intentProofView' elastos/esp 'headless ESP must keep current custody/audit projection canonical instead of duplicating spend-audit logic'
...
check_required 'Home is installed\. Open it from a terminal:' scripts/install.sh 'installer must teach how to open Home when no terminal is present'
check_required 'home --browser' scripts/install.sh 'installer must teach the Home launch command'
...
check_required 'BROWSER_ENGINE_PROTOCOL_VERSION: &str = "2\.1"' elastos/crates/elastos-common/src/browser_protocol.rs 'shared Browser contract must pin the current breaking lifecycle protocol revision (2.1)'
check_required '"protocol_version": BROWSER_ENGINE_PROTOCOL_VERSION' capsules/browser-engine-adapter/src/main.rs 'browser-engine adapter must expose the shared lifecycle protocol revision on the wire'
check_forbidden_in_path 'BROWSER_ENGINE_PROTOCOL_VERSION: &str|"protocol_version": "' capsules/browser-engine-adapter/src 'browser-engine adapter must take the lifecycle protocol revision from the shared contract, not a private literal'
check_required 'engine_protocol_version != BROWSER_ENGINE_PROTOCOL_VERSION' elastos/crates/elastos-server/src/api/gateway_browser.rs 'Runtime must reject Engine pages that do not carry the current lifecycle protocol revision'
...
# Public installer contract: Linux x86_64/aarch64 and Apple silicon macOS,
# mapped to the signed release platforms; everything else fails closed.
check_required 'Release lookup: Linux x86_64/aarch64 and macOS Apple silicon\.' scripts/install.sh 'installer help must state the two-platform release lookup: Linux x86_64/aarch64 and macOS Apple silicon'
check_required 'linux\)  PLATFORM="\$\{ARCH\}-linux"' scripts/install.sh 'installer must map Linux x86_64/aarch64 to the signed release platforms'
check_required 'PLATFORM="aarch64-darwin"' scripts/install.sh 'installer must map Apple silicon macOS to the aarch64-darwin release platform'
check_required 'macOS release lookup requires Apple silicon' scripts/install.sh 'installer must refuse macOS hosts that are not Apple silicon'
check_required 'die "Unsupported OS: \$\{OS\}"' scripts/install.sh 'installer must fail cleanly on hosts outside Linux and macOS'
check_required 'die "Unsupported architecture: \$\{ARCH\}"' scripts/install.sh 'installer must fail cleanly on architectures outside x86_64/aarch64'
check_required 'arm64\)   ARCH="aarch64"' scripts/install.sh 'installer must treat arm64 as aarch64 for Apple silicon and Linux'
check_forbidden_in_path 'x86_64-darwin|msys|mingw|cygwin|windows\)' scripts/install.sh 'installer must not admit platforms outside the two-platform contract (Intel macOS, Windows shells)'
...
check_required '## Install from the publisher' README.md 'README binary-install commands must match the accepted installer'
check_required 'Linux x86_64/aarch64 and macOS Apple silicon' README.md 'README must name the accepted public-install platforms'
check_required 'Linux x86_64/aarch64 and macOS Apple silicon' docs/INSTALL.md 'install docs must name the accepted public-install platforms'
check_required 'detects the platform \(Linux `x86_64`/`aarch64` or' docs/INSTALL.md 'install docs must state the two-platform installer contract: Linux x86_64/aarch64'
check_required 'macOS Apple silicon\), verifies the signed release' docs/INSTALL.md 'install docs must state the two-platform installer contract: macOS Apple silicon'
check_required 'Linux x86_64/aarch64 and macOS Apple silicon' docs/GETTING_STARTED.md 'getting started must name the accepted public-install platforms'
