...
const startup = source.slice(source.lastIndexOf("const initialUrl ="));
function deferred() {
  let resolve, reject;
  const promise = new Promise((ok, fail) => { resolve = ok; reject = fail; });
  return { promise, resolve, reject };
}
function start({ expired = false } = {}) {
...
test("a pending initial summary cannot advertise an address field that default launch will overwrite", async () => {
  const state = start();
