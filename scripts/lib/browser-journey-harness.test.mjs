...
test("launcher reuses the selected Browser through exact focus without toggling its Shelf item", async () => {
  ...
  const gui = {
    ...
    locator: selector => {
      ...
      throw new Error("Existing Browser must not toggle generic Shelf");
    },
  };
...
test("launcher visibility failure retains exact Frame B for caller cleanup while A and restored C remain", async () => {
  ...
  const detachWait = surface.gui.waitForFunction;
  surface.gui.waitForFunction = async (callback, captured, options) =>
    captured === null ? undefined : detachWait(callback, captured, options);
