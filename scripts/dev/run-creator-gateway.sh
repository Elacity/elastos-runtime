...
#                                            [--remote] [--carrier]
#   --remote   seal + recover against the LIVE 3 geo nodes instead of the local zero-config
#              quorum (same as ELASTOS_DKMS_REMOTE=1). Bare --remote uses the DEPRECATED
#              WireGuard mesh transport — pair with --carrier for the production path.
#   --carrier  reach the live nodes by did:key over Carrier/iroh (implies --remote; same as
#              ELASTOS_DKMS_CARRIER=1). No VPN, no dkms0 mesh. Preferred over bare --remote.
# Env passthrough honored: ELASTOS_DDRM_RIGHTS (dev|chain-mock|chain), ELASTOS_DDRM_CHAIN_ID, etc.
# Flags override the matching env when set; env still works when the flag is absent.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --remote) REMOTE_ARG=1; shift ;;
    --carrier) CARRIER_ARG=1; shift ;;
...
# closed with 403 if none is linked). ELASTOS_DDRM_SUBJECT overrides it for local testing on a
# DEV BUILD ONLY: the honouring branch is behind the `dev-modes` feature and a release binary
# handed the variable refuses to boot, because a pinned subject aims every principal's rights
# check at one operator-chosen wallet.
