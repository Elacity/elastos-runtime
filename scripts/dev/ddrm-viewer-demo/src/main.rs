...
fn handle_conn(mut stream: TcpStream, session: &PreparedSession) -> Result<(), String> {
    ...
    if path == "/" {
        // Must match the real launch shape, because this demo embeds the SHIPPED player.js: the
        // token rides the URL FRAGMENT (`#home_token=`), never the query. player.js reads it from
        // `location.hash` and presents it as the `x-elastos-home-token` header, so the query form
        // would hand it an empty token and every media fetch would 403.
        // Server side: `elastos_server::api::viewer_route_with_launch_token`.
