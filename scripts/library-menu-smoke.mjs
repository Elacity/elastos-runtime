...
const webspaceGoogle = object(`${webspacesUri}/Google`, "Google", "directory", [
  "open",
  "list",
  "properties",
], {
...
const webspaceGoogleDrive = object(`${webspacesUri}/Google/Drive`, "Drive", "directory", [
  "open",
  "list",
  "properties",
], {
  ...
  metadata: {
    schema: "elastos.library.webspace-object/v1",
    mount: "Google",
    target_uri: "google://drive/Drive",
    resolver: "google-drive",
    webspace_kind: "indexed-directory",
    readonly: true,
  },
});
const webspaceGoogleProject = object(`${webspacesUri}/Google/Drive/Project X`, "Project X", "directory", [
  "open",
  "list",
  "properties",
], {
  ...
  metadata: {
    schema: "elastos.library.webspace-object/v1",
    mount: "Google",
    target_uri: "google://drive/Drive/Project X",
    resolver: "google-drive",
    webspace_kind: "indexed-directory",
    readonly: true,
  },
});
const webspaceGoogleFile = object(`${webspacesUri}/Google/Drive/Project X/file.pdf`, "file.pdf", "file", [
  "open",
  "read",
  "download",
  "properties",
...
const folders = new Map([
  ...
  [webspacesUri, [localhostSpace, webspaceElastos, webspaceGoogle, webspaceMutable]],
  [`${webspacesUri}/Elastos`, [webspaceContent]],
  [`${webspacesUri}/Elastos/content`, [webspaceFile]],
  [`${webspacesUri}/Google`, [webspaceGoogleDrive]],
  [`${webspacesUri}/Google/Drive`, [webspaceGoogleProject]],
  [`${webspacesUri}/Google/Drive/Project X`, [webspaceGoogleFile]],
  [webspaceMutableUri, []],
]);
...
async function run() {
  ...
  try {
    ...
    assert(await page.locator(".item[data-selected='true']").count() === 3, "Shift-click must select a visible PC2-style range in list view");
    await listFileItem.click();
    ...
    assert(
      ...
      "Enter on a selected file must open/read it like PC2",
    );
    await page.locator("[data-dialog-close]").click();
    await listFileItem.click();
    ...
    assert(await page.locator(".item[data-selected='true']").count() === 2, "Two files must stay selected before PC2-style multi-Enter open");
    ...
    assert(
      ...
      "Enter on multiple selected files must open every selected item like PC2",
    );
    await page.locator("[data-dialog-close]").click();
    ...
    includesAll(await openItemMenu(page, "Google"), ["Open", "Open in New Window", "Properties"], "mounted WebSpace menu");
    excludesAll(await openItemMenu(page, "Google"), ["Download", "Compress to ZIP", "Publish", "Delete", "Rename", "Paste Into Folder"], "mounted WebSpace menu");

    await page.locator(".item").filter({ hasText: "Google" }).first().dblclick();
    await page.locator(".crumb-current").filter({ hasText: "Google" }).first().waitFor();
    ...
    assert(
      ops.some((entry) => entry.op === "download_raw" && entry.payload.uri.endsWith("/WebSpaces/Google/Drive/Project X/file.pdf") && entry.payload.transport === "raw-body"),
