...
assert(
  ...
  "Library Desktop sidebar icon must match the compact PC2 sidebar icon sizing",
);
assert(
...
assert(
  archiveManagerManifest.includes('"name": "archive-manager"') &&
    ...
    archiveManager.includes("ElastOS Archive Manager") &&
    ...
    objectProviderImpl.includes('"archive-manager" => "Archive Manager"') &&
  ...
  "Archive Manager must provide an installed viewer shell, stat-only/archive-entry/preview/root viewer routes, supported-family browsing, preview, selective extraction, policy-gated unsupported archive UX, conflict/cancel receipts, policy documentation, and no direct viewer provider access or unsafe extraction",
);
assert(
...
assert(
  ...
  "Home desktop objects must open and expose PC2-style object actions through Library/Documents orchestration",
);
assert(
...
assert(
  ...
  "Library context menus must follow PC2 Explorer labels without platform-branded shortcuts",
);
assert(
  ...
  "Library Properties must render the PC2-style tabbed property table instead of a wide diagnostic metadata grid",
);
assert(
...
assert(
  ...
  "Library must keep actions in the Explorer right-click menu, not a disruptive selection strip",
);
assert(
...
assert(
  ...
  "Library must take over browser Back/Forward for Explorer navigation instead of letting native history leave the capsule",
);
assert(
...
assert(
  ...
  "Library list rows and headers must share one Explorer column grid",
);
assert(
...
assert(
  libraryMenuSmoke.includes("PASS Library menu smoke") &&
    ...
    libraryMenuSmoke.includes("/WebSpaces/Google/Drive/Project X/file.pdf") &&
    ...
    libraryMenuSmoke.includes("Shift-click must select a visible PC2-style range in list view") &&
    libraryMenuSmoke.includes("Enter on a selected file must open/read it like PC2") &&
    libraryMenuSmoke.includes("Shift-F10 selected file menu") &&
    libraryMenuSmoke.includes("Enter on multiple selected files must open every selected item like PC2") &&
...
assert(
  ...
  "Library rename must be explicit through context menu/F2; content clicks must never start rename, and PC2-style range selection/keyboard open/context menu must stay wired",
);
assert(
