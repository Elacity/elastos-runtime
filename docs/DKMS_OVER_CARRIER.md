...
| `access grant rejected: replayed` on open attempts 2–3 | the open retry loop re-sent the single-use grant nonce from attempt 1 | **RESOLVED (A7)** — retries now regenerate a fresh-nonce grant from the cached delegation (no MetaMask popup, same delegation/watermark anchor), fail-soft to the original. Pin: `access_grant::attempt_tests::retry_uses_freshly_regenerated_grant` |
