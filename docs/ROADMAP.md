...
**ESP build wedges W0-W7** toward the shell — **W2 ✅ · W0 ✅ · W1 model ✅ · W3 de-hardcoded ✅ · W4 ✅ · W5a ✅** (the two-channel projection client; never-seen moment proven in tests). W6 ✅ · W7 ✅ — the flywheel turned once (the signed receipt re-projects as the EU AI Act containment artifact). **W3b ✅** — the consent decision-engine is now named at its seam (`Session::is_consent_broker` + `consent_broker_only_middleware`) WITHOUT a risky wide "shell" rename (the role `Shell` stays; one documented bridge). **BUG-6 ✅** — carrier-bridge line reads are now bounded DURING the read (untrusted-guest OOM closed across all 3 bridges, test-first). **BUG-8 ✅** — vsock CID allocation is checked + collision-free (no wrap-onto-a-live-VM). **BUG-5 ✅** — the carrier consent-poll no longer drops a grant that lands after the final poll (a trailing read closes the window). **BUG-4 ✅ (mechanism)** — single-use grant is refunded on a routing failure AND on the new `ProviderError::DidNotAct` (a provably-pre-effect rejection); every acted failure stays consumed (ocap-safe). Per-provider migrations to emit `DidNotAct` are a tracked follow-up. **Code-quality audit** (Kent Beck seat, adversarially verified) applied to the campaign diff — one finding's false "no bug" premise caught and corrected. **G3b ✅ (pin) + DRAINING** — universal preview==enforce conformance test across all 22 provider manifests (ratcheted ledger; drift can no longer hide); the swarm classified the 49 drifts and **36 are now resolved** (11 verb-map + 12 manifest splits + 8 egress/encrypt + 5 browser-actuator), 13 dangerous-tail drifts remain tracked (browser-actuator/drm/keys/signing/seal/broadcast — KEEP Admin). The full W0-W7 in-cloud core is LANDED; remaining tails: W5b visual Svelte shell (browser lane), W1b kernel egress firewall (KVM lane), plus the bug tail (BUG-4 op-failure residual; BUG-2/3/7 VM-lifecycle leaks which need a crosvm/KVM test env).
...
| **W1** | Egress-as-capability | **IN PROGRESS** — model landed (`EgressReach::Allowlisted` + fail-closed `EgressAllowlist`); G3b drain made the egress ops (net/exit) honestly declare `execute` — the firewall's keying input. **W1b design DONE** (`docs/W1B_EGRESS_FIREWALL.md`): per-TAP nftables chain from `EgressReach`, threaded at `supervisor.rs:1115`, fail-closed, `EgressDenied` audited, DNS-proxy plan, 5-test plan. **W1b implementation** (the nftables installer) needs `/dev/kvm` + `CAP_NET_ADMIN` → KVM lane. |
...
| **W5** | The v1 Svelte projection shell + the hero dDRM act | **W5a ✅** — headless ESP-v0 two-channel projection client (`elastos/esp/two_channel.ts`, 11 node:test, tsc strict); the never-seen "verified can be more dangerous than unsigned" proven from real fact shapes. **W5b spec ✅** (`docs/ESP_SHELL_UI.md`) — every Svelte component contract (CapsuleCard, TwoChannelObject, HazardMeter, ConsentSheet, ReceiptBadge, ShellPicker, RefractionToggle, AiActAuditCard) mapped to a proven `esp/*.ts` projection + its schema tag, with the no-authority-in-the-view invariant + ratchet pins. **W5b implementation** (the live Svelte paint + visual snapshot test) = the browser/local lane. |
...
| **W7** | Export the receipt chain as the EU AI Act audit artifact (the flywheel) | **✅ DONE + SELF-VERIFYING** — `ai_act_audit.ts`: the signed receipt re-projects to `AiActAuditRecordV1` (Art 12 record-keeping + Art 14 human oversight); fail-closed containment check (unsigned -> Art12 fail; high-risk-no-human -> Art14 fail). NOW embeds the live `ChainAttestation {verified, records, signer, error}` (mirror of the Rust audit `chain_attestation`): a PRESENT-but-broken custody chain fails Art 12 (tamper-evidence compromised), absent falls back to the signed check. 29 node:test, tsc strict. Residual: the runtime→bridge plumbing that populates the attestation on a real export is the last wire. |
...
- **PDR NEXT-band (beyond W0-W7 v1):** the dual-receipt PLATFORM self-attestation (co-sign at gate
  ...
  **Spend-meter WIRED `W3b-turn`** — `primitives::spend::SpendMeter` (atomic fail-closed `try_debit` +
  ...
  SpendDebit/BudgetExhausted), enabled via `ELASTOS_DEFAULT_SPEND_BUDGET`. Now covers ALL THREE carrier act
