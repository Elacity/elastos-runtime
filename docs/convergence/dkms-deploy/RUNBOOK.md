...
4. **Provision + read this node's public identity** (idempotent; creates the master on first run):
   ```bash
   docs/convergence/dkms-deploy/bin/dkms-preflight.sh identity \
...
```bash
docs/convergence/dkms-deploy/bin/dkms-validate-descriptor.py \
...
```bash
install -m 0644 docs/convergence/dkms-deploy/systemd/dkms-authority.service \
...
docs/convergence/dkms-deploy/bin/dkms-preflight.sh node \
...
2. Runtime-host preflight (validates descriptor + init config + probes every node endpoint):
   ```bash
   docs/convergence/dkms-deploy/bin/dkms-preflight.sh runtime \
