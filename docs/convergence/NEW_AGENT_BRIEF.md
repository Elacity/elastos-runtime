...
1. **Runtime-core plan execution — a production key authority + a default-on, non-smoke entrypoint.**
   ...
   **Day 77–78 host-owned teardown + a persisting CEK-free event sink** (`ProviderTransport::shutdown`
   ...
   read-back across a fresh process). The host now brings the rail up by launching each provider (spawn →
   init → publish material) and persists durably. The remaining step is a PRODUCTION key authority backend
   (the dev `reference` authority still mints its keys at init, so the runtime binds the cross-provider
   escrow/transcript material AFTER launch; a `dkms`/`lit` backend with stable published recipients would
   let the escrow precede launch) + folding the launchers + durable store into a default-on runtime-core
   entrypoint a NON-smoke caller drives (the smoke is still the producer + runtime bootstrap). `DrmHost` +
   `PlanSource` + `ProviderLauncher` + `EventStore` is exactly the seam the core wires its real
   transports/store into (the smoke proves it; no second code path).
...
scripts/ddrm-consumer-smoke.sh   # host LAUNCHES the rail (ProviderLauncher x3, from_launchers) -> DrmHost::open: PlanSource(drm) -> registry(rights->key->decrypt) -> PersistingEventSink over DurableEventStore (atomic CEK-free receipt+audit, read back by a fresh DurableEventStore::load); host.shutdown tears down the rail
...
`ddrm-plan-runner`=43 (the runtime-core plan executor + `RuntimeStepRunner` + `open_drm_plan` composition root + `RuntimeCapabilityTable` runtime-owned registry + `DrmHost` trusted host entrypoint that LAUNCHES the rail via `ProviderLauncher`/`from_launchers`, owns the rail's teardown, and persists CEK-free open records through the production-shaped `DurableEventStore`).
