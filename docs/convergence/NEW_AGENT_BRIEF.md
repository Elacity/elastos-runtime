...
1. **Runtime-core plan execution — a production key authority + a default-on, non-smoke entrypoint.**
   ...
   **Day 77–78 host-owned teardown + a persisting CEK-free event sink** (`ProviderTransport::shutdown`
   ...
   pattern now carries to a NON-reference authority. The remaining step is a true REMOTE production authority
   (today `dkms` is a PROVISIONED-DESCRIPTOR seam holding the key material; a real remote dKMS would resolve
   PUBLIC-only keys + DELEGATE recovery to the external node) + a backend selector in `OpenConfig` so the live
   smoke drives `dkms` end-to-end (today it's unit-tested; the live smoke runs the `reference` store).
...
4. **Production key authority** — a true REMOTE `dkms` (native) or `lit` (compat) `key-provider`
   ...
   external-descriptor seam (resolves a stable signer + recipient from a handed-in
   `dkms_authority_descriptor`, re-seals via the same contract); a real remote dKMS would resolve
   PUBLIC-only keys + DELEGATE recovery to the external node. `lit` is still `not_configured`.
...
`publish-provider`=16, `key-provider [key-authority-ref]`=37,
