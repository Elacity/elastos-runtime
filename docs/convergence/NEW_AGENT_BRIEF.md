...
| 117–120 | **A wiring** | **the QUORUM has a LIFECYCLE: a LIVE share-wise ROTATION of the 2-of-3 set to THREE successor nodes via per-node COORDINATE-BOUND refresh deltas `q(x_i)` with `q(0)=0` over GF(256) (`ddrm-envelope` / `key-provider` / `ddrm-runtime-open`)** — Day 113–116 made the threshold a real QUORUM; Day 109–112 gave the 2-of-2 set a rotation lifecycle. This unit CLOSES the gap: it generalizes that lifecycle to t-of-n, so a quorum member can be decommissioned to a successor WITHOUT the CEK reassembling and WITHOUT weakening the quorum. **The KEY INSIGHT (and why it's cheap):** the dKMS node's `rotate_share` op is UNCHANGED across schemes — it blind-XORs its escrowed payload with the operator-sealed delta (`share' = share ⊕ delta`). The 2-of-2 XOR rail hands EVERY node the SAME mask (CEK invariant trivially); a Shamir quorum CAN'T (one mask shifts every share onto the same offset and corrupts the polynomial), so the operator adds a degree-1 REFRESH polynomial `q(x)=refresh_coeff·x` with `q(0)=0`, handing each node a DIFFERENT `q(x_i)`. Because `q(0)=0` the reconstructed secret `p'(0)=CEK` is INVARIANT while every share moves to a NEW polynomial `p'(x)=p(x)⊕q(x)` (so an OLD captured share is dead next to a refreshed one). The escrowed payload is the INDEXED share `x ‖ p(x)`, so each delta is `0x00 ‖ q(x_i)` — the leading zero PRESERVES the coordinate the decrypt boundary pins, the body refreshes. The node never learns it did anything different; only the operator's delta-derivation generalized. Audited PC2 first: PC2 has NO key-authority rotation at all (a pinned-constant redeploy, `chipotle-client.ts:125`, no content migration, no refresh) and a fortiori no quorum-aware proactive refresh; the runtime OWNS proactive secret-sharing refresh end to end — the textbook defense against a mobile adversary compromising one share at a time. (1) PRIMITIVE (ddrm-envelope 33→34): `shamir_refresh_delta(refresh_coeff, x)` → `0x00 ‖ (refresh_coeff·x over GF(256))` (fail-closed on x=0 / empty coeff) + a golden test proving invariant / old-dead / coordinate-bound / fail-closed deterministically. (2) CLIENT (key-provider 47, test extended): optional `producer_vk3_b64` (the quorum generalization of `producer_vk2_b64`) — after a rotation share-3's escrow is signed by node C, so it authenticates under node C's identity (absent → byte-identical non-rotated rail). (3) RUNTIME + GATES 42–45 (`dkms_quorum_rotation_gates`, THREE real successor daemons, `--nodes 3`): (42) all three rotate their indexed shares to fresh successors with coordinate-bound deltas → a fresh key-provider on the ROTATED 3-node descriptor opens the quorum → the boundary Shamir-reconstructs the EXACT original CEK; (43) the rotated rail SURVIVES a dead successor (C' down → A'+B' → exact CEK — availability outlives rotation, which the 2-of-2 rail structurally cannot); (44) the refresh kills OLD material (old node-set pin refuses the rotated descriptor; a successor refuses a predecessor's escrow; the old⊕rotated → garbage math is pinned by the envelope test); (45) the delta is COORDINATE-BOUND (rotating a node with another coordinate's delta — the 2-of-2 single-mask mistake — silently corrupts the quorum). The dKMS node + operator-only authorization edges + live revocation are byte-identical to the node op gates 32–35 already prove, so they are NOT duplicated. Gate: ladder INTACT (ddrm-envelope=34, key-provider[key-authority-ref]=47), drift PASS, ALL smokes green (reference + dkms + 2-of-2 + tcp + the 2-of-3 quorum smoke now driving all 45 gates + chain + producer + publish + market), clippy clean. |
| 113–116 | **A wiring** | **REAL t-of-n: the CEK is SHAMIR-split 2-of-3 over GF(256) across THREE secret-holding dKMS nodes — ANY TWO live nodes serve an open, the production rail SURVIVES a dead node, and below quorum it still fails closed (`ddrm-envelope` / `key-provider` / `decrypt-provider` / `ddrm-runtime-open`)** — every threshold cycle so far was 2-of-2: real splitting, zero availability (one dead node bricked every open — gates 23–24 PROVED it). This 4-day unit makes the threshold a real QUORUM. Audited PC2 first: PC2's only t-of-n is the LEGACY Lit `decryptAndCombine` (`non-media-decrypt.js:76`) — threshold BLS across Lit's own nodes, combined inside Lit's TEE; t, n, membership, and failover are INVISIBLE and unconfigurable (availability RENTED, not owned); the CURRENT Chipotle path abandoned t-of-n entirely for a SINGLE master PKP in ONE TEE (`chipotle-client.ts:1290`). **The runtime now OWNS what PC2 rents: explicit t and n, our own field arithmetic, an explicit quorum policy, and gated failover.** (1) PRIMITIVES (ddrm-envelope 29→33): GF(256) arithmetic (`gf256_mul`, AES polynomial 0x11B, constant control-flow; `gf256_inv` = a^254) + `split_cek_shamir2(cek, coeff)` (degree-1 polynomial per byte, shares at x=1,2,3 — any single share information-theoretically useless) + `combine_cek_shamir2(x_a, share_a, x_b, share_b)` (Lagrange at x=0, distinct non-zero coordinates enforced, `Zeroizing`) + `indexed_share`/`parse_indexed_share` (`x ‖ share` — the coordinate rides INSIDE the sealed envelope, authenticated by the escrow seal + every node re-seal, never forgeable cleartext) + `threshold_node_set_id_n(t, vks)` (n-node generalization, BYTE-IDENTICAL to the 2-node id — nothing re-pins on upgrade); golden vector pinned. (2) CLIENT (key-provider 45→47): the descriptor resolves a THREE-node `threshold` block (`t:2`, all identities pairwise DISTINCT, else fail closed); `release_quorum` tries node A→B→C, succeeds with re-sealed indexed shares from ANY TWO live nodes (a dead node is a tolerated fault) and REFUSES below quorum — no single-share material ever emitted; the third escrow is REQUIRED in the session context (missing = config error, never a silent degrade). (3) DECRYPT BOUNDARY (rail-material 70→72): a THIRD pinned vk (`authority_vk3_b64`, requires vk2) flips quorum mode; `decrypt_from_carrier_quorum` verifies EACH sealed share against the pinned identities and REQUIRES the x inside the authenticated payload to MATCH the verifying node's index — a MIS-INDEXED share fails even with a valid signature, x_a == x_b (one secret-holder twice) is refused, Lagrange reconstruction happens ONLY in-boundary. (4) RUNTIME + GATES 36–41 (live daemons through the production `DrmHost`): `authority.nodes` (2|3; 3 requires `threshold`; else fail closed), three-daemon provisioning, Shamir split at publish; (36) node C KILLED → the open SURVIVES (A+B serve, durable records persisted); (37) A+C dead → BELOW quorum → fail closed, no record; (38) A restored → a DIFFERENT pair serves; (39) B killed → A+C serve (the x=1/x=3 Lagrange pair's first production use); (40) a MIS-INDEXED share (genuine node B re-sealing a payload claiming node A's x) fails at the boundary; (41) a DUPLICATED share (one node's view in both slots) fails closed. NEW `ddrm-consumer-dkms-quorum-smoke.sh` (`--threshold --nodes 3`); the 2-of-2 XOR rail + its rotation/revocation gates are untouched and still green. Also fixed the STALE drm-provider wasm-smoke case (since Day 67 a valid open emits the `planned` DrmOpenPlanV1, not `not_configured` — the case now asserts the plan shape + no key/session material). Gate: ladder INTACT (ddrm-envelope=33, key-provider[key-authority-ref]=47, decrypt-provider[rail-material]=72), drift PASS, ALL smokes green (reference + dkms + 2-of-2 + tcp + QUORUM + chain + producer + publish + market), clippy clean. |
| 109–112 | **A wiring** | **the secret-holders have a LIFECYCLE: live share-wise ROTATION to successor nodes with a proactive REFRESH (the CEK never reassembled during rotation), OPERATOR-ONLY authorization, and LIVE CALLER REVOCATION that outranks a live session (`ddrm-envelope` / `dkms-authority` / `key-provider` / `ddrm-runtime-open`)** — every prior cycle treated the two secret-holding nodes as PERMANENT; this 4-day unit gives the node-set a real operational lifecycle, fail-closed at every edge. Audited PC2 first: PC2 has NO key-authority rotation concept — its "rotation" is a constant redeploy ("rotating the key means updating this constant + redeploying both supernodes", `chipotle-client.ts:125`) or a supernode payload CID swap (`:1043`–`:1064`); nothing migrates content, nothing refreshes standing key material. PC2's revocation is an in-memory `revokedDelegations` Map capped at 10 000 (`utils/secureViewSession.ts:374`–`:399`) checked by HTTP middleware (`secureViewSession.ts:104`–`:112`) — the key-holding network (Lit) never learns. **The runtime is SUPERIOR on both: operator-SIGNED instructions land on the key-holding NODES themselves, and a rotation REFRESHES the shares so captured-then material is useless now.** (1) PRIMITIVES (ddrm-envelope 27→29): `DKMS_ROTATE_DOMAIN` + `rotation_aad(kid16, source_recipient_pub, successor_recipient_pub)` — the operator seals the refresh delta TO the rotating node, AEAD-bound to the exact rotation context (no cross-kid / cross-node / redirected-successor reuse); `DKMS_REVOKE_DOMAIN` + `sign_revocation`/`verify_revocation` (operator-only; sibling-domain signatures refused). (2) NODE (dkms-authority 15→18): a pinned OPERATOR identity (`DKMS_AUTHORITY_OPERATOR_VK`, daemon-start-resolved, never client-settable; absent → lifecycle ops `not_configured`); NEW `rotate_share` — operator-seal verified FIRST, current share unwrapped via the SAME authenticated path `recover` uses, `share' = share ⊕ delta` in `Zeroizing`, re-escrowed to the SUCCESSOR signed by the rotating node's OWN identity; both nodes rotate with the SAME delta so the CEK is invariant while an OLD captured share next to a NEW share is delta-masked garbage — the whole CEK NEVER exists during rotation; NEW `revoke_caller` (operator-signed, idempotent, DAEMON-LIFETIME revoked set threaded through every connection) — a revoked caller's `hello` is `caller_revoked` AND a `recover` under a still-LIVE session token is refused BEFORE signature work; on TCP both lifecycle ops join `recover` behind the encrypted-channel gate. (3) CLIENT (key-provider 44→45): optional `producer_vk2_b64` in the release session context — after a rotation each share's escrow is signed by the node that rotated it, so share-2 authenticates under the RIGHT producer identity. (4) RUNTIME + GATES 32–35 (live daemons, REAL successors, both transports): per-run operator ML-DSA keypair pinned into every daemon; (32) FULL rotated rail — provision successors A′/B′, rotate BOTH nodes with ONE operator-sealed delta, fresh key-provider opens the ROTATED 2-of-2 → the EXACT original CEK; (33) the refresh kills old material — old-share⊕rotated-share ≠ CEK, a successor refuses its predecessor's escrow, the old node-set pin refuses the rotated descriptor; (34) rotation is OPERATOR-ONLY live — impostor delta / tampered delta / redirected successor / no-operator node / plaintext-rotate-on-tcp ALL refused; (35) LIVE revocation — a forged revocation is refused (caller stays served), the genuine revocation cuts the SAME live session mid-stream and survives reconnect. Drift untouched. Gate: ladder INTACT (ddrm-envelope=29, dkms-authority=18, key-provider[key-authority-ref]=45), drift PASS, all dDRM smokes green (both threshold smokes drive all 35 gates), clippy clean. |
| 105–108 | **A wiring** | **the dKMS node is OFF LOCALHOST: a REAL TCP transport with an app-layer ENCRYPTED, MUTUALLY-AUTHENTICATED channel built from our OWN primitives; the FULL 2-of-2 threshold rail passes over TCP and every hostile-network edge fails closed (`ddrm-envelope` / `dkms-authority` / `key-provider` / `ddrm-runtime-open`)** — every prior cycle reached the node over a host-local Unix socket (the filesystem was the boundary); this 4-day unit puts the secret-holder on a real network where nothing is trusted. Audited PC2 first: its dDRM network boundary is `https.get(url, { rejectUnauthorized: false, timeout: 5000 })` (`chipotle-client.ts:838`–`:851`) — TLS verification DISABLED, the channel authenticates NOTHING; only the supernode provisioning payload carries an app-side signed envelope (`:737`–`:795`); its Boson proxy does NaCl crypto_box app-layer encryption (`ProxyProtocol.ts:9`/`:21`/`:24`/`:251`) but never carries dDRM and authenticates the network peer, never the key-holding NODE. **The runtime is SUPERIOR: the channel itself authenticates the node, frame by frame, with PQ-hybrid primitives we own.** (1) PRIMITIVES (ddrm-envelope 25→27): `attest_channel_key`/`verify_channel_key` — at `hello` the node signs its master-derived channel KEM key under its descriptor-pinned ML-DSA identity (a MITM terminating TCP can relay the hello but CANNOT substitute its own KEM key); `channel_frame_aad(channel_id, direction, seq)` — every sealed frame bound to its channel/direction/strictly-advancing seq (non-replayable, non-reflectable). (2) NODE (dkms-authority 13→15): `DKMS_AUTHORITY_LISTEN=tcp:HOST:PORT` (real `TcpListener`, same framed protocol, transport-generic `serve_connection_io`, 30 s server read timeout); on TCP a plaintext `recover` → `channel_required`, and ANY violation on an established channel (plaintext downgrade / tampered envelope / stale seq) DROPS the connection with no response (no oracle); `NodeAuthority` gained the domain-separated channel keypair + moved to the heap (boxed — dev-profile PQ stack pressure tipped 2 MiB test threads). (3) CLIENT (key-provider 43→44): `tcp:` endpoints connect under EXPLICIT timeouts (connect 5 s + read 5 s — a stalled node fails the release closed, no partial material, never a hang); the channel is REQUIRED on the network path — pure `resolve_node_channel_key` fails closed on a missing block / substituted key / replayed challenge / impostor identity; `DkmsNodeConn` transport-generic, seals/unseals every frame. (4) RUNTIME (+1 config test 10→11): `authority.transport` (`"unix"` default | `"tcp"`, dkms-only, unknown fails closed), OS-picked loopback ports as `tcp:127.0.0.1:PORT` descriptor endpoints, the adversarial probe establishes the channel so gates 13–17 run SEALED. (5) GATES 28–31 (live daemon, real TCP): plaintext recover refused; plaintext downgrade after establishment dropped (daemon not wedged); ONE flipped ciphertext byte dropped; an attacker-substituted channel KEM key fails under the pinned identity. (6) NEW `ddrm-consumer-dkms-tcp-smoke.sh` drives the ENTIRE 2-of-2 rail over TCP — all 31 steps incl. node-fault gates 23–24 (over TCP = mid-rail network drop: fail closed, no partial CEK, no record). Drift untouched. Gate: ladder INTACT (ddrm-envelope=27, dkms-authority=15, key-provider[key-authority-ref]=44), drift PASS, all dDRM smokes green (reference + dkms + 2-of-2 + tcp 2-of-2), clippy clean. |
| 103–104 | **A wiring** | **the threshold's identity is CRYPTOGRAPHIC + AUDITABLE: the node-set is welded into the decrypt-transcript AAD (a swapped node-set fails the AEAD open AT THE BOUNDARY), every durable open record is STAMPED with the serving node-set, rotation is fail-closed (`ddrm-envelope` / `decrypt-provider` / `ddrm-runtime-open`)** — Day 101–102 pinned the node-set at descriptor parse; this cycle pushes the binding into the cryptography + the audit trail. Audited PC2 first: its decrypt binding is `SHA-256(cek‖kid‖authority)` over a SINGLE authority (`universal-decrypt-chipotle.js:577`–`:589`); it has NO key-authority rotation concept (`chipotle-client.ts:125`/`:1043`/`:1064` — manual constant redeploys only); its audit trail can never say WHICH nodes served a decrypt (opaque inside Lit). **The runtime is SUPERIOR on all three.** (1) `DecryptTranscriptV1` gained optional `node_set_id`, appended to `to_aad()` ONLY when present — single-node encoding BYTE-IDENTICAL, threshold AAD a strict extension (24→25). The runtime seals the open under the descriptor-derived node-set (shared `derive_node_set_from_descriptor` — ONE path for the pin check, the AAD, and the rotation gate); the nodes treat the AAD as opaque bytes (no protocol change). (2) the boundary derives the SAME id from its OWN pinned vks (`threshold_node_set_id(2, authority_vk, authority_vk2)`) into `prepare_bound_open` — a swapped node-set fails the AEAD open in the sandbox even with valid per-share signatures; a threshold-provisioned boundary also REFUSES single-share material outright (rail-material 68→70). (3) a `NodeSetStampingSink` stamps `node_set_id_b64` into every durable open record (CEK-free, public hash; read back fresh + asserted equal to the producer pin; single-node records carry no stamp). (4) gate 27 (rotation): a REAL freshly-provisioned node B′ + rotated descriptor is REFUSED by the old fixture's pin via the same derivation `run()` enforces — a stale publish can never open against a rotated node-set. (5) gate 26 (live, cross-binary): a release whose AAD names a FORGED node-set SUCCEEDS at the key capsule (both nodes seal honestly — they can't catch it) and is REFUSED by the live decrypt capsule. Drift untouched. Gate: ladder INTACT (ddrm-envelope=25, key-provider[key-authority-ref]=43, decrypt-provider rail-material=70), drift PASS, all dDRM smokes green, clippy clean. |
| 101–102 | **A wiring** | **the live 2-of-2 threshold is now RESILIENT + IDENTITY-BOUND: the production rail FAILS CLOSED under a real node fault, NEVER degrades to a single node, and a silently SWAPPED node-set is DETECTED before recovery (`ddrm-envelope` / `ddrm-runtime-open` / smoke)** — Day 99–100 wired the threshold into the real open; this cycle proves it survives faults + pins WHO backs it. Audited PC2 first: PC2's run-path resilience is ONE opaque Lit RPC + a retry (`recoverCEKEnvelope`; `RequestExpired` → "retry by re-running the Lit action", `chipotle-client.ts:575`) — a downed/swapped node lives INSIDE Lit's network, so PC2 has NO per-node fault semantics + NO inspectable node-set identity. **The runtime is SUPERIOR** (it owns the two nodes, so it kills one to prove fail-closed + pins exactly which two back the rail). (1) NODE-SET IDENTITY: new pure `ddrm_envelope::threshold_node_set_id(t, vk_a, vk_b)` (domain-separated, length-prefixed SHA-256 over both vks + `t`; order-sensitive; 23→24) — `publish_escrow` PINS it into the durable fixture (`node_set_id_b64`); `host.open()` RE-DERIVES it from the published descriptor's `threshold` block + FAILS CLOSED on mismatch (a node re-pointed at a different secret-holder caught BEFORE recovery). (2) LIVE NODE-FAULT gates 23–24 (verify, threshold-only): with the full 2-of-2 rail up, KILL node B's daemon → `host.open()` fails closed (no partial CEK, no single-node fallback, no record persisted), restore; KILL node A's daemon → same; node A is RESTARTED so the downstream socket probes still run (the daemon guards are now `mut`). (3) SWAP DETECTION gate 25: a descriptor whose node B is swapped to a rogue identity re-derives to a DIFFERENT node-set-id than the pin — detected end-to-end (the boundary independently rejects the rogue's seal under node B's pinned vk, Day 97–98 step 20, so the swap fails at BOTH layers). Drift untouched. Gate: ladder INTACT (ddrm-envelope=24, key-provider[key-authority-ref]=43, decrypt-provider rail-material=68), drift PASS, all dDRM smokes green (reference + dkms + 2-of-2 with the new gates), clippy clean. |
...
We work in **day units**. Each unit is one thin, demoable, reversible increment.
The unit size has grown as trust grew: Days 45–66 were single days, Days 67–104 were
**2-day units**, and from Day 105 the user upgraded the cadence to **4-day units**
("you are managing 2 days well and i want you to run for longer") — one 10/10 prompt
now covers 4 days of scope, executed in one autonomous run. The rhythm:
...
scripts/ddrm-consumer-smoke.sh   # [--backend reference|dkms] [--threshold] [--transport unix|tcp] writes a typed OpenConfig JSON + INVOKES the default-on runtime-core entrypoint scripts/dev/ddrm-runtime-open (mode=verify); the BIN owns it: PUBLISH phase escrows the CEK to the STABLE recipient (+ for dkms PROVISIONS the node(s) + an immutable external descriptor) -> durable fixture; OPEN phase DrmHost::launch (ProviderLauncher x3, key authority RELAUNCHED from the same store/descriptor -> SAME recipient) -> PlanSource(drm) -> registry(rights->key->decrypt) -> PersistingEventSink over DurableEventStore (atomic CEK-free receipt+audit, read back fresh) + the adversarial fail-closed gates; host.shutdown tears down the rail. NO smoke assembles the host.
scripts/ddrm-consumer-dkms-smoke.sh            # sibling: the SAME open with authority.backend=dkms (single secret-holding node over the framed Unix socket; descriptor-immutability proof)
scripts/ddrm-consumer-dkms-threshold-smoke.sh  # sibling: the 2-of-2 threshold open (TWO nodes, XOR-split CEK, dual-recover, in-boundary combine, node-fault + forged-node-set + rotation gates)
scripts/ddrm-consumer-dkms-tcp-smoke.sh        # sibling: the SAME 2-of-2 threshold open over REAL TCP with the encrypted, mutually-authenticated channel (gates 28-31: plaintext recover refused, downgrade dropped, MITM-tampered frame dropped, substituted channel key fails)
...
`publish-provider`=16, `key-provider [key-authority-ref]`=47,
`dkms-authority`=18 (the EXTERNAL secret-holding authority NODE — owns the master, exposes only `hello` + `recover` + the operator-gated lifecycle ops `rotate_share`/`revoke_caller` (Day 109–112); BINDS+LISTENS on a FRAMED Unix-domain socket OR a real TCP listener (`DKMS_AUTHORITY_LISTEN=tcp:HOST:PORT`, Day 105–108), PINS-verified handshake, a node-signed SESSION TOKEN minted at `hello` + bound to the caller's ephemeral pubkey, REQUIRED + a POSSESSION PROOF verified on every `recover`, per-recover re-authorization, a KNOWN-caller ALLOW-LIST enforced at `hello`, a per-recover FRESHNESS counter (`recover_seq`), a pinned OPERATOR identity (`DKMS_AUTHORITY_OPERATOR_VK`) authorizing share-wise rotation (operator-sealed XOR refresh delta, re-escrow to a successor, CEK never reassembled) + live caller revocation (daemon-lifetime revoked set, outranks a live session), and on TCP a REQUIRED app-layer ENCRYPTED CHANNEL — attested channel KEM key at `hello`, every frame a sealed PQ-hybrid envelope AAD-bound to `(channel_id, direction, seq)`, plaintext recover refused, downgrade/tamper/replay drops the connection; lifecycle ops channel-gated too),
`decrypt-provider [rail-material]`=72, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=34
...
`sign_session_token`/`verify_session_token` (over `challenge‖caller_pub‖expires_at`) + the recover
POSSESSION PROOF `sign_recover_proof`/`verify_recover_proof` (binds `recover_seq`) + the length-prefixed socket FRAME module
`frame::write_frame`/`read_frame` (`[4-byte BE len][payload]`, `MAX_FRAME_BYTES=1 MiB`, fail-closed on torn/oversized) +
the 2-of-2 split `split_cek_xor`/`combine_cek_xor` + the node-set identity `threshold_node_set_id` /
`DecryptTranscriptV1.node_set_id` AAD extension + the Day 105–108 CHANNEL primitives
`attest_channel_key`/`verify_channel_key` + `channel_frame_aad(channel_id, direction, seq)` + the Day 109–112
LIFECYCLE primitives `rotation_aad(kid16, source_recipient_pub, successor_recipient_pub)` +
`sign_revocation`/`verify_revocation` + the Day 113–116 QUORUM primitives — GF(256) Shamir
`split_cek_shamir2`/`combine_cek_shamir2` (t=2, shares at x=1,2,3, Lagrange at x=0),
`indexed_share`/`parse_indexed_share` (`x ‖ share` sealed inside the envelope), and the n-node
node-set identity `threshold_node_set_id_n` + the Day 117–120 QUORUM-LIFECYCLE primitive
`shamir_refresh_delta(refresh_coeff, x)` → `0x00 ‖ q(x)` (the coordinate-bound proactive-refresh
delta `q(x)=refresh_coeff·x` with `q(0)=0` — the t-of-n generalization of the 2-of-2 XOR rotation delta)),
