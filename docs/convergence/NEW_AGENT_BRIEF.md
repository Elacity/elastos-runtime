...
1. **Runtime-core plan execution — host spawns its own rail + a production persisting store.**
   ...
   and **Day 77–78 host-owned teardown + a persisting CEK-free event sink** (`ProviderTransport::shutdown`
   /`RuntimeCapabilityTable::shutdown`/`DrmHost::shutdown`; `EventStore` + `PersistingEventSink` +
   `open_event_record`). Today the host's transports wrap capsules the smoke provisioned (their keys must
   be published before the escrow) and its persisting store writes to a temp dir. The remaining step is
   the `DrmHost` itself SPAWNING/connecting to the rail it owns (instead of the smoke provisioning the
   capsules) + a PRODUCTION persisting store (durable receipt/audit beyond a temp dir), so the open runs
   default-on from capabilities + a store the core itself owns end to end. `DrmHost` + `PlanSource` +
   `EventStore` + `RuntimeCapabilityTable` is exactly the seam the core wires its real transports/store
   into (the smoke proves it; no second code path).
...
scripts/ddrm-consumer-smoke.sh   # DrmHost::open: PlanSource(drm) -> registry(rights->key->decrypt) -> PersistingEventSink(durable CEK-free receipt+audit); host.shutdown tears down the rail
...
`ddrm-plan-runner`=38 (the runtime-core plan executor + `RuntimeStepRunner` + `open_drm_plan` composition root + `RuntimeCapabilityTable` runtime-owned registry + `DrmHost` trusted host entrypoint that owns the rail's teardown + persists CEK-free open records).
