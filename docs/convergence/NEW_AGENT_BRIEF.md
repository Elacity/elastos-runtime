...
`publish-provider`=16, `key-provider [key-authority-ref]`=40,
`dkms-authority`=6 (the EXTERNAL secret-holding authority NODE — owns the master, exposes only `hello` + `recover`; PINS-verified handshake + per-recover re-authorization),
`decrypt-provider [rail-material]`=65, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=18
...
attestation `attest_challenge`/`verify_attestation`),
