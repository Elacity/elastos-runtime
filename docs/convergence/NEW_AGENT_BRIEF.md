...
`publish-provider`=16, `key-provider [key-authority-ref]`=44,
`dkms-authority`=15 (the EXTERNAL secret-holding authority NODE — owns the master, exposes only `hello` + `recover`; BINDS+LISTENS on a FRAMED Unix-domain socket OR a real TCP listener (`DKMS_AUTHORITY_LISTEN=tcp:HOST:PORT`, Day 105–108), PINS-verified handshake, a node-signed SESSION TOKEN minted at `hello` + bound to the caller's ephemeral pubkey, REQUIRED + a POSSESSION PROOF verified on every `recover`, per-recover re-authorization, a KNOWN-caller ALLOW-LIST enforced at `hello`, a per-recover FRESHNESS counter (`recover_seq`), and on TCP a REQUIRED app-layer ENCRYPTED CHANNEL — attested channel KEM key at `hello`, every frame a sealed PQ-hybrid envelope AAD-bound to `(channel_id, direction, seq)`, plaintext recover refused, downgrade/tamper/replay drops the connection),
`decrypt-provider [rail-material]`=70, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=27
...
`attest_channel_key`/`verify_channel_key` + `channel_frame_aad(channel_id, direction, seq)`),
