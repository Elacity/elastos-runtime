...
1. **Runtime-core plan execution — a production key authority + a default-on, non-smoke entrypoint.**
   ...
   **Day 77–78 host-owned teardown + a persisting CEK-free event sink** (`ProviderTransport::shutdown`
   ...
   so the escrow PRECEDES launch and the "bind material after launch" shortcut is gone. The remaining step is
   a PRODUCTION EXTERNAL key authority backend (`dkms`/`lit`, still `not_configured` — the durable-store
   stability + escrow-at-publish pattern is proven on the `reference` backend and carries over) + folding the
   launchers + durable store + `DrmHost::launch` into a default-on runtime-core **`bin` entrypoint** a
   NON-smoke caller drives (the producer/escrow already moved to a publish-time fixture — the bootstrap is the
   last smoke-owned bit). `DrmHost::launch` + `PlanSource` + `ProviderLauncher` + `EventStore` is exactly the
   seam the core wires its real transports/store into (the smoke proves it; no second code path).
...
scripts/ddrm-consumer-smoke.sh   # PUBLISH phase: escrow CEK to the authority's STABLE durable-key-store recipient -> durable fixture. OPEN phase: DrmHost::launch (ProviderLauncher x3, key authority RELAUNCHED from the same store -> SAME recipient) -> PlanSource(drm) -> registry(rights->key->decrypt) -> PersistingEventSink over DurableEventStore (atomic CEK-free receipt+audit, read back fresh); proves recipient STABLE across relaunch; host.shutdown tears down the rail
...
`publish-provider`=16, `key-provider [key-authority-ref]`=35,
`decrypt-provider [rail-material]`=65, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=16
(the shared PQ-hybrid seal/unwrap crate — now also DETERMINISTIC from-seed key derivation:
`mint_session_from_seed`/`derive_seed`/`random_seed` for a STABLE authority recipient),
