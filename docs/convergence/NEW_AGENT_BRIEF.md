...
`publish-provider`=16, `key-provider [key-authority-ref]`=41,
`dkms-authority`=9 (the EXTERNAL secret-holding authority NODE — owns the master, exposes only `hello` + `recover`; PINS-verified handshake, a node-signed SESSION TOKEN minted at `hello` + REQUIRED/verified on every `recover`, and per-recover re-authorization),
`decrypt-provider [rail-material]`=65, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=20
...
attestation `attest_challenge`/`verify_attestation` + the domain-separated dKMS-node SESSION TOKEN
`sign_session_token`/`verify_session_token`),
