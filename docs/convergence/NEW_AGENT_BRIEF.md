...
`dkms-authority`=11 (the EXTERNAL secret-holding authority NODE — owns the master, exposes only `hello` + `recover`; now BINDS+LISTENS on a FRAMED Unix-domain socket, PINS-verified handshake, a node-signed SESSION TOKEN minted at `hello` + bound to the caller's ephemeral pubkey, REQUIRED + a POSSESSION PROOF verified on every `recover`, and per-recover re-authorization),
`decrypt-provider [rail-material]`=65, `chain-provider mint*` rung=10, `drm-provider`=15,
`encrypt-provider`=20 (default) / 25 (`escrow`), `key-provider`=18 (default), `ddrm-envelope`=22
