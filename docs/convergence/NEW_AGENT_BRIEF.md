...
1. **Runtime-core plan execution — construct the `RuntimeCapabilityTable` inside the trusted core.**
   ...
   seam** (`RuntimeStepRunner`), **Day 73 the composition root** (`open_drm_plan`), and **Day 74 the
   runtime-owned registry** (`RuntimeCapabilityTable` — `register` a `ProviderTransport` per provider,
   `resolve` opens a fresh handle or `None` → fail-closed). Today the registry is populated by the
   consumer smoke with capsule-backed transports. The remaining step is to construct the
   `RuntimeCapabilityTable` INSIDE the trusted core, registering transports that drive the runtime's
   REAL provider→provider rail (instead of the smoke's binaries), so the open runs default-on inside
   the runtime. `open_drm_plan` + `RuntimeCapabilityTable` + `ProviderTransport` is exactly the seam
   the core wires its real transports into (the smoke proves it; no second code path).
...
scripts/ddrm-consumer-smoke.sh   # drm(plan) -> open_drm_plan(plan, RuntimeCapabilityTable) -> rights -> key -> decrypt
...
`ddrm-plan-runner`=29 (the runtime-core plan executor + `RuntimeStepRunner` + `open_drm_plan` composition root + `RuntimeCapabilityTable` runtime-owned registry).
