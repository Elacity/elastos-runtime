...
- ⬜ **Missing middle:** standing `open_drm_plan` up INSIDE a trusted runtime-core caller wired to
  the runtime's REAL provider→provider rail (the smoke proves the entrypoint via a capsule-backed
  `CapabilityTable`; the core supplies the real table) so the open runs default-on inside the core;
  a **production key authority** backend (`dkms` native, or `lit` compat) so the dev `reference`
  backend drops out; the **producer side** actual IPFS **pin** (the availability receipt is handed
  in today; pinning stays a separate, later capability) and a multi-block (>1 MiB) `payload_cid` for
