...
- ⬜ **Missing middle:** a **REMOTE production EXTERNAL key authority** (`dkms` today is a
  ...
  `lit` compat is still `not_configured`); a **backend selector in `OpenConfig`** so `ddrm-runtime-open` can
  drive the `dkms` authority end-to-end (today the live smoke runs the reference store; `dkms` is unit-tested);
  the **producer side** actual IPFS **pin** (the availability receipt is handed in today; pinning stays a
  separate, later capability) and a multi-block (>1 MiB) `payload_cid` for large assets; a **viewer**.
