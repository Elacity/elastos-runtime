...
- 🟩 **Host launches the rail + a production-shaped durable store (Day 79–80):** `ddrm-plan-runner` gained
  ...
  CEK/secret leak.
- ⬜ **Missing middle:** a **production key authority** backend (`dkms` native, or `lit` compat) so the dev
  `reference` backend — which still mints its keys at init, forcing the runtime to bind the cross-provider
  escrow/transcript material AFTER launch — drops out; folding the launchers + durable store into a
  default-on runtime-core entrypoint a NON-smoke caller drives (the smoke is still the producer + runtime
  bootstrap); the **producer side** actual IPFS **pin** (the availability receipt is handed in today;
