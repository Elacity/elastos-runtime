...
- 🟩 **Runtime-owned capability registry (Day 74):** `RuntimeCapabilityTable` (in `ddrm-plan-runner`)
  ...
  25→29.
- ⬜ **Missing middle:** constructing the `RuntimeCapabilityTable` INSIDE a trusted runtime-core caller
  whose transports drive the runtime's REAL provider→provider rail (the smoke proves the registry with
  capsule-backed transports; the core owns the real ones) so the open runs default-on inside the core;
