...
- ⬜ **Missing middle:** a **key authority** (ElastOS-native PQ-hybrid dKMS, or a
  Lit-compat backend behind `key-provider`) that emits `SealedDecryptMaterialV1` so the
  dev escrow shim drops out; the **runtime core** executing the plan default-on (today the
  dev orchestrator stands in for it); the **producer side** real `plaintext_ref`→bytes
  resolution in the non-inline `seal` (so it content-addresses too) + the actual IPFS pin
  (a separate, later capability); a **viewer**.
