...
- ⬜ **Missing middle:** standing the Day-72 `RuntimeStepRunner` up INSIDE the trusted core wired
  to the runtime's REAL provider→provider rail (the smoke proves the injected-handle seam; the
  core supplies the real handles) so the open runs default-on inside the core; a **production key
  authority** backend (`dkms` native, or `lit` compat) so the dev `reference` backend drops out;
  the **producer side** actual IPFS **pin** (the availability receipt is handed in today; pinning
  stays a separate, later capability) and a multi-block (>1 MiB) `payload_cid` for large assets;
  a **viewer**.
