...
- ⬜ **Missing middle:** giving the `DrmHost` REAL owned transports (the host spawns/connects to the
  runtime's provider→provider rail, owning the transports rather than the smoke pre-spawning them) and a
  PERSISTING `RuntimeEventSink` (durable receipt + audit, the analogue of `mediaSessionManager.create` +
  the audit log) so the host runs from capabilities + a sink the core itself owns end to end; a
  **production key authority** backend (`dkms` native, or `lit` compat) so the dev `reference` backend
  drops out; the **producer side** actual IPFS **pin** (the availability receipt is handed in today;
  pinning stays a separate, later capability) and a multi-block (>1 MiB) `payload_cid` for large assets;
  a **viewer**.
