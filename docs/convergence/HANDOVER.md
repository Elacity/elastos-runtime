...
- ⬜ **Missing middle:** a **key authority** (ElastOS-native PQ-hybrid dKMS, or a
  ...
  dev orchestrator stands in for it); the **producer side** actual IPFS **pin** (the
  availability receipt is handed in today; pinning stays a separate, later capability) and
  a multi-block (>1 MiB) `payload_cid` for large assets; a **viewer**.
