...
- **Phase C (underway)** — the producer half. Days 58–60 landed the crypto half (KID ==
  ...
  **cross-binary producer smoke** `encrypt(seal_inline) → key(release_from_escrow_ref) →
  ...
  content-market` so the listing's `content_id` IS the producer's KID (content-market=13).
  Next: `metadata.json` enrichment via `ipfs-provider` (title/poster/mime), a live-Base
  event-scan path, a live producer→consumer round trip, then real `plaintext_ref`→IPFS.
