...
- ⬜ **Missing middle:** wiring the Day-71 executor to the runtime's REAL provider→provider
  rail (it runs today via the smoke's injected `StepRunner`; the seam is in place) so the open
  runs default-on inside the core; a **production key authority** backend (`dkms` native, or
  `lit` compat) so the dev `reference` backend drops out; the **producer side** actual IPFS
  **pin** (the availability receipt is handed in today; pinning stays a separate, later
  capability) and a multi-block (>1 MiB) `payload_cid` for large assets; a **viewer**.
