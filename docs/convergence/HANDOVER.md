...
- ⬜ **Missing middle:** a **production EXTERNAL key authority** backend (`dkms` native, or `lit` compat —
  still `not_configured`) so the in-runtime `reference` authority is replaced by a real threshold/HSM-backed
  one (the durable-key-store stability + escrow-at-publish pattern is now proven on the reference backend and
  carries over); folding the launchers + durable store + `DrmHost::launch` into a default-on runtime-core
  **`bin` entrypoint** a NON-smoke caller drives (the producer/escrow already moved to a publish-time
  fixture — the bootstrap is the last smoke-owned bit); the **producer side** actual IPFS **pin** (the
  availability receipt is handed in today; pinning stays a separate, later capability) and a multi-block
  (>1 MiB) `payload_cid` for large assets; a **viewer**.
