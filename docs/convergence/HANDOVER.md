...
- **Phase C (underway)** — the producer half. Days 58–60 landed the crypto half (KID ==
  ...
  **cross-binary producer smoke** `encrypt(seal_inline) → key(release_from_escrow_ref) →
  ...
  (content-market=22). Next: a live-Base event-scan path (real `AssetCreated`/
  `DigitalAssetRegistered` logs → reconstruct), a live producer→consumer round trip, then
  real `plaintext_ref`→IPFS.
