...
- **Phase C (underway)** — the producer half. Days 58–60 landed the crypto half (KID ==
  ...
  **cross-binary producer smoke** `encrypt(seal_inline) → key(release_from_escrow_ref) →
  ...
  assembler. Next: a `content-market` index that scans the mint event into a listing, a
  live-Base producer→consumer round trip, then real `plaintext_ref`→IPFS in the producer op.
