...
- ⬜ **Missing middle:** the **runtime core** executing the Day-67 plan default-on — driving
  this canonical `release` itself (today the dev orchestrator smoke stands in); a **production
  key authority** backend (`dkms` native, or `lit` compat) so the dev `reference` backend drops
  out; the **producer side** actual IPFS **pin** (the availability receipt is handed in today;
  pinning stays a separate, later capability) and a multi-block (>1 MiB) `payload_cid` for large
  assets; a **viewer**.
