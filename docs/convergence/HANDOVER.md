...
- ⬜ **Missing middle:** a PRODUCTION persisting store (durable receipt/audit beyond the smoke's temp
  dir) and the `DrmHost` itself SPAWNING/connecting to the rail it owns (the transports still wrap
  capsules the smoke provisioned for their published keys) so the open runs default-on inside the core;
  a **production key authority** backend (`dkms` native, or `lit` compat) so the dev `reference` backend
