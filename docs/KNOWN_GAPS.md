...
| G4 | Human-**approval** loop | The bridge from "preview the gate" → a signed, no-effect intent a human/authority approves before any dispatch. Not yet built. | *Pending feature scaffold* (added with the approval-intent decision core). | Approval-intent core + recorded approve/deny decision exist; ratchet test asserts fail-closed default-deny. |
