...
| `type: microvm` (e.g. `chat`, `agent`, `localhost-provider`, `did-provider`, `shell`, `webspace-provider`, `ipfs-provider`, `tunnel-provider`, ...) | KVM + crosvm on Linux; Apple Vz on macOS *(Phase 6 in progress — see below)* | **🚧 Phase 6 planned, Day 1 pending operator kickoff.** Day-by-day plan in [`vz-backend/PHASE_6_PLAN.md`](vz-backend/PHASE_6_PLAN.md). 8-day sequence: Days 1–4 audit + restore truthful `components.json` darwin-arm64 entries for the 8 microVM capsules (`chat`, `did-provider`, `ipfs-provider`, `localhost-provider`, `shell`, `site-provider`, `tunnel-provider`, `webspace-provider`) + 3 native helpers (`cloudflared`, `kubo`, `llama-server`) + `vmlinux` kernel artifact, with code-signing + notarization of the Mac runtime binary; Day 5 activates the self-hosted Mac CI runner (`MAC_VZ_FULL_BOOT_ENABLED=true`); Day 6 lands the first end-to-end FORCE_FULL smoke green on the self-hosted runner (triage + fix any substrate bugs the dev-Mac runs missed); Day 7 expands `vz_perf_harness.rs` with the real-microVM-boot path against the now-truthful metadata (flips `notes.real_vz_boot_measured` to `true`, schema bump v2 → v3); Day 8 ships `PHASE_6_RETROSPECTIVE.md` + `PHASE_7_ENTRY_CHECKLIST.md` + the tagged release with signed + notarized binary. Estimated effort: 42–58 h (one focused week). **✅ Phase 5 complete (8 days). Phase 5 Day 8 closed the phase with a DRY hoist + perf-schema v2 bump + retrospective.** New `cross_platform_smoke_log_dry_run_reason` helper consolidates the Day-5/Day-6 dry-run-reason echo logic into `scripts/lib/cross-platform.sh`; all three Mac smokes refactored to use the predicate + log helper pair (`cross_platform_smoke_should_dry_run` + `cross_platform_smoke_log_dry_run_reason`), with operator-visible echo lines preserved byte-for-byte in the production CI auto-detect path (verified via diff against the Day-7 baseline). Shell-helper assertion count: 44 → 47 (3 new for `log_dry_run_reason`). New `elastos-server/tests/common/mod.rs` extracts `seed_cached_synthetic_capsule` + `synthetic_components_manifest` from `vz_perf_harness.rs` (Day 7) and `vz_supervisor_startup_orphan_cleanup.rs` (Day 4) — single source of truth for the on-disk cached-capsule shape `ensure_capsule` consults to short-circuit the IPFS download path. Perf-report `schema_version` bumped 1 → 2: new top-level `git_sha` field populated by `measure-{vz,crosvm}-baseline.sh` from `git rev-parse --short=12 HEAD` (with `-dirty` suffix when uncommitted changes are present); harness defaults to `"unknown"` when `ELASTOS_VZ_PERF_GIT_SHA` env var is unset (purely additive — v1 consumers ignoring `git_sha` still parse v2 files). New `docs/vz-backend/PHASE_5_RETROSPECTIVE.md` documents what shipped across all 8 days, what went well, what didn't (Day-5/6/7 cascade rationale, self-hosted runner provisioned but not active), and 8 carry-forward items for Phase 6. New `docs/vz-backend/PHASE_6_ENTRY_CHECKLIST.md` enumerates the Phase-5 closeout gates (9 items, all checkable) + 3 Phase-6 unblockers (`components.json` darwin-arm64 metadata, self-hosted runner activation, first end-to-end full-boot smoke). Test count preserved at 598; Linux side untouched. See [`vz-backend/PHASE_5_DAY_8_NOTES.md`](vz-backend/PHASE_5_DAY_8_NOTES.md), [`vz-backend/PHASE_5_RETROSPECTIVE.md`](vz-backend/PHASE_5_RETROSPECTIVE.md), and [`vz-backend/PHASE_6_ENTRY_CHECKLIST.md`](vz-backend/PHASE_6_ENTRY_CHECKLIST.md). **Phase 5 Day 7 ships the performance measurement substrate + honest baseline document.** New `elastos-server/tests/vz_perf_harness.rs` (11 tests; 6 measurement + 5 schema/utility guards) measures six Phase-4/5 synthetic Rust paths both backends share — `supervisor_new_{cold,warm}`, `synthetic_capsule_launch`, `provider_registry_send_raw_{single,concurrent}`, `capability_manager_validate` — and emits a `schema_version: 1` JSONL stream when `ELASTOS_VZ_PERF_REPORT` is set. Two new aggregation scripts (`scripts/measure-vz-baseline.sh` cross-OS, `scripts/measure-crosvm-baseline.sh` Linux-only with clean-exit on Mac) run the harness 5 times each, compute per-metric medians of the per-run percentiles, and write canonical `target/{vz,crosvm}-baseline.json`. Both scripts honor the Day-5/Day-6 precedence (FORCE_FULL → DRY_RUN → CI auto-detect → default). New `docs/vz-backend/PERFORMANCE_BASELINE.md` documents methodology, code-path-by-code-path explanations of what each metric measures, initial Mac (M-series) numbers (all six paths sub-millisecond at p99 — `capability_manager_validate` p99 = 38 µs; `synthetic_capsule_launch` p99 = 93 µs; `supervisor_new_cold` p99 = 31 µs; `supervisor_new_warm` p99 = 4 µs; `send_raw_concurrent` per-call p99 = 13 µs; `send_raw_single` at the resolution floor), per-metric observations explaining the cold/warm 4× delta + the resolution-floor behaviour, the explicit list of what we cannot measure yet (real Vz boot, real cross-VM RPC over vsock, real bridge teardown — all Phase-6-gated on darwin-arm64 release metadata), and a pre-baked Phase-6 comparison table with `_TBD_` cells for Linux + real-boot rows. JSON wire format frozen at `schema_version: 1`. No production Rust code changes — the harness lives entirely under `tests/`. See [`vz-backend/PHASE_5_DAY_7_NOTES.md`](vz-backend/PHASE_5_DAY_7_NOTES.md) and [`vz-backend/PERFORMANCE_BASELINE.md`](vz-backend/PERFORMANCE_BASELINE.md). **Phase 5 Day 6 wires the self-hosted full-boot CI lane (default off, operator opt-in).** A new fourth job `mac-vz-full-boot` in `.github/workflows/mac-vz.yml` is doubly-gated on the repo variable `MAC_VZ_FULL_BOOT_ENABLED == 'true'` AND a runner registered with the exact label set `[self-hosted, macOS, ARM64, vz-capable]`. When both gates are open, the job builds release artefacts, probes the Mac substrate (`Virtualization.framework` presence), and runs the three Phase-5 smokes with the new top-priority env var `ELASTOS_VZ_SMOKE_FORCE_FULL=1` set. FORCE_FULL sits at the top of a documented 5-layer smoke precedence table (FORCE_FULL → explicit `DRY_RUN=0` → explicit `DRY_RUN=1` → CI auto-detect → default-full); its canonical implementation lives in the new `cross_platform_smoke_should_dry_run` helper and is pinned by 3 new shell-helper assertions (41 → 44). A new `.github/workflows/_self-hosted-probe.yml` heartbeat workflow (cron `0 0,6,12,18 * * *` + `workflow_dispatch`) probes the runner pool every 6 hours plus an always-on fallback job on `ubuntu-latest` records the variable's current value for an audit trail in the Actions UI. New `docs/vz-backend/SELF_HOSTED_RUNNER_SPEC.md` documents hardware (Apple Silicon, ≥ 16 GiB RAM, ≥ 100 GiB disk, macOS 13+), the exact label set, the provisioning checklist (Rust toolchain → data dir → runner agent → repo variable), security posture (hardware isolation, network segmentation, restricted repo variable), the two-stage kill switch (variable for routine maintenance; label removal for emergency), and Phase-6 deferrals (darwin-arm64 release metadata, signing/notarisation, perf baselines, multi-runner fleet, auto-recovery). `CI_RUNBOOK.md` § 3a covers the lane end-to-end with the same precedence table and operator workflow. No Rust code changes in Day 6 — this is pure CI ops surface + shell substrate. See [`vz-backend/PHASE_5_DAY_6_NOTES.md`](vz-backend/PHASE_5_DAY_6_NOTES.md) and [`vz-backend/SELF_HOSTED_RUNNER_SPEC.md`](vz-backend/SELF_HOSTED_RUNNER_SPEC.md). **Phase 5 Day 5 lights up macOS Apple-Silicon CI substrate.** A new `.github/workflows/mac-vz.yml` workflow gates every push + PR with three jobs running on `macos-latest`: `mac-rust-tests` (fmt + clippy + scoped tests under both `--test-threads=1` and `=4`); `mac-shell-helpers` (the bash-3.2 helper-library unit tests, now 41 + 5 = 46 total assertions); `mac-smokes-dry-run` (the three Phase-5 smokes with `ELASTOS_VZ_SMOKE_DRY_RUN=1` set). Per-job `timeout-minutes` caps (30/5/10) + `concurrency: cancel-in-progress: true` + Mac-specific cargo-cache prefix-key keep open-source CI minutes spend bounded. New `cross_platform_in_ci` predicate in `scripts/lib/cross-platform.sh` auto-enables the smokes' dry-run lane when `GITHUB_ACTIONS` or `CI` is set; explicit `ELASTOS_VZ_SMOKE_DRY_RUN=0` always wins so self-hosted runners with a provisioned `~/.local/share/elastos` (a Phase 5 Day 6+ deliverable) opt back into the full smoke. The helper-library unit-test count grew 37 → 41 (+4 dual-recognition assertions). Five new `just ci-*` recipes (`ci-mac`, `ci-rust`, `ci-shell`, `ci-smokes-dry`, `ci-linux-untouched`) mirror the workflow exactly — `just ci-mac` is the local one-stop-shop for reproducing a CI failure. New `docs/vz-backend/CI_RUNBOOK.md` covers failure-reading, manual-trigger via `workflow_dispatch:`, local-reproduction recipes, and explicit gap calling-out (real Vz boot, full Linux end-to-end smokes, perf benchmarks, and real `darwin-arm64` metadata all carry forward to Days 6–8 + Phase 6). `linux-untouched.yml` gained `workflow_dispatch:` so operators can re-run the protected-paths gate after a rebase without pushing. See [`vz-backend/PHASE_5_DAY_5_NOTES.md`](vz-backend/PHASE_5_DAY_5_NOTES.md) and [`vz-backend/CI_RUNBOOK.md`](vz-backend/CI_RUNBOOK.md). **Phase 5 Day 4 wires `prune_stale_mac_artifacts` into the supervisor startup path with one-shot orphan telemetry.** Default `Supervisor::new` on Mac now invokes the Phase-4-Day-5 cleanup helper automatically (logged via `tracing::info!` with per-category counts) and caches the report for one-shot delivery on the first `SupervisorRequest::EnsureCapsule` response. The original opt-in `Supervisor::prune_stale_mac_artifacts` surface remains for explicit operator sweeps. New public types: `OrphanCounts` (operator-facing JSON projection — three integer fields: `overlays_removed`, `sockets_removed`, `bridge_sockets_removed`, the last two split from the original combined `sockets_removed` per the Day-4 contract so operators can pivot on the carrier-bridge orphan rate independently); `SupervisorResponse::orphans_pruned: Option<OrphanCounts>` (skip-serialises when absent so legacy dashboards keep working unchanged); `VzConfig::prune_orphans_on_startup: bool` (default `true`; companion builder `with_prune_orphans_on_startup`; operator opt-out for the dual-supervisor edge case); `Supervisor::new_with_vz_config(data_dir, registry, vz_config)` for tests and future operator harnesses. The Linux launch path remains byte-identical: the Mac branch is gated behind `#[cfg(target_os = "macos")]`, the `prune_stale_mac_artifacts` helper is a no-op stub on Linux, and a dedicated Linux-only test (`supervisor_new_is_noop_on_linux_even_with_prune_flag_set`) enforces this contract. Tests landed: +9 total (1 updated socket-split assertion in `prune_stale_mac_artifacts_removes_overlays_and_sockets_but_preserves_unrelated_files`; 3 new supervisor unit tests covering auto-prune, opt-out, Linux no-op, and `OrphanCounts` projection; 2 new RPC-contract integration tests in `elastos-server/tests/vz_supervisor_startup_orphan_cleanup.rs` covering the full `Supervisor::new` → `handle_request(EnsureCapsule)` round-trip + the opt-out wire-format guard; 1 new JSON wire-format test `supervisor_response_json_wire_format_for_orphans_pruned` covering populated/zero/None cases). All green under both `--test-threads=1` and `=4`. Operator alert recipe (Datadog): `@orphans_pruned.bridge_sockets_removed:[1 TO *]` over a 1h window → page on-call; sustained non-zero carrier-bridge orphan rate is the Phase-4-Day-2 surface re-emerging. See [`vz-backend/PHASE_5_DAY_4_NOTES.md`](vz-backend/PHASE_5_DAY_4_NOTES.md). **Phase 5 Day 3 completes the three-smoke Mac-parity port: `scripts/chat-wasm-native-interop-smoke.sh` is now Mac-aware.** The shared helper library grew with `cross_platform_curl_or_skip` (publisher-gateway probe with structured skip on outage) and `cross_platform_alert_on_vz_error_in_logs` (the Day-3 tripwire on the Phase-4-Day-7 `VzError::Display` contract — greps for the nine stable `kind_label:` tokens). Helper unit-test count grew 26 → **37** (over the ≥30 target). The smoke gains four new lanes: `ELASTOS_VZ_SMOKE_DRY_RUN=1` (parity with Days 1/2), gateway-reachability probe, `ELASTOS_CHAT_INTEROP_OFFLINE=1` flag with required `ELASTOS_BIN_OVERRIDE`, and an install.sh-failure capture that surfaces the published install.sh's bash-3.2 incompatibility (`GATEWAYS[@]: unbound variable`) as a clean Phase-6-prerequisite skip. The smoke's tail runs the VzError alerting tripwire on every collected log AND the Vz substrate probe. A new Mac-only Rust integration test `elastos-server/tests/vz_chat_interop_smoke.rs` locks in the bidirectional `ProviderRegistry::send_raw` contract (round-trip <5 s; unknown-scheme error surface; provider-error propagation) — running in <1 ms on M1. Carry-forward to Phase 6: the published install.sh itself needs a Mac port; Day 3 surfaces this with an actionable operator message. See [`vz-backend/PHASE_5_DAY_3_NOTES.md`](vz-backend/PHASE_5_DAY_3_NOTES.md). **Phase 5 Day 2 extends the Linux-smoke-parity port: `scripts/home-frontdoor-smoke.sh` is now Mac-aware.** Day 1's helper library grew with `kill_pid_then_group` (SIGTERM → bounded-grace SIGKILL escalation, replacing ad-hoc `kill -- "-${pid}"` patterns), `free_port_via_python3` (DRY hoist of inline `socket.bind` port allocators), and `cross_platform_assert_native_binary_release_metadata` (single source of truth for the Mac pre-flight `components.json` check across all current and future install smokes). `scripts/lib/runtime-cleanup.sh` lost its Linux-only `/proc/<pid>` dependency and now uses the helper library; a new `scripts/lib/runtime-cleanup-test.sh` lands 5 assertions covering the fix. The cross-platform unit-test count grew from 15 → 26. The Day-1 smoke was refactored to call the new DRY helpers (no behaviour change; cleaner failure path). A new Mac-only Rust integration test `elastos-server/tests/vz_home_frontdoor_smoke.rs` locks in the supervisor Launch → Status → Stop → VzError RPC contract for the three Home-frontdoor providers (`localhost-provider`, `did-provider`, `webspace-provider`) — visibly-skipping with telemetry until Phase 6 lands the truthful `darwin-arm64` release metadata. Same operator escape hatches as Day 1 (`ELASTOS_VZ_SMOKE_DRY_RUN=1`, `ELASTOS_VZ_SMOKE_FORCE_PROCEED=1`). See [`vz-backend/PHASE_5_DAY_2_NOTES.md`](vz-backend/PHASE_5_DAY_2_NOTES.md). **Phase 5 Day 1 began the port** with `scripts/local-carrier-setup-smoke.sh` Mac-aware via the new `scripts/lib/cross-platform.sh` helper library (bash-3.2-clean `pid_is_running`, `read_pids_into_array`, `vz_host_is_capable`, `vz_discover_launchable_capsule`) and three new operational lanes: `ELASTOS_VZ_SMOKE_DRY_RUN=1` CI fast-lane, Mac pre-flight, Vz substrate readiness probe. See [`vz-backend/PHASE_5_DAY_1_NOTES.md`](vz-backend/PHASE_5_DAY_1_NOTES.md) for the original runbook + Phase-6 gap analysis; [`vz-backend/PHASE_5_PLAN.md`](vz-backend/PHASE_5_PLAN.md) for the full 8-day Phase-5 plan. **Phase 4 Day 8 closes Phase 4 with a typed `VzErrorReport` readback RPC.** A new `SupervisorRequest::CapsuleVzError { handle }` variant (`op = "capsule_vz_error"`) returns the structured `elastos_vz::VzErrorReport` projection of `RunningVm::last_vz_error()` — `kind_label` (stable telemetry filter, e.g. `"vz_internal"`, `"vz_timed_out"`, `"vz_unknown"`) + `description` are always set; `domain` + `code` populate only for `VzError::Unknown` (so future / unmodelled Apple variants stay greppable without a binding update); `vm_id` + `budget_secs` populate only for `VzError::TimedOut` (so dashboards can size the fleet-wide `VzConfig::stop_timeout`). Every optional field skip-serialises so legacy dashboards keep working unchanged and field presence becomes a typed signal. `SupervisorResponse::vz_error: Option<VzErrorReport>` is the wire-format field; `capsule_status` enriches stopped Vz capsules with BOTH `last_exit_reason` (Day 7) AND `vz_error` (Day 8) for single-query observability. The dispatcher's three-state outcome (`Found(None)` / `Found(Some(report))` / `NotFound`) maps cleanly onto `status: "ok"` + skip-serialised field / `status: "ok"` + populated field / `status: "not_found"`. With Day 8 the Mac substrate has the same observability surface as Linux's crosvm path: structured per-capsule error readback, telemetry labels for every terminal state, and stable JSON wire formats for both the alert path and the triage path. See [`vz-backend/PHASE_4_DAY_8_NOTES.md`](vz-backend/PHASE_4_DAY_8_NOTES.md). **Phase 4 Day 7 makes Vz failures structurally typed.** A new `VzError` enum (`elastos_vz::VzError`) classifies Apple's `VZErrorCode` (`Internal` / `InvalidConfiguration` / `InvalidState` / `InvalidStateTransition` / `NetworkError` / `OperationCancelled` / `NotSupported`) plus our synthetic `TimedOut { vm_id, budget }` for the Day 6 stop-timeout case and a forward-compatible `Unknown { domain, code, description }` for codes the binding doesn't yet model. `VzMachineHandle::start` / `stop` return `Result<(), VzError>` directly; the FFI boundary reads `NSError.domain` + `.code` + `.localizedDescription` and routes the typed variant through. The companion `VzExitReason` enum (`GuestCleanStop` / `HostInitiatedStop` / `StoppedWithError` / `ForcedAfterTimeout`) is the canonical exit-code + telemetry-label source of truth and is cached on `RunningVm::last_exit_reason()`. `SupervisorResponse` gains an optional `last_exit_reason: Option<String>` field (`#[serde(skip_serializing_if = ...)]` — backward compatible with legacy dashboards). `Supervisor::stop_capsule` and `capsule_status` populate it with one of `guest_clean_stop` / `host_initiated_stop` / `stopped_with_error` / `forced_after_timeout` so operators piping `elastos status` JSON into Datadog / Grafana can alert on `forced_after_timeout` rate without grepping log lines. See [`vz-backend/PHASE_4_DAY_7_NOTES.md`](vz-backend/PHASE_4_DAY_7_NOTES.md). **Phase 4 Day 6 hardens Vz stop semantics + bridge teardown observability.** `VzConfig::stop_timeout` (default 30 s, configurable) puts a typed-error upper bound on Apple's `stopWithCompletionHandler:` block — wedged framework calls no longer pin the supervisor's `stop_capsule` indefinitely (Mac has no `kill -9` equivalent on a Vz VM, so this is the only enforcement point). On timeout, `wait_for_exit` waiters resolve with the new `DelegateExit::ForcedAfterTimeout` exit code 137 (matches Linux's `128 + SIGKILL(9)` convention so operator tooling sees a consistent forced-stop marker across substrates). `BridgeContext::on_terminate` exposes the Carrier dispatch loop's lifecycle as an `Arc<tokio::sync::Notify>` — the Mac supervisor awaits this notify with a 10 s budget after `vm.stop()` resolves so it can deterministically log "bridge terminated cleanly" vs "bridge orphaned — continuing best-effort." Best-effort cleanup posture: overlay removal, provider-route unregistration, and bridge wait all run regardless of whether the stop succeeded, so a wedged stop never leaves on-disk state that blocks future launches. See [`vz-backend/PHASE_4_DAY_6_NOTES.md`](vz-backend/PHASE_4_DAY_6_NOTES.md). **Phase 4 Day 5 closes shutdown semantics + crash-recovery audit on Vz.** The full teardown graph (`Supervisor::stop_capsule` → `RunningVm::stop` → `VzMachineHandle::stop` → drop chain → bridge task exit) is documented with surface deltas vs Linux/crosvm in [`vz-backend/PHASE_4_DAY_5_NOTES.md`](vz-backend/PHASE_4_DAY_5_NOTES.md). Two integration tests prove the graceful-failure surface: an in-flight cross-VM RPC against a stalled provider VM that subsequently stops returns a typed `ProviderError` (`unhealthy` / `closed` / `timed out`) within 30 s — never silent, never hung, never panicked; closing the host side of a Carrier socket terminates the dispatch loop and a fresh bridge becomes responsive in <1 s. An opt-in `Supervisor::prune_stale_mac_artifacts` helper (Mac-only, idempotent) removes orphaned overlay + socket files from a prior crashed supervisor without touching unrelated files in the same directories; a fresh `Supervisor` does not falsely report stale on-disk artifacts as running. The Vz teardown failure-mode matrix documents which `VZErrorCode` values can fire on stop (`VZErrorInternal`, `VZErrorVirtualMachineGuestPaniced`, `VZErrorOperationCancelled`) and how each is surfaced today (typed `ElastosError::Compute`, no panic). **Phase 4 Day 4 audits capsule-manifest plumbing end-to-end on the Vz launch path.** Every manifest field the Linux supervisor's `launch_capsule` reads is honoured by `build_vm_config_for_mac` / `start_capsule_vm_macos` at parity (full table in [`vz-backend/PHASE_4_DAY_4_NOTES.md`](vz-backend/PHASE_4_DAY_4_NOTES.md)). The only Mac-specific addition is a pre-flight memory guard: a manifest requesting more RAM than the host can satisfy is rejected with an actionable error before any handle/CID/overlay is allocated, instead of bubbling up Apple's opaque `VZErrorInvalidVirtualMachineConfiguration` at boot. A new visibly-skipping integration test (`elastos-server/tests/vz_supervisor_smoke.rs`) drives the production launch pipeline against a real installed capsule and verifies launch within 30 s + clean stop within 10 s; an optional `provides:` round-trip exercises the Phase 4 Day 3 cross-VM dispatch path end-to-end against a real Vz boot. **Phase 4 Day 3 audits cross-VM provider RPC under N concurrent microVMs.** The dispatch path `run_carrier_bridge_loop → handle_request → ProviderRegistry::send_raw → VmCapsuleProvider::send_raw → VmRawBridge::send_raw_blocking` has one shared-state touch per layer: the registry holds a read lock only across the `Arc<dyn Provider>` clone (released before await); each `VmCapsuleProvider` has its own `Mutex<Option<VmIo>>` that serializes against a single provider VM but never against a sibling VM. The host bridge has NO request-id allocator by design — pairing is by strict order over the per-VM connection, which the Mutex enforces. New tests prove: (a) two synthetic provider VMs + three consumer tasks issuing 60 cross-VM RPCs see every nonce paired with the right provider and the right consumer (no cross-talk, no losses, no deadlocks), (b) 100 parallel `PendingRequestStore` create + half-grant / half-deny calls end with exactly 50 Granted + 50 Denied (no losses, no double-resolution), (c) 1000 parallel `CapabilityManager::validate` calls finish in <5s wall-clock (proves the verify path stays read-mostly under load). See [`vz-backend/PHASE_4_DAY_3_NOTES.md`](vz-backend/PHASE_4_DAY_3_NOTES.md). **Phase 4 Day 2** audited Carrier-bridge multiplexing under N concurrent Vz VMs ([`vz-backend/PHASE_4_DAY_2_NOTES.md`](vz-backend/PHASE_4_DAY_2_NOTES.md)). **Phase 4 Day 1** unlocked N concurrent Vz VMs by moving Apple's GCD serial queue to per-`VZVirtualMachine` ownership and exercising the supervisor's `next_cid` allocator with a 100-parallel-caller uniqueness test (see [`vz-backend/PHASE_4_DAY_1_NOTES.md`](vz-backend/PHASE_4_DAY_1_NOTES.md)). Earlier Phase 3 days landed the supervisor → `VzProvider` seam (Day 1), the launch prefix (Day 2), `CapsuleBackend::VzVm` (Day 3), the Carrier console socketpair (Day 4), delegate-driven exit codes + the host→guest vsock primitive (Day 5), `MacVsockDial` provider-bridge integration (Day 6), and the `com.apple.vm.networking`-entitlement-gated bridged networking path (Day 7 — see [`vz-backend/PHASE_3_DAY_7_NOTES.md`](vz-backend/PHASE_3_DAY_7_NOTES.md)). |
