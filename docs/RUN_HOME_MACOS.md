...
| `ELASTOS_DDRM_SUBJECT` | **dev builds only.** Pin the on-chain wallet `subject` (else the principal's linked EVM account). Compiled out without the `dev-modes` feature, and a release binary handed this variable REFUSES TO BOOT — a pin points every principal's rights check at one operator-chosen wallet, so aiming it at a wallet holding the access token unlocks the asset for the whole node |
...
| `link an EVM wallet to buy access` (403) | Buy attempted in a chain mode with no linked wallet | Link an EVM account. (`ELASTOS_DDRM_SUBJECT` also satisfies it, but only on a dev build — a release binary refuses to boot with it set) |
