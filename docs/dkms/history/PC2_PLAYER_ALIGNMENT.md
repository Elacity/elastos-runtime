...
- **B2b — runtime scoped media routes** (`elastos-server`
  ...
  must own the session. `POST …/media/:session/close` (same gate, idempotent 204)
  releases the session explicitly — fired by the player on `pagehide` and by the
  Home shell on window close; a 60s sweeper releases expired sessions on the clock
  (see `api/session_lifecycle.rs`).
