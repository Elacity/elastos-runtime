...
**Last updated:** 2026-06-16 (Day 139 — dev-mode guard hardening (audit HIGH+2MEDs closed by construction) + PC2 creator core sale/rights parity LANDED & pushed; pre-audit security backlog #1–#5 NOW CLOSED — see the Day 139 closure addendum below). Prior: 2026-06-15 (Day 138 — open-latency optimization Tiers 1+2, WireGuard demoted, live staged progress UI, audio-only DASH fix).
**Active branch:** `feat/ddrm-hardening-and-creator-parity` (pushed to `origin`; forked from `feat/ddrm-home-playback`). Prior: `feat/ddrm-home-playback` (v0.4.0 + full dDRM convergence stack).
...
> **🔐 Sprint 1 (W1–W4) — TRUSTLESS, NO-ENROLLMENT AUTHORIZATION on the PQ dKMS quorum (the trust-model heart; LANDED + tested, live Base read proven).**
> The dKMS node no longer trusts an enrollment allow-list + an unsigned rights receipt; it now authorizes each recover ITSELF from a wallet-signed grant + a node-side on-chain read — the scalability/trustlessness fix for millions of sovereign runtimes (no manual enrollment, access follows the live on-chain token; sell/transfer ⇒ next open evaluates false ⇒ fail closed). Mirrors PC2's Lit-action `hasAccessByContentId`-in-the-TEE, but with the wallet/session signatures + the chain read both done by the secret-holder.
> **(W1) `ddrm-envelope::access` (feature `access-grant`, OFF by default so the wasm decrypt boundary stays lean):** `AccessDelegationV1`/`AccessRequestV1`/`AccessGrantV1` + a byte-identical canonical (sorted-key, no-whitespace) JSON encoder (PC2 `canonicalize` parity), EIP-191 (secp256k1 `personal_sign`) recovery, an EIP-1271 smart-account hook, an ML-DSA-65 per-request session signature, and a `ReplayGuard` (revocation + per-request-nonce). 18 unit tests incl. PC2 exploit-regression vectors (tampered coveredAddresses, expired/not-yet-valid window, over-wide window, wrong kid/quorum, replay, revoked, wrong session key, malformed sig, smart-account path). Client-side builder (`build_delegation`/`build_request`/`assemble_grant`) so the runtime constructs grants from a single wallet sign. **(W2) `dkms-authority`:** a pinned, read-only, multi-RPC Base capability (`NodeChain`, config-only via `DKMS_CHAIN_RPC_POOL`/`DKMS_RIGHTS_CONTRACT`/`DKMS_RIGHTS_SELECTOR`) + `authorize_access()` — verifies the wallet (EIP-191/1271) + session signatures + window/nonce/node-set/kid binding ITSELF, then evaluates `hasAccessByContentId` per covered address ITSELF, requiring agreement among reachable RPCs (a contract revert ⇒ `false`, PC2 `.catch(()=>false)`; disagreement or insufficient reachability ⇒ FAIL CLOSED). Gated into `recover_inner`. **Live proof:** the production Base AuthorityGateway (`0x09dBe796…`) answers the node's exact `hasAccessByContentId(address,bytes16)` calldata (selector `0x54d42821`) and reverts for an unregistered contentId across three independent RPCs — exactly the fail-closed `false` path. 11 node tests (quorum agreement/disagreement/reachability, revert→false, forged grant rejected before chain, foreign-quorum rejected, chain-false denies, chain-unavailable fails closed). **(W3) anonymous-safe callers:** the legacy unsigned-receipt path is honored ONLY for an enrolled (allow-listed) caller; an anonymous caller (no allow-list — the millions-of-runtimes posture) MUST present a grant and can never forge `allowed:true`. `DKMS_AUTHORITY_ALLOWED_CALLERS` demoted from the security boundary to an optional DoS gate + the legacy-path trust scope (behind feature `legacy-receipt-authz`, on by default during migration). Tests: anon-without-grant refused; enrolled-legacy still authorizes. **(W4) wire:** `KeyReleaseRequestV1.access_grant` (opaque `Value`, keeps core free of the PQ-crypto capsule) → `key-provider` forwards it into the recover request → the node deserializes + verifies. The gateway (`library.rs`) is the thread point; the LIVE browser→MetaMask `personal_sign` collection + session-key mint there is the remaining live-integration step (deferred, needs browser+wallet, like the W0 geo-mesh validation).
> **(W7 codeable slice)** `ddrm-envelope::split_cek_shamir` generalizes the producer split to any t-of-n (degree-(t−1) over GF(256); pairs with the existing general `lagrange_combine_at_zero`); tests: 3-of-5 any-3 reconstruct, any-2 fail, byte-identical to `split_cek_shamir2`. **Deferred (live/ops, next sprint):** W0 live geo image/video open (needs dkms0 mesh-join), W4 browser wallet round-trip, W5 on-chain node registry + public transport (needs a deployed registry + firewall change), W6 video over the remote quorum (gated on live geo nodes), and the rest of W7 (client-rail t-of-n generalization, live 5-of-12 reconfiguration, master.seed backups, operator-key custody, monitoring). **Gate:** `ddrm-envelope` 39 default + 57 with `access-grant`; `dkms-authority` 34 (default) green + trustless-only (`--no-default-features`) build green; `key-provider` bin + `elastos-common` + `elastos-server` compile; lints clean. **Authorization is now trustless: a recover succeeds ONLY with a valid wallet-signed grant the node verifies + an on-chain token the node reads itself — anonymous-but-authorized works, forged fails closed.**

> **⚡ Day 138 checkpoint (2026-06-15) — OPEN-LATENCY OPTIMIZATION (Tiers 1+2), TRANSPORT CLEANUP, LIVE PROGRESS UI, AUDIO-ONLY FIX. The live on-chain revocation gate is UNCHANGED — every open still does the wallet-signed grant + node-side `hasAccessByContentId` read; these changes only remove redundant handshake/signature/round-trip cost behind that gate.**
>
> **What landed (committed on `feat/ddrm-home-playback`):**
> - **Tier 1 — warm `key-provider` daemon + cross-open node-session pool (`d6412e078`, stack fix `adc14baef`).** `key-provider` now runs as a long-lived Unix-socket daemon (spawned by `run-creator-gateway.sh` when `QUORUM_OPEN_READY`), self-initialized from `KEY_PROVIDER_DAEMON_{DESCRIPTOR,CALLER_SEED_B64}`. `DkmsNodeConn` is now `Send` (boxed `Read`/`Write`), and `release_quorum` recovers the 2-of-3 **in parallel** across threads (each given a **16 MiB stack** — the default 2 MiB overflowed during ML-KEM-768/ML-DSA-65), reusing warm node connections from an `Arc<Mutex<HashMap>>` pool across opens. The gateway helper (`ddrm-media-authority/src/quorum.rs::key_release`) dials the daemon socket when `ELASTOS_DDRM_KEY_PROVIDER_SOCKET` is set, else falls back to a per-open spawn. **Effect: quorum-recover ~10s cold → ~3.5s warm (≈65% faster on repeat opens).**
> - **Tier 2 — popup-free re-opens via cached wallet-signed delegation (`018900d95`).** Mirrors PC2's `pc2-secure-view-session.js`: a `GrantSessionStore` (in `api/access_grant.rs`) caches the wallet-signed `AccessDelegationV1` keyed by `(owner, kid)` for a TTL (`DELEGATION_WINDOW_SECS` = 3600s). `prepare_owned_grant` returns `already_delegated` when a live session exists, so `shell-windows.js` skips the MetaMask `personal_sign` and `assemble_cached` mints the grant from cache. **The CEK is still recovered fresh and the chain check still runs every open** — only the *re-signature* is skipped, and only for the **same asset** within the TTL. (Full PC2 asset-agnostic delegation needs an `AccessDelegationV1` schema change + geo-node redeploy — deferred.)
> - **Transport cleanup (WireGuard → Carrier).** `run-creator-gateway.sh` relabels Carrier/did:key as the production transport and the `tcp:10.66.66.x` WireGuard mesh as a **deprecated rollback-only** path. Live mode = `ELASTOS_DKMS_REMOTE=1 ELASTOS_DKMS_CARRIER=1` reaches the 3 geo nodes by DID over QUIC/relay/hole-punch (no VPN). The carrier sidecar (`scripts/dev/dkms-carrier-client`) keeps warm connections (`ConnCache` + keep-alive + pre-warm of `DKMS_CARRIER_WARM_DIDS`).
> - **Live staged progress UI for protected opens (`4fc4cf500`).** `shell-windows.js` + `style.css` render an honest checklist (preparing session → verifying access & recovering keys → decrypting & preparing playback) instead of a blank spinner; stages advance client-side as `openOwnedRequest`/`launchOwned*` progress.
> - **Audio-only DASH fix (`2b4ea0564`, THIS session).** `media-provider::fragment_rendition` previously always applied `-vf scale` + libx264, which aborts ffmpeg on an audio-only source (no video stream). Now branches on video presence: audio-only encodes with `-vn -c:a aac` and fragments identically. **Audio assets now package + mint** (was broken before).
>
> **Media-pipeline parity status (answer to "are we doing the right transcode/encode/package like PC2?"):**
> - **Video (`video/*`)** → media path: faithful PC2 port (`media-provider` header documents the exact parity anchors — `libx264 -crf <ladder> -preset slow -profile:v <p> -pix_fmt yuv420p`, `aac -b:a 128k`, fragment `+frag_keyframe+empty_moov+default_base_moof+separate_moof`, split + MPD via `ddrm-media::{mp4,mpd}`), then CENC per-fragment under one asset CEK + dKMS escrow. **Solid parity.** (Single top-rung today; full multi-rung adaptive ladder is P4.)
> - **Audio (`audio/*`)** → same media path, now with the audio-only encode branch (above). **Fixed but not yet live-tested** — verify a real `.mp3`/`.m4a` mints + plays.
> - **Image / PDF / 3D models** → **object path**: classified non-media (`creator.js::classifyMedia` = `video/*`||`audio/*` only), so they are encrypted as-is (no transcode — correct, matches PC2's generic-file handling) with a generated cover thumbnail (PC2 `elacity-creator` cascade: custom upload › blurred still for images › video frame › audio waveform › text teaser › generative gradient).
>
> **Verified live this session:** gateway restarted clean in `REMOTE+CARRIER+chain` mode; server on `:8090`, warm `key-provider` daemon LIVE, rights gate = live on-chain `hasAccessByContentId`, 3 geo nodes over Carrier.
>
> **PENDING / next (in priority order):**
> 1. **Tier 3 — short-TTL in-memory CEK cache** behind the live chain gate (default TTL **90s**), hosted in the same persistent recovery daemon as Tier 1, for near-instant repeat opens with a bounded, documented security tradeoff. (Scoped-with-Tier-1; not yet implemented.)
> 2. **Audio-only live verification** (mint + play a real audio file end-to-end).
> 3. **PC2 creator-UX backlog** (user-requested, not yet built): **(a)** a creator **live mint-progress dropdown** showing granular pipeline sub-stages (PC2's processing tracker); **(b)** **early-start background processing** — kick off transcode/encrypt/pin the moment the file is selected (while the creator fills in title/description/supply) so the heavy work is done/closer by the time they hit mint. PC2 starts processing on upload, not on submit.

> **🛡️ Day 139 checkpoint (2026-06-16) — DEV-MODE GUARD HARDENING + PC2 CREATOR CORE PARITY. Committed as `ab3adb310` and pushed to the NEW branch `feat/ddrm-hardening-and-creator-parity` (origin: `Elacity/elastos-runtime`). Plan: [`.cursor/plans/ddrm_hardening_and_creator_parity_2373f015.plan.md`]. The live on-chain authorization gate is UNCHANGED.**
>
> **Phase 1 — security: insecure dev modes are now impossible in a release build by construction (closes the audit HIGH + 2 MEDs; implements [`DEV_MODE_GUARD_SPEC.md`](./DEV_MODE_GUARD_SPEC.md)).**
> - A single per-crate **`dev-modes` cargo feature, OFF by default**, gates all three forgeable dev paths: (1) the `reference` key backend (`key-provider` refuses to *select* it without the feature even though `key-authority-ref` still compiles for the production dkms quorum path); (2) `legacy-receipt-authz` (removed from `dkms-authority` `default`); (3) `Dev`/`ChainMock` rights modes. `rights_mode()` now **defaults to `Chain`** in a non-dev build; `ddrm-runtime-open` defaults to the `dkms` backend.
> - **Fail-closed startup guard** (`enforce_release_build_rights_safety()` in `rights_authority.rs`, called from `run_gateway_direct` in `gateway_cmd.rs`): a release build handed `ELASTOS_DDRM_RIGHTS=dev|chain-mock` refuses to boot.
> - **Defense-in-depth:** `security_now()` clamps security-expiry checks to the node's real clock (a backdated caller `now_unix` can no longer extend an expired delegation/token); a process-held `ReplayGuard` is wired into `authorize_access`; the dkms recover-thread spawn `.expect()` is now a **counted fault** (a spawn failure under resource pressure no longer aborts the warm daemon).
> - **Local dev preserved:** `run-creator-gateway.sh` builds `dkms-authority` + `elastos-server` with `--features dev-modes`; `key-provider` stays `--features key-authority-ref` (production posture — dkms compiled, reference fenced at selection), so the local gateway exercises the SAME secure key-release posture production ships.
> - **Docs/tests:** GAP-7 + the `capability_conformance` harness + `SECURITY_AUDIT.md` flipped to **closed**. Release-posture tests green: `dkms-authority` 24/24, `publish-provider` 16/16, `creator` 14/14, conformance 8 pass (+ GAP-7-closed assertion). *(Pre-existing, unrelated env-dependent failures in `gateway_browser_route_tests` + `setup::tests` — confirmed failing identically with my changes stashed; they need a display session / installed-binary checksums.)*
>
> **Phase 2 — PC2 creator core sale + rights parity (the providers were already PC2-complete; this wired the orchestrator + UI to them).**
> - `MintMeta` (`creator.rs`) gains `access_method` (`free`|`buy_once`|`buy_and_resell`), `reseller_cut`, `royalties[]` (`{address, royalty%, identifier?}`), `licensing`, `legal_attestation`, `is_adult`; `op_type_for`/`op_type_code`/`distribution_label` now emit code **2 / "Buy & Resell"**.
> - `finalize_mint` wires `royalties[]` + `reseller_cut` into `prepare_publish`; `validate_royalty_total` enforces the PC2 95% creator / 5% protocol split.
> - `asset/v1.1` metadata gains `Resell-Allowed`/`RRL-Percent` (= `reseller_cut/10`, deci-percent — PC2 `900`⇒90%)/`Licensing`/`Adult` attributes + `properties.legal`; the **distribution-right token-type file (`…0003.json`)** is emitted only when resellable; `contract.json` carries the resale flags.
> - **Creator UI** (`capsules/creator/{index.html,creator.js}`, cache-bust `creator-20260615e`): access-method selector cards (free/buy_once/buy_and_resell, show/hide price), royalty rows with a live 95/5 total hint, a resale-royalty (RRL %) input (resell-only), an AI-licensing toggle (mirrors PC2 `getLicensingData` perpetual↔training-rights), a required legal-attestation checkbox, an adult flag, a category dropdown with `3d-model`/`ai-model` auto-detect, and a currency selector — all included in the `/prepare-mint` body.
>
> **NOT YET DONE (the user has not tested this work):**
> - ⚠️ **Live mint verification is the immediate next step** (the user's manual test). Restart with `scripts/dev/run-creator-gateway.sh`, then at `http://localhost:8090/apps/home/` mint one each of **free / buy_once / buy_and_resell** and confirm: on-chain `opType`/`opRawData`/`sellRawData`, the royalty split, the `…0003.json` distribution-right file, and that `base.ela.city` shows title/description/distribution/royalty/licensing. (Hard-refresh the creator to pull the new UI.)
>
> **✅ PRE-AUDIT SECURITY BACKLOG #1–#5 — ALL CLOSED (2026-06-16, Day 139). See the closure checkpoint immediately below this list for the per-item fix + test + commit. The original backlog is retained here for the record.**
>
> **🔐 PRE-AUDIT SECURITY BACKLOG (from the internal adversarial review + the user's security researcher, 2026-06-15/16). Full detail with `file:line` evidence is in [`docs/PRE_AUDIT.md`](../../PRE_AUDIT.md) — that file is DELIBERATELY UNTRACKED / NOT pushed to the public repo (it lists the found-and-fixed exploit locations); keep it local for the auditor.** Priority order (the researcher's "Cursor prompts"):
> 1. **🔴 (HIGHEST) CEK reconstruction has no integrity check.** A single malicious node can return a validly-sealed, validly-indexed but **wrong-valued** share; it passes every structural check and combines into a **silently wrong CEK** (the unauthenticated AES-CTR layer won't even error). Benign for media (garble), **disastrous for the agent-key-custody roadmap** (a silently-wrong signing key). The commitment that catches it (`dkg_cek_binding`, `ddrm-envelope/src/lib.rs:676`) **already exists but is test-only.** **Fix:** on the live quorum/threshold open path (`decrypt-provider/src/rail_shim.rs` `reconstruct_quorum_two_cek`/`reconstruct_threshold_cek`), bind the reconstructed CEK to the published commitment before use, and add cheater-detection across the 3 fetched candidate shares (the rail already fetches all three — `key-provider/main.rs:1809-1843` — so a 2-of-3 consistency check is cheap) so a Byzantine wrong-valued share **fails the open closed**. Add a malicious-share test.
> 2. **🟠 GAP-8: tamper-evident audit + the missing open event.** The runtime-core audit (`primitives/audit.rs:300-331`) is best-effort (silent-drop on serialize/lock/write failure, no hash-chain/signature/fsync/seq, `emit()` returns `()`), **and the dDRM open path emits no audit event at all** (only `tracing` — undercuts the custody pitch). **Fix:** hash-chain (`seq`+`prev_hash`) + per-entry/checkpoint signature (reuse `ddrm-envelope` ML-DSA), make `emit` return `Result` and fail-loud (fail the audited op closed for custody), `sync_all()`, append-only, and **emit a content-open `AuditEvent` from `viewer_open.rs`.**
> 3. **🟠 Action enforcement is delegated to provider capsules.** The host bridge validates the token is signed/caller-bound/resource-matched, but localhost `read` and `write` share the same resource string and fine-grained *action* checks are left to each provider (`carrier_bridge.rs:688`, `api/handlers/provider.rs:112-118`). **Fix:** audit every provider's dispatch (wallet, key, decrypt, localhost storage) for an explicit operation→action check, or enforce the action centrally at the bridge.
> 4. **🟠 Quick hardening (one pass):** (a) make `DKMS_AUTHORITY_NODE_SET_ID_B64` **mandatory in release builds** (`dkms-authority/main.rs:1793`); (b) add a `MAX_LINE_BYTES` cap to `vsock-proxy` (`tools/vsock-proxy/src/main.rs`) + confirm the guest netns is non-routable; (c) make `gf256_mul` (`ddrm-envelope/lib.rs:317`) **branchless** with arithmetic masks (`subtle` already vendored); (d) clamp `effective_now` at **session issuance** to the node clock to match `security_now` (`dkms-authority/main.rs:99`).
> 5. **🟡 Metadata confidentiality quick wins + honesty.** The `(wallet, content_id, time)` triple is visible to dKMS node operators (≥2-of-3), the chain, the RPC provider, and our own `info!` logs. **Fix (quick):** stop logging `subject`/`principal_id`/`content_id` at `info!` in `viewer_open.rs:282-289`; add frame padding to the dKMS channel; write `docs/THREAT_MODEL.md` stating plainly what node operators/chain/observers can see (the access pattern) and that the trust model is **2-of-3 (not "no collusion")**. (Deep fix — blinded identifiers / oblivious lookup — is roadmap, flag for the firm.)
>
> **Pre-audit "verified clean" (the money-saver — the external firm scopes these OUT, evidence in `PRE_AUDIT.md`):** GF(256) has **no** table-lookup side channel (table-free shift-XOR); no exploitable variable-time secret compare (auth is signature-rooted); a single malicious node cannot steal the key or forge authorization; node-set trust is attested not config-trust; the quorum-release proof verifier is clean; 6 of 7 host adapters are fail-closed + identity-bound; the dev-mode fences are by-construction (Phase 1 above). **What still needs the external firm:** a funded crypto/protocol audit of the PQ-hybrid envelope + Shamir/threshold scheme before custodying real value; a live pen-test of a deployed instance; the node-operator trust model at scale (attestation/slashing toward permissionless nodes).

> **✅ Day 139 checkpoint addendum (2026-06-16) — PRE-AUDIT BACKLOG #1–#5 CLOSED, each fail-closed with a regression test. #1–#4 committed as `3f282dd0d`; #5 + `docs/THREAT_MODEL.md` in the sibling "metadata minimization + threat model" commit. `PRE_AUDIT.md` stays UNTRACKED (found-and-fixed exploit detail; hand to the auditor out-of-band, do not push).**
>
> - **#1 CEK reconstruction integrity (was HIGH) — CLOSED.** The reconstructed CEK is bound to a published commitment BEFORE use on the live quorum/threshold open path, with cheater-detection across the fetched candidate shares; a Byzantine well-formed-but-wrong share fails the open closed, and an uncommitted 2-of-2 open is refused. `ddrm-envelope` (`cek_commitment`/`verify_cek_commitment`/`combine_cek_shamir2_checked`), `decrypt-provider/src/rail_shim.rs` (`decrypt_from_carrier_{quorum,threshold}_checked`), propagated through `encrypt`/`key`-provider + the media-authority/producer rails. Byzantine-share regression test in `decrypt-provider`.
> - **#2 Tamper-evident audit + content-open (GAP-8) — CLOSED.** `elastos-runtime/src/primitives/audit.rs` is now hash-chained (`seq`+`prev_hash`+`record_hash`), **ed25519-signed** (in the trusted core, crypto-agility tag), `emit()→Result`, persisted 0600 signing key, `verify_chain`. A content-open custody record is written before an authorized open proceeds (`viewer_open.rs`) and a failed append fails the open closed (503). *Caveat in code + THREAT_MODEL: tamper-evident vs external editing + non-repudiable, NOT vs a live-compromised runtime — external anchoring is roadmap.*
> - **#3 Central action-enforcement — CLOSED.** `required_action_for(op)` (`provider_resource.rs`) maps each operation to its required capability `Action` (fail-closed `Admin` default); validation at the bridge (`carrier_bridge.rs`) and the HTTP provider proxy (`api/handlers/provider.rs`) now checks the operation's required action, not the token's self-declared action.
> - **#4 Quick hardening — CLOSED.** (a) `DKMS_AUTHORITY_NODE_SET_ID_B64` mandatory in release (cross-quorum-replay defense; fallback only in test/dev-modes); (b) `MAX_LINE_BYTES` (16 MiB) cap on every `vsock-proxy` line read + guest netns confirmed non-routable (host-only /30, no `ip_forward`/NAT/route); (c) `gf256_mul` made branchless (arithmetic masks, no control-flow timing leak; golden vectors prove identity); (d) `effective_now` clamped to the node clock at issuance (a caller can only shorten its window, never extend past TTL).
> - **#5 Metadata minimization + threat model — CLOSED.** `viewer_open.rs` logs `subject`/`content_id` only as non-reversible `log_fp` fingerprints (no raw `(wallet, content_id)` in `info!`); the dKMS channel pads each frame's plaintext to a coarse size bucket before sealing (`ddrm-envelope` `channel_pad`, applied at every channel seal/open site); and [`docs/THREAT_MODEL.md`](../../THREAT_MODEL.md) states the **2-of-3 (not "no collusion")** trust model and the observable `(wallet, content_id, time)` access pattern honestly, with oblivious lookup flagged as the highest-value next scope.
>
> **Gates:** per-crate green — `ddrm-envelope` 41/41, `dkms-authority` 24/24, `key-provider` 18/18, `decrypt`/`encrypt`/server suites consistent; `just alignment-check` GREEN. *(These capsule crates + `vsock-proxy` are not `elastos`-workspace members, so `just verify` does not exercise them — tested per-crate.)* **Next:** the live multi-asset mint + dDRM open test the user drives at `http://localhost:8090/apps/home/`.
...
All in `docs/dkms/history/`. Read 1→3 to onboard; the rest are reference.
...
1. docs/dkms/history/HANDOVER.md — §0–§3 + Day 136 checkpoint at top
2. docs/dkms/history/NEW_AGENT_BRIEF.md — mandate + working method
3. PRINCIPLES.md — constitution (wins on conflict)
4. docs/dkms/history/SYSTEM_ARCHITECTURE_MAP.md — whole journey map
5. docs/RUN_HOME_MACOS.md — Mac gateway, WebAuthn (localhost not 127.0.0.1), env vars
6. docs/dkms/RUN_E2E.md — consumer open runbook + smoke scripts
7. docs/dkms/deploy/RUNBOOK.md — live quorum node daemons (playback needs ≥2/3 up)
8. docs/dkms/history/CONVERGENCE_AUDIT.md — what's green vs gap
