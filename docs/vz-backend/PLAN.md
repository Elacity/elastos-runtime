...
**Status as of `73cd293 + Day-6 commit`**: Phase 3 Day 6 complete. The Day-5 vsock dial primitive is now plumbed through `elastos-server/src/vm_provider.rs` end-to-end: a new `MacVsockDial` closure type (`Arc<dyn Fn(u32) -> BoxFuture<io::Result<OwnedFd>> + Send + Sync>`) is accepted by `VmCapsuleProvider::new_with_vsock_dialer`, and `VmRawBridge::try_connect_once` short-circuits the `AF_VSOCK` path when the dialer is set, routing all host→guest connections through `RunningVm::connect_vsock` (Apple's `VZVirtioSocketDevice.connectToPort:`). On the supervisor side, `Supervisor::register_provider_route_with_vsock_dialer` is the Mac sibling of `register_provider_route`; `start_capsule_vm_macos` now builds a dialer closure capturing `Weak<RwLock<HashMap<String, RunningCapsule>>>` plus the capsule handle, registers it whenever the manifest declares `provides:`, and attaches the resulting `ProviderRoute` to the inserted `RunningCapsule`. A torn-down VM surfaces as `io::ErrorKind::NotConnected` rather than a dangling Arc or panic. The Linux flow is byte-identical: `VmCapsuleProvider::new` and `register_provider_route` still drive the existing `socket(AF_VSOCK,…)` path. Five new tests cover the surface: bridge round-trips JSON over a dialer-provided socketpair, propagates dialer errors, takes priority over the AF_VSOCK path; the supervisor helper actually lands a provider in the registry; and the closure-shape returns `NotConnected` for missing/dropped running maps. Outcome log in [`PHASE_3_DAY_6_NOTES.md`](PHASE_3_DAY_6_NOTES.md). Day 7 next: `VZBridgedNetworkDeviceAttachment` (entitlement-gated, fail-closed today) + multi-VM concurrent boot stress.
