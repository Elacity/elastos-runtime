...
**Status as of `<Phase-5-Day-8 commit>`**: ✅ **Phase 5 complete.** All 8 days shipped; see [`PHASE_5_RETROSPECTIVE.md`](PHASE_5_RETROSPECTIVE.md) for the closeout and [`PHASE_6_ENTRY_CHECKLIST.md`](PHASE_6_ENTRY_CHECKLIST.md) for the Phase-6 gates. Day 8 closed the phase with a **DRY hoist + perf-schema v2 bump + retrospective**. New `cross_platform_smoke_log_dry_run_reason` helper in `scripts/lib/cross-platform.sh` consolidates the Day-5/Day-6 dry-run-reason echo logic; all three Mac smokes refactored to use the predicate + log helper pair (operator-visible echo lines preserved byte-for-byte in the production CI auto-detect path; verified via a diff against the Day-7 baseline). Bash helper assertion count: 44 → 47 (3 new for `log_dry_run_reason` covering explicit DRY_RUN=1, CI auto-detect byte-exact echo, CI side-effect export). New `elastos-server/tests/common/mod.rs` extracts `seed_cached_synthetic_capsule` + `synthetic_components_manifest` from `vz_perf_harness.rs` (Day 7) and `vz_supervisor_startup_orphan_cleanup.rs` (Day 4) — single source of truth for the on-disk cached-capsule shape `ensure_capsule` consults to short-circuit the IPFS download path. Perf-report `schema_version` bumped 1 → 2: new top-level `git_sha` field, populated by `measure-{vz,crosvm}-baseline.sh` from `git rev-parse --short=12 HEAD` (with `-dirty` suffix when uncommitted changes are present), with `ELASTOS_VZ_PERF_GIT_SHA` env-var contract between wrapper and harness. Schema v2 is purely additive — v1 consumers ignoring `git_sha` still parse v2 files. Test count preserved at 598 (the existing `perf_report_json_schema_is_stable_for_consumers` test was extended to cover v2 + the default-branch fallback). New `PHASE_5_RETROSPECTIVE.md` documents what shipped, what went well, what didn't (Day-5/6/7 cascade rationale, self-hosted runner provisioned but not active), and 8 carry-forward items. New `PHASE_6_ENTRY_CHECKLIST.md` enumerates the Phase-5 closeout gates (9 items, all checkable) + 3 Phase-6 unblockers (`components.json` darwin-arm64 metadata, self-hosted runner activation, first end-to-end full-boot smoke) + the Phase-6 backlog. All quality gates green: `fmt`, `clippy`, 598 Rust tests on macOS, 47 helper assertions, byte-identical CI smoke output, schema_version=2 verified via Python check, Linux side untouched.

**Status as of `<Phase-5-Day-7 commit>`**: Phase 5 Day 7 complete — Performance measurement substrate + honest baseline document landed. New `elastos-server/tests/vz_perf_harness.rs` (11 tests; 6 measurement + 5 schema/utility guards) measures the Phase-4/5 synthetic Rust paths the Vz and crosvm backends both lean on: `supervisor_new_cold`, `supervisor_new_warm`, `synthetic_capsule_launch`, `provider_registry_send_raw_{single,concurrent}`, `capability_manager_validate`. Each metric emits a `schema_version: 1` JSONL line into `ELASTOS_VZ_PERF_REPORT` (if set) so the cross-platform `scripts/measure-vz-baseline.sh` (Mac/Vz canonical lane) and `scripts/measure-crosvm-baseline.sh` (Linux/crosvm comparison lane) can aggregate 5 runs each and write `target/{vz,crosvm}-baseline.json`. Both scripts honor the Day-5/Day-6 precedence (FORCE_FULL → explicit DRY_RUN → CI auto-detect → default). New `docs/vz-backend/PERFORMANCE_BASELINE.md` documents methodology, what we measure today vs what's Phase-6-gated, initial Mac (M-series) numbers (all paths sub-millisecond at p99), per-metric observations explaining the cold/warm delta + `send_raw_single` resolution-floor behaviour, and a pre-baked Phase-6 comparison table with `_TBD_` cells for Linux + real-boot rows. JSON wire format frozen at `schema_version: 1`. No production Rust code changes — the harness lives entirely under `tests/`. Day-8 (last Phase-5 day) will populate the Linux side of the comparison table + ship the Phase-5 retrospective.

**Status as of `<Phase-5-Day-6 commit>`**: Phase 5 Day 6 complete — Self-hosted full-boot CI lane shipped (wired but dormant pending operator provisioning). New `mac-vz-full-boot` job in `.github/workflows/mac-vz.yml` doubly-gated on `vars.MAC_VZ_FULL_BOOT_ENABLED == 'true'` (default off so public forks stay green) + `runs-on: [self-hosted, macOS, ARM64, vz-capable]`. New top-priority env var `ELASTOS_VZ_SMOKE_FORCE_FULL=1` sits at the top of a documented 5-layer smoke precedence table (FORCE_FULL → explicit DRY_RUN → CI auto-detect → default), wired into all three smokes with operator-visible echo lines. Canonical predicate `cross_platform_smoke_should_dry_run` in `scripts/lib/cross-platform.sh` encodes the full precedence; 3 new shell-helper assertions pin it (41 → 44). New `.github/workflows/_self-hosted-probe.yml` heartbeat (cron `0 0,6,12,18 * * *` + `workflow_dispatch`) probes the runner pool every 6 hours plus an always-on fallback job records the variable's current value for the operator audit trail. New `docs/vz-backend/SELF_HOSTED_RUNNER_SPEC.md` documents hardware + label set + provisioning + security posture (hardware isolation, network segmentation, restricted repo variable) + two-stage kill switch + Phase-6 deferrals. `CI_RUNBOOK.md` § 3a covers the lane end-to-end. No Rust code changes; this day is pure CI ops surface + shell substrate.

**Status as of `<Phase-5-Day-5 commit>`**: Phase 5 Day 5 complete — `.github/workflows/mac-vz.yml` (new) gates every push + PR on a macOS Apple-Silicon runner with three jobs: `mac-rust-tests` (fmt + clippy + tests under `--test-threads=1` and `=4`, scoped to `-p elastos-server -p elastos-vz`); `mac-shell-helpers` (the bash-3.2 helper-library unit tests, 41 + 5 assertions); `mac-smokes-dry-run` (the three Phase-5 smokes in their CI fast-lane with `ELASTOS_VZ_SMOKE_DRY_RUN=1`). Each job has a generous timeout-minutes cap (30 / 5 / 10), shares a `concurrency:` group so rapid-fire pushes auto-cancel older runs, and uses `Swatinem/rust-cache@v2` with a Mac-specific `prefix-key` so the Linux `ci.yml` cache doesn't collide. New `cross_platform_in_ci` predicate in `scripts/lib/cross-platform.sh` auto-enables the dry-run lane in CI; explicit `ELASTOS_VZ_SMOKE_DRY_RUN=0` operator override always wins (verified by `CI=true ELASTOS_VZ_SMOKE_DRY_RUN=0 scripts/local-carrier-setup-smoke.sh` proceeding past the dry-run check into the Mac pre-flight). Shell-helper assertion count grew 37 → 41 (4 new cases covering env-unset → not-in-CI, `GITHUB_ACTIONS=true` → in-CI, `CI=true` → in-CI, both → in-CI). Five new `just ci-*` recipes (`ci-mac`, `ci-rust`, `ci-shell`, `ci-smokes-dry`, `ci-linux-untouched`) mirror the workflow's job steps verbatim so local Mac dev reproduces CI exactly. New `docs/vz-backend/CI_RUNBOOK.md` is the operator runbook: workflow inventory, failure-reading guide, manual-trigger instructions, local-reproduction recipes, and the explicit list of what CI does NOT cover (real Vz boot, full Linux end-to-end smokes, perf benchmarks, real darwin-arm64 metadata). `workflow_dispatch:` added to `linux-untouched.yml` so operators can re-run the gate after a rebase without pushing. Linux-untouched (vs `bcf5a0a`) remains green. See [`PHASE_5_DAY_5_NOTES.md`](PHASE_5_DAY_5_NOTES.md) and [`CI_RUNBOOK.md`](CI_RUNBOOK.md).

**Status as of `<Phase-5-Day-4 commit>`**: Phase 5 Day 4 complete — `Supervisor::new` on Mac now auto-prunes orphan overlays + control sockets + carrier-bridge sockets at startup, with the result cached for one-shot delivery on the first `SupervisorRequest::EnsureCapsule` response. New public types: `OrphanCounts` (operator JSON projection — three integer fields: `overlays_removed`, `sockets_removed`, `bridge_sockets_removed`; the last two split from the original combined field so operators can pivot on the carrier-bridge orphan rate independently — that rate maps to the Phase-4-Day-2 bridge-teardown surface, hence the separate alert dimension); `SupervisorResponse::orphans_pruned: Option<OrphanCounts>` (skip-serialises when absent so legacy dashboards keep working); `VzConfig::prune_orphans_on_startup: bool` defaulting to `true` (companion builder `with_prune_orphans_on_startup`); `Supervisor::new_with_vz_config(data_dir, registry, vz_config)` for tests and future operator harnesses. The original opt-in `Supervisor::prune_stale_mac_artifacts` (Phase 4 Day 5) remains for explicit sweeps. The Linux launch path remains byte-identical: the Mac branch is gated behind `#[cfg(target_os = "macos")]`, the helper is a no-op stub on Linux, and the dedicated `supervisor_new_is_noop_on_linux_even_with_prune_flag_set` test enforces this. +9 tests landed (1 updated): the existing `prune_stale_mac_artifacts_removes_overlays_and_sockets_but_preserves_unrelated_files` was updated for the 1+1 socket-category split; 3 new supervisor unit tests (auto-prune default, opt-out, Linux no-op, `OrphanCounts` projection); 2 new RPC-contract integration tests in `elastos-server/tests/vz_supervisor_startup_orphan_cleanup.rs` (full `Supervisor::new` → `handle_request(EnsureCapsule)` round-trip + opt-out wire-format guard); 1 new JSON wire-format test `supervisor_response_json_wire_format_for_orphans_pruned` in `vz_shutdown_semantics.rs`. All green under both `--test-threads=1` and `=4`. See [`PHASE_5_DAY_4_NOTES.md`](PHASE_5_DAY_4_NOTES.md).


**Status as of `<Phase-5-Day-3 commit>`**: Phase 5 Day 3 complete — `scripts/chat-wasm-native-interop-smoke.sh` is now Mac-aware. The shared helper library at `scripts/lib/cross-platform.sh` grew by two: `cross_platform_curl_or_skip` (publisher-gateway probe with structured skip on outage; replaces inline `curl -fsSL …` blow-ups) and `cross_platform_alert_on_vz_error_in_logs` (the Day-3 tripwire on the Phase-4-Day-7 `VzError::Display` contract — greps for the nine stable `kind_label:` tokens and fails the smoke loudly on any hit, with a false-positive guard for prose mentioning the bare token). The cross-platform unit-test count grew from 26 → 37 (over the ≥30 target); 11 new assertions cover reachable / unreachable / empty URL paths plus eight log-grep scenarios including the false-positive guard. The smoke gains four new lanes: `ELASTOS_VZ_SMOKE_DRY_RUN=1` (parity with Days 1/2), gateway-reachability probe (graceful exit-0 skip), `ELASTOS_CHAT_INTEROP_OFFLINE=1` flag (plan-mandated mitigation; requires `ELASTOS_BIN_OVERRIDE`), and an install.sh-failure capture that surfaces the published install.sh's bash-3.2 incompatibility (`GATEWAYS[@]: unbound variable` observed on this Mac) as a clean Phase-6-prerequisite skip rather than a cryptic crash. The smoke's tail runs the VzError alerting tripwire on every collected log and the Vz substrate probe (advisory). A new Mac-only Rust integration test `elastos-server/tests/vz_chat_interop_smoke.rs` locks in the bidirectional `ProviderRegistry::send_raw` contract with three tests (round-trip <5 s; unknown-scheme error surface; provider-error propagation) — running in <1 ms on M1. Phase 6 carry-forward: the published install.sh itself needs a Mac port; Day 3 surfaces this with an actionable operator message but does NOT fix it (out of scope). Full notes in [`PHASE_5_DAY_3_NOTES.md`](PHASE_5_DAY_3_NOTES.md). Quality gates all green.

**Status as of `<Phase-5-Day-2 commit>`**: Phase 5 Day 2 complete — `scripts/home-frontdoor-smoke.sh` is now Mac-aware and the shared helper library at `scripts/lib/cross-platform.sh` grew by three: `kill_pid_then_group` (SIGTERM → bounded-grace SIGKILL escalation; replaces ad-hoc `kill -- "-${pid}"` patterns), `free_port_via_python3` (DRY hoist of the inline `socket.bind` blocks), and `cross_platform_assert_native_binary_release_metadata` (single source of truth for the Mac pre-flight that asserts `components.json` carries the right `darwin-arm64` release entries). The cross-platform test file grew from 15 → 26 assertions; a new `scripts/lib/runtime-cleanup-test.sh` lands 5 more after `scripts/lib/runtime-cleanup.sh` was reworked to use `pid_is_running` + `kill_pid_then_group` instead of Linux-only `/proc/<pid>` checks. Day-1's smoke was refactored to call the new DRY pre-flight helpers (no behaviour change; cleaner failure path). A new Mac-only Rust integration test `elastos-server/tests/vz_home_frontdoor_smoke.rs` locks in the Launch → Status → Stop → VzError RPC contract that the shell smoke depends on for the three Home-frontdoor providers (`localhost-provider`, `did-provider`, `webspace-provider`) — visibly-skipping with telemetry until Phase 6 lands the `darwin-arm64` release metadata. Full notes in [`PHASE_5_DAY_2_NOTES.md`](PHASE_5_DAY_2_NOTES.md). Quality gates all green.

**Status as of `<Phase-5-Day-1 commit>`**: Phase 5 Day 1 complete — `scripts/local-carrier-setup-smoke.sh` is now Mac-aware. A new bash-3.2-clean helper library at `scripts/lib/cross-platform.sh` provides `pid_is_running` (POSIX `kill -0` replacement for Linux-only `/proc/<pid>` checks), `read_pids_into_array` (bash 3.2 replacement for `mapfile -t`), `vz_host_is_capable` (cheap `sw_vers`-based pre-flight, complementing the Rust `is_supported()`), and `vz_discover_launchable_capsule` (mirrors the `vz_supervisor_smoke.rs` discovery logic at the shell layer). The smoke gains three new lanes: (a) `ELASTOS_VZ_SMOKE_DRY_RUN=1` for CI fast-lane parse + helper-source verification without paying the build cost, (b) a Mac pre-flight that detects missing `components.json` darwin-arm64 entries before any Python staging blow-up and exits cleanly with an actionable operator message pointing at Phase 6 (PLAN.md L321), and (c) a Vz substrate readiness probe at the tail that visibly-skips with a clear message when no installed rootfs.ext4 is found. `ELASTOS_VZ_SMOKE_FORCE_PROCEED=1` lets Phase-6 developers bypass the pre-flight to iterate on the components.json restoration directly. 15 new helper-library assertions in `scripts/lib/cross-platform-test.sh` cover every helper function on both bash 3.2 (macOS) and bash 4+ (Linux). Linux behaviour byte-identical: every new path is wrapped in `OS_TOKEN == darwin` conditions or env-var opt-ins. The Day-4 `vz_supervisor_smoke.rs` Rust test (the source-of-truth for the supervisor RPC contract this shell smoke wraps) re-validated green. Day 1 surfaced two follow-up findings for Day 2: `scripts/lib/runtime-cleanup.sh`'s `/proc/<pid>` checks are Linux-only (Day 2 swaps them for `pid_is_running`); `home-frontdoor-smoke.sh` uses bash-4-only `mapfile -t` (Day 2 swaps for `read_pids_into_array`). Quality gates: bash syntax clean on both files; `scripts/check-linux-untouched.sh bcf5a0a` green. Operator runbook + full audit in [`PHASE_5_DAY_1_NOTES.md`](PHASE_5_DAY_1_NOTES.md). 8-day Phase-5 plan in [`PHASE_5_PLAN.md`](PHASE_5_PLAN.md).
