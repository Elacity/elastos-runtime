...
> **Status:** **Phase 6 substrate CLOSED. Days 1–7 complete: a real Linux 5.15 kernel boots end-to-end through `elastos-vz` on a real Apple Silicon Mac, reaching userspace handover (`Run /init`) in ~0.3 s wall-clock — full kernel printk captured via our `vm_console` tracing forwarder (see [`PHASE_6_DAY_7_NOTES.md`](PHASE_6_DAY_7_NOTES.md) § 4). 14/14 elastos-vz tests green including `single_vm_boots_to_userspace` (Day 7) and `concurrent_load_with_real_kernel` (Day 6); no substrate code path remains untested. Build-recipe limitation banner + reproducible 5-command recipe shipped Days 6–7. Phase-7 Days 1–6 complete: `elastos setup --profile minimal` on a bare Mac downloads Canonical's pinned arm64 kernel + initrd, decompresses the kernel via a new `compression: "gzip"` fetcher path (Day 2), the supervisor wires kernel + initrd into `vz_config` at construction time (Day 3 — fixes a latent `default_data_dir()` mismatch where `VzConfig::new()` pointed at `~/.local/share/elastos/...` but the installer wrote to `~/Library/Application Support/elastos/...`), a new `elastos doctor` CLI inspects the resolved Vz substrate paths read-only, runs the elastos-vz guest-kernel sanity check, and emits exact remediation hints for absent artifacts (Day 4 — earned its keep on first run by surfacing a deferred Day-3 data-dir bug on `state_dir`/`rootfs_cache_dir`, see [`PHASE_7_DAY_4_NOTES.md`](PHASE_7_DAY_4_NOTES.md) § 5), Day-5 closes that loop by extending the same `#[cfg(target_os = "macos")]` block to also override `state_dir` + `rootfs_cache_dir` so all four Vz substrate paths now resolve under one consistent `~/Library/Application Support/elastos/` tree (see [`PHASE_7_DAY_5_NOTES.md`](PHASE_7_DAY_5_NOTES.md)), AND Day-6 polishes doctor's output by wrapping its body in a thread-local `tracing::Level::WARN` subscriber via `with_default` — the supervisor's startup `vz: startup orphan-prune complete` INFO line no longer bleeds into triage output, while `serve`/`setup` keep their normal `elastos=info` logging (see [`PHASE_7_DAY_6_NOTES.md`](PHASE_7_DAY_6_NOTES.md)). The same Day-7 boot test still reproduces end-to-end in 0.32 s ([`PHASE_7_DAY_1_NOTES.md`](PHASE_7_DAY_1_NOTES.md) → [`PHASE_7_DAY_2_NOTES.md`](PHASE_7_DAY_2_NOTES.md) → [`PHASE_7_DAY_3_NOTES.md`](PHASE_7_DAY_3_NOTES.md) → [`PHASE_7_DAY_4_NOTES.md`](PHASE_7_DAY_4_NOTES.md) → [`PHASE_7_DAY_5_NOTES.md`](PHASE_7_DAY_5_NOTES.md) → [`PHASE_7_DAY_6_NOTES.md`](PHASE_7_DAY_6_NOTES.md)). 386/386 elastos-server lib tests + 109/109 elastos-vz tests pass; Linux byte-identical. The Mac Vz path is now operator-ready: setup fetches the substrate, supervisor wires it correctly, doctor inspects it cleanly. Remaining Phase-7 work (none blocking end-user features, all infrastructure/distribution): activate self-hosted Mac runner (hardware procurement only — scaffolding done); sign `elastos-server` for distribution (needs Apple Developer ID cert); carrier RPC validation (Phase 8, needs bootable rootfs).** Closes the
