...
Design + honest bounds live in `FLINT_MANDATE_ENGINE.md`, and the mandate/receipt/signed-intent
wire format in `SPEC-mandate-v1.md`; both ship on the Flint branch and are deliberately not part
of this branch. The wire format a verifier can implement from the documents on this branch is
