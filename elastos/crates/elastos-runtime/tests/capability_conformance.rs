...
// ─────────────────────────────────────────────────────────────────────────────
// 1b. Provider-route conformance — a route is authority, so binding one is a privileged
//     effect. The security-critical routes are FIRST-WRITER-WINS: boot binds them, and a
//     later registration (a launched capsule's manifest `provides:` claim) is refused
//     rather than silently overwriting. Without this, registration order IS ambient
//     authority (Principle 3) and the mediated key/decrypt plane breaks (Principle 15).
//
//     The invariant is restated here independently of the registry's own constant so the
//     test still fails if someone shrinks or reorders that list.
// ─────────────────────────────────────────────────────────────────────────────

/// The `elastos://<name>/...` sub-routes that must be first-writer-wins.
const PINNED_SUB_ROUTES: &[&str] = &[
    "encrypt", "publish", "media", "key", "decrypt", "drm", "rights", "wallet", "chain",
];

/// The main URI schemes that must be first-writer-wins. `localhost` is the user's whole
/// file plane; a capsule manifest can name it (`provides: "localhost://..."`).
const PINNED_SCHEME_ROUTES: &[&str] = &["localhost"];

struct RouteProbeProvider {
    name: &'static str,
    scheme: &'static str,
}

#[async_trait::async_trait]
impl Provider for RouteProbeProvider {
    async fn handle(&self, _request: ResourceRequest) -> Result<ResourceResponse, ProviderError> {
        Err(ProviderError::Provider("probe provider never acts".into()))
    }
...
fn probe(name: &'static str, scheme: &'static str) -> Arc<dyn Provider> {
    Arc::new(RouteProbeProvider { name, scheme })
}
...
/// Every pinned `elastos://` sub-route is first-writer-wins: the second registration is
/// refused, the first provider keeps the route, and an explicit teardown frees the slot.
#[tokio::test]
async fn pinned_sub_route_is_first_writer_wins() {
    for name in PINNED_SUB_ROUTES {
        let registry = ProviderRegistry::new();
        registry
            .register_sub_provider(name, probe("boot", "elastos"))
            .await
            .unwrap_or_else(|e| panic!("boot must be able to bind pinned '{name}': {e}"));

        let err = registry
            .register_sub_provider(name, probe("usurper", "elastos"))
            .await
            .unwrap_err();
        assert!(
            err.to_string().contains("pinned"),
            "'{name}' must be refused as pinned, got: {err}"
        );
        assert_eq!(
            bound_provider(&registry, "elastos-sub-provider", name)
                .await
                .as_deref(),
            Some("boot"),
            "'{name}' must still route to the first writer"
        );

        // Case-folding must not open a side door.
        assert!(
            registry
                .register_sub_provider(&name.to_uppercase(), probe("usurper", "elastos"))
                .await
                .is_err(),
            "'{name}' must be refused case-insensitively"
        );

        // A genuine teardown frees the slot so the same provider can restart. Teardown is keyed
        // on the bound provider, so the incumbent has to hand its own binding back.
        let incumbent = registry
            .get_sub_provider(name)
            .await
            .expect("the pinned slot is bound");
        assert!(registry.unregister_sub_provider(name, &incumbent).await);
        registry
            .register_sub_provider(name, probe("restarted", "elastos"))
            .await
            .unwrap_or_else(|e| panic!("teardown must free pinned '{name}': {e}"));
        assert_eq!(
            bound_provider(&registry, "elastos-sub-provider", name)
                .await
                .as_deref(),
...
/// Every pinned main scheme is first-writer-wins on the same terms.
#[tokio::test]
async fn pinned_scheme_route_is_first_writer_wins() {
    for scheme in PINNED_SCHEME_ROUTES {
        let registry = ProviderRegistry::new();
        assert!(
            registry.register(probe("boot", scheme)).await,
            "boot must be able to bind pinned scheme '{scheme}'"
        );
        assert!(
            !registry.register(probe("usurper", scheme)).await,
            "a second writer must be refused for pinned scheme '{scheme}'"
        );
        assert_eq!(
            bound_provider(&registry, "scheme", scheme).await.as_deref(),
            Some("boot"),
            "'{scheme}://...' must still route to the first writer"
        );

        let incumbent = registry
            .get(scheme)
            .await
            .expect("the pinned scheme is bound");
        assert!(registry.unregister(scheme, &incumbent).await);
        assert!(
            registry.register(probe("restarted", scheme)).await,
            "teardown must free pinned scheme '{scheme}'"
        );
    }
}

/// Unpinned routes stay last-write-wins — the guard must be narrow, not a blanket freeze.
#[tokio::test]
async fn unpinned_routes_are_not_frozen() {
    let registry = ProviderRegistry::new();
    registry
        .register_sub_provider("market", probe("first", "elastos"))
        .await
        .unwrap();
    registry
        .register_sub_provider("market", probe("second", "elastos"))
        .await
        .expect("an unpinned sub-route must still be rebindable");
    assert_eq!(
    ...
    assert!(registry.register(probe("first", "documents")).await);
    assert!(
        registry.register(probe("second", "documents")).await,
        "an unpinned scheme must still be rebindable"
    );
}
