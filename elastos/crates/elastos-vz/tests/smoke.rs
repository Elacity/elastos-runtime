...
//!
//! Updated for `load`, `start`, `stop`, `status`,
//! `info`, `http_port`: invalid input and unloaded handles now
//! return typed errors. The shared unavailable-backend message is
//! reserved for unsupported host/substrate paths and for the
...
fn microvm_manifest(name: &str) -> CapsuleManifest {
    CapsuleManifest {
        ...
        // None means "no authority constraint" for a synthetic test manifest
        // that never boots a real Vz machine. Mac VZ tests don't exercise the
...
async fn vz_provider_load_returns_rootfs_not_found_for_missing_capsule() {
    // Input validation happens before substrate availability checks.
    // A missing rootfs is surfaced as CapsuleNotFound regardless of
    // platform; the unavailable-backend message is reserved for the
    // non-macOS fall-through covered below.
    let provider = VzProvider::with_defaults().unwrap();
...
#[tokio::test]
async fn vz_provider_load_fails_closed_when_backend_unavailable_on_non_macos() {
    // On Linux a perfectly-formed capsule still cannot boot
    // because Apple's Vz framework is unreachable. After
    // input validation, load() must surface the shared
    ...
    let err = provider
        .load(
            &capsule_dir,
            microvm_manifest("smoke-unavailable-non-macos"),
        )
        .await
        .unwrap_err();
    let msg = err.to_string();
    assert!(
        msg.contains(elastos_vz::VZ_BACKEND_UNAVAILABLE_MESSAGE),
        "expected unavailable-backend message: {msg}"
    );
    assert!(
        msg.contains("load_with_vm_config"),
        "expected failing call-site context: {msg}"
    );
}
...
async fn vz_provider_lifecycle_methods_fail_closed_for_unloaded_handle() {
    // start / status / info / http_port on an unknown handle return
    // CapsuleNotFound; stop is idempotent (Ok). Mirrors crosvm's
    // contract so the supervisor can treat both substrates uniformly.
    let provider = VzProvider::with_defaults().unwrap();
...
async fn vz_provider_vsock_connect_fails_closed_for_unknown_handle() {
    // Dialing a vsock connection on a capsule that the provider
    // does not own returns the same
