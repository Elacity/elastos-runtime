//! Guest-network configuration (macOS, data-only).
//!
//! Mirrors the public shape of [`elastos_crosvm::NetworkConfig`] so
//! the supervisor's existing call sites map cleanly. This module is
//! data-only; the FFI builder owns actual `VZNATNetworkDeviceAttachment`
//! and bridged-device construction.
...
pub struct NetworkConfig {
    ...
    /// Reserved for compatibility with the crosvm-shaped config.
    _tap_fd: AtomicI32,
}
...
impl NetworkConfig {
    ...
    /// Set up the network. This data-only module fails closed; the
    /// FFI builder attaches Vz network devices during VM construction.
    pub fn setup(&self) -> Result<()> {
        Err(ElastosError::Compute(format!(
            "{} (NetworkConfig::setup: tap='{}')",
            VZ_BACKEND_UNAVAILABLE_MESSAGE, self.tap_name
        )))
    }

    /// Tear down the network. idempotent no-op because
    /// nothing was set up.
...
mod tests {
    ...
    #[test]
    fn network_setup_fails_closed_when_backend_unavailable() {
        let config = NetworkConfig::new("vm-network-unavailable");
        let err = config.setup().unwrap_err();
        assert!(err.to_string().contains(VZ_BACKEND_UNAVAILABLE_MESSAGE));
    }

    #[test]
    fn network_teardown_is_noop_ok() {
        let config = NetworkConfig::new("vm-network-teardown");
