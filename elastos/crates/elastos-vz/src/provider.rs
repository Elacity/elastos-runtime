...
//! [`crate::VZ_BACKEND_UNAVAILABLE_MESSAGE`] until the runtime supports
...
//! framework fails closed with the shared unavailable-backend
...
// migration-error path. On macOS-prod the non-test reference is
// `cfg`-gated out, so the import is technically unused there.
#[cfg_attr(all(target_os = "macos", not(test)), allow(unused_imports))]
use crate::VZ_BACKEND_UNAVAILABLE_MESSAGE;

/// Apple Virtualization.framework compute provider.
///
/// Apple's threading rules apply per `VZVirtualMachine` instance,
/// not per process, so each `VzMachineHandle::new` constructs a
/// fresh serial queue labelled with the VM's `vm_id`. A single
/// provider can host concurrent VMs without serializing all of them
/// on one GCD queue.
...
impl ComputeProvider for VzProvider {
    async fn load(&self, path: &Path, manifest: CapsuleManifest) -> Result<CapsuleHandle> {
        ...
        // production launch paths reject
        ...
        // manifest doesn't carry an initramfs path
        // (`elastos-common` is Linux-untouched), so
...
impl VzProvider {
    ...
    ///
    /// Supervisor-facing load seam. Apple's
    /// `VZVirtualMachineConfiguration` is frozen the moment
    /// `VZVirtualMachine::initWithConfiguration:queue:` is invoked,
    /// so every boot arg (session token, command payload, capsule
    /// args, carrier path, …) must be baked into the `VmConfig`
    /// before `load`. This mirrors the Linux crosvm composition flow.
    ///
    ...
    pub async fn load_with_vm_config(
        ...
        {
            // Non-macOS keeps the fail-closed contract: there is
            // no Vz framework here. The workspace builds, but
            // every load resolves to the shared unavailable-backend
            // message.
            let _ = (vm_config, socket_path); // silence unused
            Err(ElastosError::Compute(format!(
                "{} (load_with_vm_config: capsule='{}')",
                VZ_BACKEND_UNAVAILABLE_MESSAGE, manifest.name
            )))
        }
    }
    ...
    ///
    /// The supervisor's launch flow on Mac is:
    ///
    ...
    /// Save a loaded VZ VM to disk. This is intentionally outside
    /// the generic [`ComputeProvider`] trait because hibernation
    /// is substrate-specific and must not be implied on platforms
    /// without a real save/restore primitive.
    pub async fn hibernate_to(&self, handle: &CapsuleHandle, path: &Path) -> Result<()> {
        let mut vms = self.vms.write().await;
        let vm = vms
            .get_mut(&handle.id)
            .ok_or_else(|| ElastosError::CapsuleNotFound(handle.id.0.clone()))?;
        vm.hibernate_to(path).await
    }

    /// Restore a loaded VZ VM from a saved machine-state file and
    /// resume it.
    pub async fn restore_from_hibernation(
        &self,
        handle: &CapsuleHandle,
        path: &Path,
    ) -> Result<()> {
        ...
        let vm = vms
            .get_mut(&handle.id)
            .ok_or_else(|| ElastosError::CapsuleNotFound(handle.id.0.clone()))?;
        vm.restore_from_hibernation(path).await
    }

    /// Whether the loaded VM configuration supports VZ
    /// save/restore.
    pub async fn supports_hibernation(&self, handle: &CapsuleHandle) -> Result<bool> {
        let vms = self.vms.read().await;
        let vm = vms
            .get(&handle.id)
            .ok_or_else(|| ElastosError::CapsuleNotFound(handle.id.0.clone()))?;
        Ok(vm.supports_hibernation())
    }

    /// Unsupported after VM load. Apple's
    /// `VZVirtualMachineConfiguration` is frozen post-init; no
    /// session credentials can be applied after the VM has been
    /// loaded. Bake session args into [`VmConfig::boot_args`] (use
    ...
    /// late-binding API. It always fails closed.
    pub async fn set_session_for_vm(
        ...
        Err(ElastosError::Compute(format!(
            "vz: set_session_for_vm is unsupported — \
             ...
             (capsule='{}'; see docs/MAC.md)",
            capsule_id.0
        )))
    }

    /// Unsupported after VM load. Late-binding network attachment
    /// is not possible on Vz because Apple's
    ...
    /// support requires the Apple entitlement.
    ///
    ...
    pub async fn set_network_for_vm(
        ...
        Err(ElastosError::Compute(format!(
            "vz: set_network_for_vm is unsupported — \
             ...
             attached automatically by the builder. Bridged mode requires \
             the `com.apple.vm.networking` Apple \
             entitlement. \
             (capsule='{}'; see docs/MAC.md)",
            capsule_id.0
        )))
    ...
    /// Unsupported after VM load. Apple's
    /// `VZVirtualMachineConfiguration` is frozen post-init; no
    /// boot arg can be appended after the VM has been loaded. Bake
    /// every boot arg into [`VmConfig::boot_args`] before calling
    /// [`Self::load_with_vm_config`].
    ///
    /// This method is intentionally kept to surface a clear,
    /// typed migration message to any caller still on the
...
mod tests {
    ...
    async fn vz_provider_load_returns_capsule_not_found_when_rootfs_missing() {
        // This used to assert VZ_BACKEND_UNAVAILABLE_MESSAGE here.
        // The provider now validates inputs in the same order
    ...
    #[tokio::test]
    async fn vz_provider_set_session_for_vm_returns_typed_migration_error() {
        // The mid-life-cycle set_session_for_vm shape is unsupported because
        // VZVirtualMachineConfiguration is frozen after load.
        // The method exists only to surface a typed migration
        // message pointing callers at VmConfig::with_session +
        // load_with_vm_config.
        let provider = VzProvider::new(VzConfig::default()).unwrap();
        let capsule_id = CapsuleId::new("vz-session-mutation".to_string());
        ...
        // Must NOT carry the unavailable-backend message.
        assert!(
            !msg.contains(VZ_BACKEND_UNAVAILABLE_MESSAGE),
            "set_session_for_vm should not use the unavailable-backend message; got: {msg}"
        );

        // Must point at the correct replacement API.
        assert!(
    ...
    #[tokio::test]
    async fn vz_provider_set_network_for_vm_returns_typed_migration_error() {
        // Late-binding network attachment is unsupported; default
        // Vz networking is NAT (attached by the builder). Bridged
        // mode requires Apple's `com.apple.vm.networking` entitlement.
        let provider = VzProvider::new(VzConfig::default()).unwrap();
        let capsule_id = CapsuleId::new("vz-network-mutation".to_string());

        let err = provider
            .set_network_for_vm(&capsule_id, NetworkConfig::new("vz-network-mutation"))
            .await
            .unwrap_err();
        let msg = err.to_string();
        ...
        assert!(
            !msg.contains(VZ_BACKEND_UNAVAILABLE_MESSAGE),
            "set_network_for_vm should not use the unavailable-backend message; got: {msg}"
        );
        assert!(
            msg.contains("NAT"),
    ...
    #[tokio::test]
    async fn vz_provider_append_boot_args_returns_typed_migration_error() {
        // The mid-life-cycle append_boot_args_for_vm shape is unsupported because
        // VZVirtualMachineConfiguration is frozen after load.
        // The method exists only to surface a typed migration
        // message pointing callers at load_with_vm_config.
        let provider = VzProvider::new(VzConfig::default()).unwrap();
        let capsule_id = CapsuleId::new("vz-config-bootargs".to_string());
        ...
        // Must NOT carry the shared unavailable-backend message.
        assert!(
            !msg.contains(VZ_BACKEND_UNAVAILABLE_MESSAGE),
            "append_boot_args_for_vm should not use the unavailable-backend message; got: {msg}"
        );
    ...
    async fn vz_provider_load_with_vm_config_rejects_non_microvm_capsule_type() {
        // load_with_vm_config enforces the same MicroVM-only
        // constraint as load(), so wrong
    ...
    async fn vz_provider_load_with_vm_config_returns_kernel_not_found_when_kernel_missing() {
        // load_with_vm_config validates inputs in a stable order:
        // capsule type, then kernel, then rootfs. The supervisor
        // can rely on this order when surfacing typed errors
        // upstream.
        let provider = VzProvider::new(VzConfig::default()).unwrap();
        let manifest = capsule_manifest("vz-config-no-kernel", CapsuleType::MicroVM);
        let vm_config = synthetic_vm_config(
            "vz-config-no-kernel",
            PathBuf::from("/nonexistent/kernel-for-vz-config"),
            PathBuf::from("/nonexistent/rootfs-for-vz-config"),
        );
    ...
    async fn vz_provider_load_with_vm_config_preserves_baked_boot_args() {
        // Any boot args the supervisor bakes into VmConfig.boot_args
        ...
        let mut vm_config = synthetic_vm_config(
            "vz-config-bootargs",
            PathBuf::from("/nonexistent/kernel-vz-config-bootargs"),
            PathBuf::from("/nonexistent/rootfs-vz-config-bootargs"),
        );
