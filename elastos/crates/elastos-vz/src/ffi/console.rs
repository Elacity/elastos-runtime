...
//!    console (macOS 12+). We use this for the **Carrier bridge**;
//!    the bridge connects on
//!    `/dev/hvc1` (`crate::CARRIER_GUEST_DEVICE_PATH`), the second
//!    virtio-console port because the first is the kernel console.
//!
//! Never back the console with a regular file. An ever-growing
...
///
/// * **Interactive-stdio**: Vz is wired directly to the operator's
///   terminal — guest output prints on stdout and Vz
...
/// build the kernel-console attachment wired to
...
///
/// Build the `socketpair(AF_UNIX, SOCK_STREAM)`-backed Carrier
/// bridge channel:
///
...
///
/// the Carrier console wires the host side to
...
mod tests {
    ...
    fn kernel_console_produces_an_owned_host_read_fd() {
        ...
        // test before the main VM path wires the forwarder.
        let _clone = host_read.try_clone().expect("clone host_read");
    }

    /// the interactive variant must NOT produce
    ...
    /// The Carrier console is backed by a real
