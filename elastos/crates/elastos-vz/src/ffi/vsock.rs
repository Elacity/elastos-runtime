...
/// an owned host-side fd connected to it. **Phase 3 Day 5.**
///
...
/// `VZFileHandleSerialPortAttachment` pattern Day 4 used for
/// the Carrier console.
...
mod tests {
    ...
    /// Phase 3 Day 5 pure-string contract: the error message
    /// shape downstream operators see when a vsock connect is
    /// surfaced via `RunningVm::connect_vsock` embeds the
    /// vm_id and port so log-grep stays useful.
    #[test]
