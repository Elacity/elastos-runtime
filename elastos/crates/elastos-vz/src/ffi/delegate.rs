...
//! observation.
//!
//! Phase 3 Day 5 replaced the Day-3 polling `wait_for_exit_code`
//! with a delegate-driven `oneshot`: when the guest cleanly
//! stops, or when Apple tears the VM down because of an error,
//! the corresponding delegate method fires and we signal the
//! supervisor with a typed exit reason.
//!
