...
//!
//! Apple's `VZNATNetworkDeviceAttachment` gives every VM outbound NAT
//! plus DHCP through macOS's shared internet, with no
//! entitlement beyond `com.apple.security.virtualization`
//! (which the signed runtime carries). It is the default network
//! attachment. `VZBridgedNetworkDeviceAttachment` requires the
//! additional `com.apple.vm.networking` entitlement and remains opt-in.
//!
//! Constructed devices:
...
/// stable MAC across reboots should use the bridged path where
/// the MAC is derived from `NetworkConfig`.
...
/// pick the interface yet (`NetworkConfig.bridge_interface`). For now we use the first
