...
//!
//! A generic platform's `machineIdentifier` is the closest thing Vz
...
/// not used by the main VM path yet. Renders the on-disk path Vz will
...
mod tests {
    ...
    fn different_vm_ids_get_different_identifiers() {
        ...
        assert_ne!(
            a_bytes, b_bytes,
            "distinct VM ids must mint distinct identifiers"
        );
    }
