...
//!
//! Phase 0 §D pitfall #10 (`docs/vz-backend/PHASE_0_SCOPE.md`) made
//! "one queue per `VzProvider`" the design choice; this module is
//! that single source of truth. Day 1's reality probe verified the
//! same `dispatch2::DispatchQueue::new(label, None)` constructor
//! produces a usable serial queue on this host.
//!
//! Day 2 deliverable: a thin owned wrapper so Phase 2 main has one
//! type to pass into the (Day 3) `VZVirtualMachine` init site. The
...
impl VzDispatchQueue {
    ...
    /// Borrow the raw `DispatchQueue`. Phase 2 main will hand this
