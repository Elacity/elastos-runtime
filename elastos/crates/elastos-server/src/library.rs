...
///
/// This bridge keeps publish/share/status effects provider-mediated so Library
/// stays UI/object scoped instead of becoming a content or storage backend.
...
fn read_library_file_bytes(
    ...
    match crate::auth::read_principal_root_object(
        ...
        Ok(bytes) => Ok(bytes),
        Err(err) if is_unencrypted_principal_root_object(&err) => {
            protect_existing_plaintext_library_file(data_dir, principal_id, target)
        }
        Err(err) => Err(err),
    }
}
...
fn protect_existing_plaintext_library_file(
    data_dir: &Path,
    principal_id: &str,
    target: &LibraryTarget,
) -> anyhow::Result<Vec<u8>> {
    let bytes = fs::read(&target.path).with_context(|| {
        format!(
            "failed to read existing plaintext library object {:?}",
            target.path
        )
    })?;
    crate::auth::write_principal_root_object(
