...
pub fn media_play_route(session_id: &str) -> String {
    format!("/apps/{}/?session={}", MEDIA_VIEWER_CAPSULE, session_id)
}
...
pub fn remove_media_session(session_id: &str) -> bool {
...
/// Release every expired media session NOW (the periodic-sweeper hook). Same sweep the
...
pub(super) fn sweep_media_sessions(now: u64) -> usize {
    let swept = {
        let mut map = store().lock().expect("media session store poisoned");
        super::session_bounds::sweep_expired(&mut map, now, &|s: &Arc<MediaSession>| s.expires_at)
    };
...
/// The media store's registration with the session-lifecycle foundation.
pub(super) struct MediaSessionLifecycle;
pub(super) static MEDIA_SESSION_LIFECYCLE: MediaSessionLifecycle = MediaSessionLifecycle;

impl super::session_lifecycle::SessionLifecycle for MediaSessionLifecycle {
    fn kind(&self) -> &'static str {
        "media"
    }
    fn close(&self, session_id: &str) -> bool {
        remove_media_session(session_id)
    }
    fn sweep(&self, now: u64) -> usize {
        sweep_media_sessions(now)
    }
}
...
/// POST /api/viewers/:viewer/media/:session/close — explicit release of a media session
/// (the player's `pagehide` beacon and the Home shell's window-close hook land here).
///
...
/// is surfaced.
pub async fn viewer_media_close(
    State(state): State<GatewayState>,
    Path((viewer, session_id)): Path<(String, String)>,
    headers: HeaderMap,
) -> Response {
    match authorize_media_session(&state.data_dir, &headers, &viewer, &session_id) {
        Ok(_session) => {
            super::session_lifecycle::close("media", &session_id);
...
mod tests {
    ...
    /// Session-lifecycle foundation: an explicit close removes the session (releasing what
    /// it pins) and a repeated close is a harmless no-op — the close routes and the shell's
    /// window hook may both fire for the same session.
    #[test]
    fn explicit_close_removes_the_session_and_is_idempotent() {
        let id = "lifecycle-close-media-unique";
        put_media_session(id, session(1, 9_000_000_000));
        assert!(remove_media_session(id), "first close removes the session");
    ...
    /// Session-lifecycle foundation: the periodic sweeper hook releases expired sessions on
    /// the clock — no client close and no store access required — and leaves live ones alone.
    #[test]
    fn the_periodic_sweep_hook_releases_only_expired_sessions() {
        let dead = "lifecycle-sweep-dead-unique";
        let live = "lifecycle-sweep-live-unique";
        put_media_session(live, session(1, 9_000_000_000));
        put_media_session(dead, session(1, 1)); // expired long ago; inserted LAST
                                                // The store is process-global and shared across parallel tests (any admission or
                                                // lookup also sweeps), so assert the resulting STATE, not the returned count — the
                                                // count contract is pinned by the session_lifecycle probe tests.
        sweep_media_sessions(crate::auth::now_ts());
        assert!(get_media_session(dead).is_none(), "expired ⇒ released");
        assert!(get_media_session(live).is_some(), "live ⇒ untouched");
