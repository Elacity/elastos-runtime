...
///
/// Covers BOTH levers that can defeat the on-chain rights check: the mode itself
/// (`ELASTOS_DDRM_RIGHTS`) and the SUBJECT it runs against (`ELASTOS_DDRM_SUBJECT`). A pinned
/// subject is not a lesser setting — it points every principal's rights check at one operator-
/// chosen wallet, so a wallet that holds the access token unlocks the asset for everyone on the
/// node. Same outcome as `dev` mode, different door.
pub(crate) fn enforce_release_build_rights_safety() -> Result<(), String> {
    #[cfg(not(feature = "dev-modes"))]
    {
        ...
        if env_nonempty("ELASTOS_DDRM_SUBJECT").is_some() {
            return Err(
                "ELASTOS_DDRM_SUBJECT pins ONE operator-chosen wallet as the subject of every \
                 on-chain rights check, regardless of who is signed in — a free-content lever, not \
                 a deployment knob. This binary is a release build compiled WITHOUT the `dev-modes` \
                 feature. Refusing to start (fail closed). Unset ELASTOS_DDRM_SUBJECT for \
                 production, or rebuild with `--features dev-modes` for local/CI."
                    .to_string(),
            );
        }
    }
...
mod tests {
    ...
    /// RELEASE posture, subject pin: `ELASTOS_DDRM_SUBJECT` forces EVERY on-chain rights check
    /// onto one operator-chosen wallet, regardless of who is signed in. Point it at a wallet that
    /// holds the access token and every principal on the node unlocks every asset — the same
    /// free-content lever `ELASTOS_DDRM_RIGHTS=dev` is, reached by a different door. It belongs to
    /// the dev toolbox, so a release build refuses to start with it set (fail closed, loudly).
    #[test]
    #[cfg(not(feature = "dev-modes"))]
    fn release_build_refuses_a_pinned_drm_subject() {
        let _g = crate::api::ddrm_env_lock();
        std::env::remove_var("ELASTOS_DDRM_RIGHTS");

        std::env::set_var(
            "ELASTOS_DDRM_SUBJECT",
            "0x1111111111111111111111111111111111111111",
        );
        let err = enforce_release_build_rights_safety()
            .expect_err("release build must refuse to start with a pinned DRM subject");
        assert!(
            err.contains("ELASTOS_DDRM_SUBJECT") && err.contains("Refusing to start"),
            "unexpected guard error: {err}"
        );

        // Blank/whitespace is "unset" everywhere else in the runtime — it must not trip the guard.
        std::env::set_var("ELASTOS_DDRM_SUBJECT", "   ");
        assert!(
            enforce_release_build_rights_safety().is_ok(),
            "an empty subject pin is not a pin"
        );
        std::env::remove_var("ELASTOS_DDRM_SUBJECT");
        assert!(enforce_release_build_rights_safety().is_ok());
    }

    /// DEV posture, subject pin: local development still pins a subject wallet freely.
    #[test]
    #[cfg(feature = "dev-modes")]
    fn dev_build_allows_a_pinned_drm_subject() {
        let _g = crate::api::ddrm_env_lock();
        std::env::set_var(
            "ELASTOS_DDRM_SUBJECT",
            "0x1111111111111111111111111111111111111111",
