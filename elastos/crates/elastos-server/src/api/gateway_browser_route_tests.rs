...
async fn test_browser_open_launches_engine_with_attached_stream_receipt() {
    ...
    #[cfg(unix)]
    {
        // String prefix (not Path::starts_with, which is component-wise): the dir is euid-scoped
        // (…-streams-<euid>/), so the leaf component is not literally "elastos-browser-streams".
