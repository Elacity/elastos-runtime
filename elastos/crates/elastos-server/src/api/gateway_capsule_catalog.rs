...
mod tests {
    ...
    fn capsule_interface_registry_lists_declared_affordances() {
        ...
        let catalog = capsule_catalog_summary(data_dir.path());
        // The catalog also scans the repo's DEV_CAPSULES_ROOT, whose dev capsules
        // legitimately declare their own interfaces, so global counts are not
        // hermetic. Assert on the capsule this test actually controls.
        let marketplace = catalog
            .capsules
            .iter()
            .find(|capsule| capsule.name == "marketplace")
        ...
        let marketplace_entry = registry
            ...
            .expect("marketplace interface is registered");
