...
fn browser_stream_socket_path(stream_id: &str, directory: &str) -> anyhow::Result<PathBuf> {
    ...
    // Unix socket paths have a small platform limit. Keep Browser stream sockets in a short base
    // (`/tmp`, via `browser_runtime_stream_root`) rather than platform temp roots like macOS
    // `/var/folders/.../T` (which would overflow `sun_path`).
    //
    // SECURITY: that base is world-writable, so a FIXED directory name there could be pre-created and
    // owned by any local user, who could then unlink our deterministically-named socket and re-bind
    // their own to MITM or DoS the decrypted stream. Scope the directory to our euid, and (in
    // `ensure_private_stream_dir`) create it 0700-owned-by-us, refusing any pre-existing directory we
    // do not own or that is group/other-writable.
    let stream_dir = browser_runtime_stream_root().join(scoped_stream_dir_name(directory));
    ensure_private_stream_dir(&stream_dir)?;
    Ok(stream_dir.join(socket_name))
}

/// Scope a Browser stream socket directory name to the current euid, so the (shared `/tmp`) base is
/// not a fixed name any local user could pre-create and own. Applies to both socket directories
/// (`BROWSER_RUNTIME_STREAM_TMP_DIR` and `BROWSER_ADAPTER_IPC_TMP_DIR`).
#[cfg(unix)]
fn scoped_stream_dir_name(directory: &str) -> String {
...
mod tests {
    ...
    async fn attach_runtime_stream_path_repairs_adapter_ipc_missing_path() {
        ...
        // Socket dirs are euid-scoped for multi-user safety (…-ipc-<euid>/, …-streams-<euid>/).
