...
async fn test_library_gateway_lists_webspaces_through_runtime_provider() {
    ...
    let google = root_objects
        .iter()
        .find(|object| object["uri"] == "localhost://WebSpaces/Google")
        .expect("indexed Google WebSpace mount should be listed");
    assert_eq!(google["kind"], "directory");
    assert_eq!(google["metadata"]["target_uri"], "google://drive");
    assert_eq!(google["metadata"]["resolver"], "google-drive");
    assert_eq!(
        google["metadata"]["cache_policy"],
        "metadata-and-thumbnails"
    );
    assert_eq!(google["metadata"]["webspace_kind"], "mounted-webspace");
    ...
    let (google_status, google_list) = post_library(
        app.clone(),
        &token,
        "list",
        json!({
            "uri": "localhost://WebSpaces/Google",
        }),
    )
    .await;
    assert_eq!(google_status, StatusCode::OK);
    let google_names: Vec<&str> = google_list["data"]["objects"]
        .as_array()
        .unwrap()
        .iter()
        .filter_map(|object| object["name"].as_str())
    ...
    for expected in ["_meta.json", "Drive", "Shared"] {
        assert!(
            google_names.contains(&expected),
            "missing indexed Google child {expected}"
        );
    }
    ...
    let (project_status, project) = post_library(
        ...
        "list",
        json!({
            "uri": "localhost://WebSpaces/Google/Drive/Project X",
        }),
    )
    .await;
    ...
    let file = project["data"]["objects"]
        ...
        .expect("indexed Google file should be listed");
    assert_eq!(
        file["metadata"]["target_uri"],
        "google://drive/Drive/Project X/file.pdf"
    );
    assert_eq!(file["metadata"]["resolver"], "google-drive");
...
async fn test_library_gateway_reads_external_webspace_file_through_adapter_cache() {
    ...
    let uri = "localhost://WebSpaces/Google/Drive/Project X/file.pdf";
    let (read_status, read) = post_library(
        app,
        &token,
        "read",
    ...
    let bytes = base64::engine::general_purpose::STANDARD
        .decode(read["data"]["data"].as_str().unwrap())
        .unwrap();
    assert_eq!(bytes, b"google adapter bytes");
}
