...
fn load_existing_gateway_runtime_did(data_dir: &std::path::Path) -> Option<String> {
    // A subordinate managed runtime is handed its parent's trusted signer DID directly (it may have
    // no local device key of its own), so an explicit env injection wins outright — checked first and
    // never cached (the parent can rotate it).
