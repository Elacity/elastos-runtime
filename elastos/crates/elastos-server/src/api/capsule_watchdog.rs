...
mod tests {
    ...
    fn every_command_spawn_site_is_a_known_seam_and_capsule_seams_strip_secrets() {
        ...
        const HOST_TOOL_FILES: &[&str] = &[
            ...
            // node integration launcher for the remote-VZ browser transport (a repo script,
            // not an operator-pinned provider capsule).
            "crates/elastos-server/src/api/gateway_browser_transport.rs",
            // the Home terminal PTY: spawns the operator's own login shell, not a capsule.
            "crates/elastos-server/src/api/gateway_home_terminal.rs",
