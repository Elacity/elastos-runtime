...
pub fn build_capability_resource(
    ...
    match scheme {
        ...
        "peer" => simple_elastos_resource(
            "peer",
            op,
            // `connect` plus the gossip/topic messaging ops the carrier peer provider serves; the
            // per-op required action (read/message/execute) is enforced separately by
            // `required_action_for` (PRE-AUDIT #3). Unlisted ops (e.g. `raw_socket`) fail closed.
            &[
                "connect",
                "gossip_send",
                "gossip_recv",
