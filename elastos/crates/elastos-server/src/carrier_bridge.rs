...
///
/// Reads bytes from `reader` into `buf` until either a newline is
/// encountered, EOF is reached, or `max_bytes` is exceeded. Returns
/// the number of bytes read (NOT including the limit-check byte).
/// On `Ok(0)` the caller MUST treat it as EOF. On `Ok(n)` where
/// `n > max_bytes - 1` the caller MUST treat the line as oversized
/// and resync via [`drain_to_newline`].
    ...
    R: tokio::io::AsyncBufRead + Unpin,
{
    ...
    loop {
        ...
        let (consume_n, found_newline) = match available.iter().position(|&b| b == b'\n') {
            Some(idx) => (idx + 1, true),
            None => (available.len(), false),
        };
        let take = consume_n.min(max_bytes.saturating_sub(buf.len()));
...
/// **Phase 10.5 M1 — drain helper.**
///
/// Reads and discards bytes from `reader` until the next newline (or
/// EOF). Used to resync the stream after [`read_line_byte_budgeted`]
/// reports an oversized line, so the next well-formed request lines
/// up cleanly.
...
pub struct BridgeContext {
    ...
    /// Optional bridge-termination observer. **Phase 4 Day 6 (Mac VZ).**
    ///
    /// When set, [`run_carrier_bridge_loop`] calls
    /// `notify.notify_waiters()` on EVERY exit path (EOF, read error,
    /// write error, oversized-line teardown) before returning. Lets
    /// the supervisor's `stop_capsule` await the bridge's natural
    /// termination after `vm.stop()` resolves, rather than relying on
    /// send-and-pray observability.
    ///
    /// `None` for legacy callers (Linux crosvm bridges, WASM-stdio
    /// bridges) where lifecycle observation is not wired today.
...
/// connects to at VM start, then hands the accepted stream into the
/// shared bridge loop. Must be called BEFORE starting the VM so the
/// socket exists when crosvm launches.
///
/// macOS / Vz capsules use [`spawn_carrier_bridge_on_stream`] instead,
/// because the host endpoint comes directly from a
/// `socketpair(AF_UNIX, SOCK_STREAM)` carrier-console attachment —
/// there is no listener to bind. The shared bridge loop is the same.
    ...
    tokio::spawn(async move {
        let (stream, _) = match listener.accept().await {
            ...
            Err(e) => {
                ...
                if let Some(notify) = ctx.as_ref().and_then(|c| c.on_terminate.clone()) {
        ...
        run_carrier_bridge_loop(stream, ctx, socket_display).await;
    });
...
/// `tokio::net::UnixStream` — **Phase 3 Day 4 (Mac VZ)** entry point
/// for the macOS / Vz flow.
///
/// On Mac, the host endpoint of the Carrier console is the host-side
/// fd of a `socketpair(AF_UNIX, SOCK_STREAM)` set up by
/// `elastos-vz::ffi::console::build_carrier_console_slot`. The
/// supervisor takes that fd via `RunningVm::take_carrier_host_fd`,
/// converts it to a `tokio::net::UnixStream`, and hands it to this
/// function — no listener / bind / accept needed.
///
/// The bridge dispatch loop is byte-identical to
/// [`spawn_carrier_bridge`]; only the connection-acquisition half
/// differs.
...
/// Shared per-connection bridge dispatch loop. Reads newline-delimited
/// JSON `RequestEnvelope`s off the supplied stream, dispatches them
/// through `bridge_ctx` providers, and writes `ResponseEnvelope`s back
/// on the same stream.
///
/// `label` is a human-readable identifier used in trace log lines
/// (socket path on Linux, capsule handle on Mac).
///
/// **Phase 4 Day 6 (Mac VZ):** when `bridge_ctx.on_terminate` is set,
/// fires `notify_waiters()` on every exit path (EOF, read error,
/// write error, oversized-line teardown) before returning.
///
/// **Phase 10.5 M1:** the framed-line read is byte-budgeted at
/// `CARRIER_MAX_LINE_BYTES + 1`. Pre-Phase-10.5 this was
/// `read_line(&mut line)`, which is unbounded — a guest could grow
/// the host's `String` to multi-GiB before the post-read length
/// check fired (which was too late by then).
