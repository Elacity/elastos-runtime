...
use elastos_server::content::{ContentProvider, ContentProviderExternalConfigs};
...
async fn setup_server_infrastructure_impl(
    ...
    let content_provider = Arc::new(ContentProvider::new_with_external_configs(
        data_dir.clone(),
        Arc::downgrade(&provider_registry),
        ContentProviderExternalConfigs {
            operator_alert_sink: content_operator_alert_sink_config_from_env(),
            storage_market_admission: content_storage_market_admission_config_from_env(),
            external_repair_fleet: content_external_repair_fleet_config_from_env(),
            federated_operator_alert_exchange:
                content_federated_operator_alert_exchange_config_from_env(),
            federated_quota_ledger_exchange:
                content_federated_quota_ledger_exchange_config_from_env(),
            federated_abuse_control_exchange:
                content_federated_abuse_control_exchange_config_from_env(),
        },
    ));
...
fn content_operator_alert_sink_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_OPERATOR_ALERT_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_OPERATOR_ALERT_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let url = std::env::var("ELASTOS_CONTENT_OPERATOR_ALERT_URL")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())?;
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_OPERATOR_ALERT_AUTHORIZATION") {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
        }
    }
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_OPERATOR_ALERT_TIMEOUT_SECS") {
...
fn content_federated_operator_alert_exchange_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) =
        std::env::var("ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_AUTHORIZATION")
    {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
    ...
    if let Ok(value) =
        std::env::var("ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_TIMEOUT_SECS")
    {
        if let Ok(timeout_secs) = value.trim().parse::<u64>() {
            config["timeout_secs"] = serde_json::Value::from(timeout_secs);
        }
...
fn content_federated_quota_ledger_exchange_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) =
        std::env::var("ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_AUTHORIZATION")
    {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
    ...
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_TIMEOUT_SECS")
    {
        if let Ok(timeout_secs) = value.trim().parse::<u64>() {
            config["timeout_secs"] = serde_json::Value::from(timeout_secs);
        }
...
fn content_federated_abuse_control_exchange_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) =
        std::env::var("ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_AUTHORIZATION")
    {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
    ...
    if let Ok(value) =
        std::env::var("ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_TIMEOUT_SECS")
    {
        if let Ok(timeout_secs) = value.trim().parse::<u64>() {
            config["timeout_secs"] = serde_json::Value::from(timeout_secs);
        }
...
fn content_storage_market_admission_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let url = std::env::var("ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_URL")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())?;
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_AUTHORIZATION") {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
        }
    }
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_TIMEOUT_SECS") {
        if let Ok(timeout_secs) = value.trim().parse::<u64>() {
            config["timeout_secs"] = serde_json::Value::from(timeout_secs);
        }
    }
...
fn content_external_repair_fleet_config_from_env() -> Option<serde_json::Value> {
    if let Ok(raw) = std::env::var("ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_CONFIG") {
        match serde_json::from_str::<serde_json::Value>(&raw) {
            Ok(value) => return Some(value),
            Err(err) => {
                tracing::warn!(
                    "Ignoring invalid ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_CONFIG JSON: {}",
                    err
                );
                return None;
            }
    ...
    let url = std::env::var("ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_URL")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())?;
    let mut config = serde_json::json!({ "url": url });
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_AUTHORIZATION") {
        let value = value.trim();
        if !value.is_empty() {
            config["authorization"] = serde_json::Value::String(value.to_string());
        }
    }
    if let Ok(value) = std::env::var("ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_TIMEOUT_SECS") {
        if let Ok(timeout_secs) = value.trim().parse::<u64>() {
            config["timeout_secs"] = serde_json::Value::from(timeout_secs);
        }
    }
...
mod tests {
    ...
    #[test]
    fn content_operator_alert_sink_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_OPERATOR_ALERT_CONFIG",
            "ELASTOS_CONTENT_OPERATOR_ALERT_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_OPERATOR_ALERT_CONFIG",
            r#"{"url":"https://alerts.example.invalid/content","authorization":"Bearer secret","timeout_secs":7}"#,
        );

        let config = content_operator_alert_sink_config_from_env().unwrap();
        assert_eq!(config["url"], "https://alerts.example.invalid/content");
        assert_eq!(config["authorization"], "Bearer secret");
        assert_eq!(config["timeout_secs"], 7);
    }

    #[test]
    fn content_operator_alert_sink_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_OPERATOR_ALERT_CONFIG",
            "ELASTOS_CONTENT_OPERATOR_ALERT_URL",
            "ELASTOS_CONTENT_OPERATOR_ALERT_AUTHORIZATION",
            "ELASTOS_CONTENT_OPERATOR_ALERT_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_OPERATOR_ALERT_URL",
            "http://127.0.0.1:9799/content-alerts",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_OPERATOR_ALERT_AUTHORIZATION",
            "Bearer local",
        );
        std::env::set_var("ELASTOS_CONTENT_OPERATOR_ALERT_TIMEOUT_SECS", "9");

        let config = content_operator_alert_sink_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/content-alerts");
        assert_eq!(config["authorization"], "Bearer local");
        assert_eq!(config["timeout_secs"], 9);
    }

    #[test]
    fn content_federated_operator_alert_exchange_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_CONFIG",
            r#"{"url":"https://alerts.example.invalid/exchange","authorization":"Bearer exchange","timeout_secs":8}"#,
        );

        let config = content_federated_operator_alert_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "https://alerts.example.invalid/exchange");
        assert_eq!(config["authorization"], "Bearer exchange");
        assert_eq!(config["timeout_secs"], 8);
    }

    #[test]
    fn content_federated_operator_alert_exchange_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_URL",
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_AUTHORIZATION",
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_URL",
            "http://127.0.0.1:9799/alerts/exchange",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_AUTHORIZATION",
            "Bearer local-exchange",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_OPERATOR_ALERT_EXCHANGE_TIMEOUT_SECS",
            "10",
        ...
        let config = content_federated_operator_alert_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/alerts/exchange");
        assert_eq!(config["authorization"], "Bearer local-exchange");
        assert_eq!(config["timeout_secs"], 10);
    }

    #[test]
    fn content_federated_quota_ledger_exchange_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_CONFIG",
            r#"{"url":"https://quota.example.invalid/exchange","authorization":"Bearer quota","timeout_secs":8}"#,
        );

        let config = content_federated_quota_ledger_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "https://quota.example.invalid/exchange");
        assert_eq!(config["authorization"], "Bearer quota");
        assert_eq!(config["timeout_secs"], 8);
    }

    #[test]
    fn content_federated_quota_ledger_exchange_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_URL",
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_AUTHORIZATION",
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_URL",
            "http://127.0.0.1:9799/quota/exchange",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_QUOTA_LEDGER_EXCHANGE_AUTHORIZATION",
            "Bearer local-quota",
        ...
        let config = content_federated_quota_ledger_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/quota/exchange");
        assert_eq!(config["authorization"], "Bearer local-quota");
        assert_eq!(config["timeout_secs"], 10);
    }

    #[test]
    fn content_federated_abuse_control_exchange_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_CONFIG",
            r#"{"url":"https://abuse.example.invalid/exchange","authorization":"Bearer abuse","timeout_secs":8}"#,
        );

        let config = content_federated_abuse_control_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "https://abuse.example.invalid/exchange");
        assert_eq!(config["authorization"], "Bearer abuse");
        assert_eq!(config["timeout_secs"], 8);
    }

    #[test]
    fn content_federated_abuse_control_exchange_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_CONFIG",
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_URL",
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_AUTHORIZATION",
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_URL",
            "http://127.0.0.1:9799/abuse/exchange",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_FEDERATED_ABUSE_CONTROL_EXCHANGE_AUTHORIZATION",
            "Bearer local-abuse",
        ...
        let config = content_federated_abuse_control_exchange_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/abuse/exchange");
        assert_eq!(config["authorization"], "Bearer local-abuse");
        assert_eq!(config["timeout_secs"], 10);
    }

    #[test]
    fn content_storage_market_admission_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_CONFIG",
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_CONFIG",
            r#"{"url":"https://market.example.invalid/admission","authorization":"Bearer market","timeout_secs":8}"#,
        );

        let config = content_storage_market_admission_config_from_env().unwrap();
        assert_eq!(config["url"], "https://market.example.invalid/admission");
        assert_eq!(config["authorization"], "Bearer market");
        assert_eq!(config["timeout_secs"], 8);
    }

    #[test]
    fn content_storage_market_admission_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_CONFIG",
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_URL",
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_AUTHORIZATION",
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_URL",
            "http://127.0.0.1:9799/market/admission",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_STORAGE_MARKET_ADMISSION_AUTHORIZATION",
            "Bearer local-market",
        ...
        let config = content_storage_market_admission_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/market/admission");
        assert_eq!(config["authorization"], "Bearer local-market");
        assert_eq!(config["timeout_secs"], 11);
    }

    #[test]
    fn content_external_repair_fleet_config_uses_explicit_json() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_CONFIG",
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_URL",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_CONFIG",
            r#"{"url":"https://repair.example.invalid/dispatch","authorization":"Bearer fleet","timeout_secs":8}"#,
        );

        let config = content_external_repair_fleet_config_from_env().unwrap();
        assert_eq!(config["url"], "https://repair.example.invalid/dispatch");
        assert_eq!(config["authorization"], "Bearer fleet");
        assert_eq!(config["timeout_secs"], 8);
    }

    #[test]
    fn content_external_repair_fleet_config_uses_env_endpoint() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::new(&[
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_CONFIG",
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_URL",
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_AUTHORIZATION",
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_TIMEOUT_SECS",
        ]);
        std::env::set_var(
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_URL",
            "http://127.0.0.1:9799/repair/dispatch",
        );
        std::env::set_var(
            "ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_AUTHORIZATION",
            "Bearer local-fleet",
        );
        std::env::set_var("ELASTOS_CONTENT_EXTERNAL_REPAIR_FLEET_TIMEOUT_SECS", "11");

        let config = content_external_repair_fleet_config_from_env().unwrap();
        assert_eq!(config["url"], "http://127.0.0.1:9799/repair/dispatch");
        assert_eq!(config["authorization"], "Bearer local-fleet");
        assert_eq!(config["timeout_secs"], 11);
    }
