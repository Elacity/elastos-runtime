...
async fn setup_server_infrastructure_impl(
    ...
    // Register llama/ai even on that path so /api/provider/ai/* is not mock-only.
    ...
    if let Some(path) = crate::find_installed_provider_binary("ai-provider") {
        ...
        let ollama_url = std::env::var("OLLAMA_URL").ok().filter(|v| {
        ...
        // Explicit OLLAMA_URL (Sparks / remote OpenAI-compat) is first-class.
        // Only wire llama local_url when OLLAMA_URL is unset so dogfood is not
        // overridden by a local llama endpoint.
        if ollama_url.is_none() {
