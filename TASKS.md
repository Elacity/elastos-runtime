...
| --- | --- | --- | --- |
| C1-site | A useful, truthful public storefront | Revised copy in user review / coordinator | One Linux command and a Mac developer-guide link replace the long install blocks. The page identifies the older public download and pending 0.7.1 downloads. Local content/layout checks pass. The isolated seed now serves matching website bytes; user copy review and public approval remain open. |
...
| C1-seed | Seed serves the reviewed local candidate and storefront | Private first-win checks pass / coordinator | Frozen 3e27cac8 passes 25 HTTP/artifact checks, desktop/mobile layout, passkey sign-out/sign-in, Recovery Kit and Profile creation. Test-session cleanup is complete. The current copy revision is staged with matching website and full artifact hashes; user review and public approval remain open. Journey donors enter C2–C5. |
| C1-delivery | Signed three-platform candidate installation | Active / installer owner | Local native preparation and unsigned platform-input admission pass fixture tests and independent review; real builds are next. Candidate bootstrap now runs setup and terminal Home, with install-only mode for automation. Complete publisher input import, dependency closure, ordered metadata/client rollout, real three-platform installs and browser Home entry. |
| J1 / C2 | Install, passkey, Recovery Kit, Profile and usable Home | Planned / Home and installer owners | Eight setup commits from the integration donor; W2-W5; rerun the smoke with System entry through the ElastOS menu, then complete the app matrix; full Linux/Mac staging. Publish J1 for review first under D1. |
