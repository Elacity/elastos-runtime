...
| C1-delivery | Signed three-platform candidate installation | Active / installer owner | All three native packages pass full file verification, including managed FFmpeg/FFprobe, and shared archives match. Installer completion, gateway readiness/shutdown and combined CLI tests pass. Publisher input import passes offline checks and review. Complete publication promotion, regenerate final inputs, and prove signed fresh installs with browser Home entry. |
