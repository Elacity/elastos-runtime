...
Executable closeout order, 20 September 2026. Notion owns acceptance. One card
is active. Automatic continuation applies only to a ready owned successor.
Publication and public deployment stay gated.
...
| 2 | CR1 | Done | Source: unset and `direct` start Isolated; `public` is an error; Isolated strips and rejects foreign relay hints before MemoryLookup and connect; default listener is Isolated. Receipt `.audit/cr1-one-network-policy-receipt.json`. Remaining: Public N0 stays a test constructor; approved relay is CR3. |
