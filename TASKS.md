...
| C1-delivery | Signed three-platform candidate installation | Active / installer owner | Candidate bootstrap runs setup and terminal Home. A real Mac build exposed missing media dependencies; managed FFmpeg/FFprobe delivery, explicit private import and fresh-directory permission tests now pass. The focused required video donor passes real provider/Runtime validation. Finish source-bound native inputs on all three platforms, publisher import, remaining dependencies, ordered metadata/client rollout, signed fresh installs and browser Home entry. |
