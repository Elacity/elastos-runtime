...
| C1-seed | Seed serves the reviewed local candidate and storefront | Private first-win checks pass / coordinator | Frozen 3e27cac8 passes 25 HTTP/artifact checks, desktop/mobile layout, passkey sign-out/sign-in, Recovery Kit and Profile creation. Target package is prepared. Close the test session, then request concrete public update approval. Journey donors enter C2–C5. |
| C1-delivery | Signed three-platform candidate installation | Active / installer owner | Bootstrap, canonical paths and process ownership pass 38 source tests. Exact signed head/release binding passes independent review, 42 installer tests and 10 updater tests. Next: complete three-platform artifacts, ordered candidate metadata/client rollout, isolated publisher and real platform smoke assertions. |
