...
| C1-delivery | Signed three-platform candidate installation | Active / installer owner | Bootstrap, canonical paths and process ownership pass 38 source tests. Exact signed head/release binding passes independent review, 42 installer tests and 10 updater tests. Full-platform manifest export/ledger repair passes 21 publisher tests and three shell export cases. Native/guest target selection and ARM musl preflight are also repaired and reviewed. Universal app staging and pre-signing byte checks now pass five regression groups and are wired into CI. Next: complete three-platform input admission and external dependencies, ordered candidate metadata/client rollout, isolated publisher and real platform smoke assertions. |
