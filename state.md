...
| CID-scoped Content receipt source repair | `e06950d5`; private `public-ledger-repair-cause.private.json` | A genuinely signed old-format row before the requested CID made the unmodified bounded-index fetch fail with `missing field peer_selection`; the repair returns exact local bytes for the requested current signed row. A matching tampered row and a matching old row remain errors, and whole-ledger dashboard status reports the incompatible ledger. Seventeen fetch, seven status, seventeen publish and one unpublish tests passed, as did formatting, diff and Home entropy checks. Independent review found no blocking issue. | This is source proof, not an installed Linux or public fix. No signed ledger row was defaulted, rewritten or reissued. Linux staging fell below the 10% free-space floor after other build/cache growth, so an exact cache reclaim approval and fresh reserve check precede the isolated build. The public repair candidate needs fresh artifact/rollback review and approval. |
