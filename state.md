...
| --- | --- | --- | --- |
| Local source candidate | `feat/0.7.1-model-assistant-closeout`; code-bearing SmolLM2 revision `a1573675`; foundation documentation revision `6a4ddfc0` (tree `e33bac9c`); `.audit/local-smol-readiness-installed.json` | Five-field offers and distinct IDs pass focused readiness checks. | The documentation revision adds no product proof; the Owner installation below is bound to `a1573675`, and public installation remains separate. |
...
| Public Runtime and provider | `f087a0d9` (tree `87e61b3e`); `.audit/public-seed-gate-61974/receipt.json` and private `public-metadata-read-diagnosis.json`; Runtime SHA-256 `62ddda9a31b9b512ebb82deb6c8152229b1c84dee969cf8621e2c742755a1d70`, ipfs-provider SHA-256 `0609b6e089e8ab08254f41c4f685a39c6c7d5c8bc8cfe24a6b1457705df3152e` | The running public Runtime/provider match installed hashes. Public Home 8090 and holder 61954 use separate data roots; both Kubo repositories now return the same 726-byte index for the signed SmolLM2 CID offline. | Six Marketplace and one System Get records failed at `metadata_read` before a complete index was recorded, including three with the current installed provider. Partial in-flight bytes are unknown. The journal and gateway log omit the inner Content/provider error, so the exact substage remains unknown. Public Get and signed-in Assistant remain separate open proofs. |
| Bounded index diagnostic candidate | `43d8bd21` (tree `fcba5c4a`); private `index-substage-diagnostic.json`; isolated Linux source snapshot from `f087a0d9` plus patch SHA-256 `5c379d12`; Linux test binary SHA-256 `7c1e92c5` | A controlled 64 KiB index failure on Mac and isolated Linux source emits `local_bounded_index_cat` and `availability_fallback=failed`. A provider error does not claim Runtime stream validation. Static fields exclude the fixture CID and private marker. Independent review found no remaining actionable finding. | This is source and test-binary evidence, not an installed diagnostic Runtime or a diagnosis of the public failure. The isolated and public installed Runtime hashes remain `62ddda9a`. Linux free space after the build was 31,370,676/314,748,412 KiB (9.97%), below the 10% reserve. Stop Linux writes until capacity or safe reclaim restores the floor; public Get still needs an exact installed candidate and approval. |
...
| Isolated public-artifact Linux fixture | `.audit/public-seed-gate-61974/receipt.json` and `.audit/public-seed-gate-61974/engine-preflight-receipt.json` (the port is historical receipt metadata); copied public Runtime, provider and manifest hashes above, with separate identity and data root | The retained Linux data root and live process match the copied public Runtime/provider hashes. The pinned llama.cpp `b10516` engine receipt verifies 52 files and 10 symlinks; the running engine's mapped libraries match. The second existing typed SmolLM2 run has a terminal journal whose execution-binding hash recomputes from that engine SHA-256, admitted weights SHA-256 and bounded Linux profile. CPU, RAM and free space passed at observation. No new run was sent. Read-only DMI and cloud-init data identify a Hetzner virtual server. | The purchased server type/plan code is absent from inspected operator records and host metadata. Hetzner's shared and dedicated Cloud CPU policies differ; the shared FAQ gives no numeric baseline or burst duration for this bounded run. This plan-specific policy gate stays open. This isolated result is neither public Get nor proof of the local `a1573675` code path or signed-in UI. The exact owner fact and next check are in [TASKS Now](TASKS.md#now). |

The read-only public free-space reading in the metadata diagnosis is
36,180,800/314,748,412 KiB (11.49%). It is dated evidence. Before any later public action, obtain
a fresh read-only artifact and resource preflight. Get diagnosis can proceed
without a CPU-plan decision; seed-local Assistant inference needs that decision
and a verified engine. The next proof and public approval sequence are in
