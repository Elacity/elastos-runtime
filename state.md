...
- W1b per-TAP egress firewall (default-deny enforcement + signed `EgressDenied` custody), **VERIFIED on a real kernel** (`nft v1.0.9` + real NFLOG group 100, `flint` W1b-C4 turn, 2026-06-29, 7/7): a netns-peer harness drives the actual `EgressFirewall`/`NflogReader` code — rules install, the allowed host runtime-API is reachable while every other destination is dropped + counted + NFLOG'd (the compromised-guest backstop, both the `input` and `forward` hooks), per-drop `EgressDenied` (kernel NFLOG → signed custody, keyed `vm-{name}`) plus a flood→`suppressed` reconcile marker land on the durable chain and survive verify-on-open, and `nft list ruleset` is clean after teardown (no leak). The harness is a netns peer, not a booted microVM guest; the firewall is interface-keyed, so the guest-over-TAP attach is established by construction and already boot-proven (G-HWV). Host-only containment (no NAT); the `Allowlisted` internet-egress positive is intentionally NOT built (see `docs/KNOWN_GAPS.md` G-EGR; reproduce via `tests/c4_egress_spine.rs` with `ELASTOS_C4=1` + root + `modprobe nfnetlink_log`).
