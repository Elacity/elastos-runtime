...
| Isolated Owner/Consumer Homes 61986/61987 | Same source and Runtime hash above; separate owner-only identities and data roots. Consumer's ordinary signed SmolLM2 Get admitted 144,835,448 complete package bytes and a 144,811,072-byte GGUF with SHA-256 `c4a3dd03`. The private exact-reply receipt binds one Owner and one Consumer run record to the same grant and request. | Consumer Services received Owner's exact named SmolLM2 catalogue. The person approved its six-hour grant in Owner Inbox. Consumer Marketplace listed the exact shared offer, Assistant selected the remote Owner route, and one short prompt completed at Owner with a terminal reply visible in Consumer Home. Both Homes retained the grant, reply and installed model through restart. After Owner restart, one Consumer discovery used a stale isolated publisher ticket; refreshing that peer ticket and restarting Consumer restored the exact listing without another run. | This Consumer Get used the seed route; the separate-holder Get is in the next row. No second dispatch after Owner restart was attempted. End/refusal, second-principal denial, failed transfer, retry, recovery and sustained inference remain open. The stale publisher ticket was isolated route configuration, not a grant revision change. |
| Fresh isolated separate-holder Get, 24 September | `feat/0.7.1-model-assistant-closeout` document HEAD `4a857a58` (tree `58ca0e3e`); installed code `bb65974b` (tree `b8cd6199`); private `.audit/j3-fresh-holder-receiver-get.private.json` SHA-256 `a2fed516`. Fresh signed-in receiver Home 61988 and separate Owner holder 61986 have distinct device identities, matching built/installed/running Runtime SHA-256 `a7114f39`, signed catalogue SHA-256 `c81d4357` and verified b10516 engine SHA-256 `d0878274`. | From empty preparation and Kubo state, one ordinary Marketplace **Get** admitted signed SmolLM2: 726 index bytes and all 144,835,448 package bytes across five manifest-matching files; GGUF SHA-256 `c4a3dd03`. The receiver had one Owner source and zero HTTP gateways. Its local bounded index read missed, availability fallback completed, and its sole Carrier peer record identifies the Owner device with 2,215 successful interactions and zero failures. Owner Kubo woke during Get; directional process UDP traffic matched the transfer. Marketplace and System both show **Available on this device**; System shows Keep checked. That UI phrase requires Runtime `dispatch_ready=true`, admission and a valid offer ID. `activation_pending=false`. Across 92 samples free space stayed at least 20.79%. Independent review accepts this narrow direct separate-holder Get. | Peer reputation records successful fetch or ensure calls, not served bytes; UDP counters are process-wide. No byte-exact holder request transcript exists. This receiver has made no inference run and has not been restarted after Get. The public seed, public holder and human Homes were untouched. The authenticated post-End refusal, second-principal denial, restart dispatch, Stop, failure/retry/recovery and full J3 acceptance remain open. |
| Isolated exact grant End, 24 September | Same `bb65974b` source and `a7114f39` installed Runtime; private `.audit/j3-shared-local-exact-end-refusal.private.json` SHA-256 `c8962725`. | Owner used Inbox **Revoke access** on the one active exact SmolLM2 grant. Its active row disappeared while the older generic and Wallet requests remained. Consumer Services showed **Denied** after refresh, and Marketplace then showed no shared model. One attempt through the stale selected remote offer produced “Run acceptance is unknown”; Owner service runs and model-provider journal stayed at one prior completed run with no new pending entry or provider journal change. After an Owner restart, the End state and Consumer denial remained, and installed/served hashes and Home health matched. | This supports zero new Owner model adapter dispatch for the bounded attempt and proves removal of Consumer access. It does not prove that Owner received a create or sent an authenticated pre-dispatch refusal: the saved run ticket addressed the earlier Owner process, and Assistant reported unknown acceptance. The earlier successful reply and protected request state remain; a separate immutable history UI was not observed. The separate-holder Get is recorded above; second-principal denial and failure/retry/recovery stay open. |
| Isolated post-End ordinary UI check, 24 September | Same installed `bb65974b` code and `a7114f39` Runtime, clean document HEAD `9c1edeb6`; private `.audit/j3-post-end-ordinary-ui-route-gap.private.json` SHA-256 `f654dac6`. Current Owner/Consumer peer identities and tickets match, both Homes return 200, and the Mac retains over 10% free space. | A fresh Consumer Assistant chat offered only its own local SmolLM2. Services still showed the Owner model **Denied** and Marketplace showed no shared offer. No new prompt was submitted. Source routes fresh remote creates only through an approved Consumer grant; a stale remote selection is rejected locally before Carrier. Independent read-only review found no smaller ordinary UI route to an Owner-bound post-End refusal. | The requested authenticated Owner pre-dispatch refusal remains unproved. The earlier stale-selected attempt supports zero observed provider dispatch, but its unknown acceptance does not show whether Owner received it. A coordinated create/End race would first require a new active grant and a separate bounded experiment; the current ended grant and prior reply remain preserved. |
