...
- Installed protected-content e2e proof path:
  `scripts/protected-content-installed-e2e-proof.sh --phase <name>`, run
  against a real installed client Runtime and the `deploy/custody-host/`
  simulation-only three-node harness. `scripts/custody-harness-ci-smoke.sh`
  is the CI-safe rehearsal of its `provision` and `preflight` phases against
  a fresh throwaway instance of that harness.
